...
const TEST_LOCAL: boolean =
	hasSetting('remote') || !!process.env.TRAVIS ? false : TEST_LOCAL_DEFAULT;
...
function getTest(): InstallTest {
	...
	if (index === -1 || !process.argv[index + 1]) {
		...
		console.error('Choose from:');
		console.error(
			'Greasyfork, openuserjs, userscripts.org, userstyles.org or openusercss'
		);
		process.exit(1);
		return null;
	}
...
function installScriptFromInstallPage(
	...
	it('should be possible to open the install page', async function () {
		this.timeout(20000);
		this.slow(20000);
		const { prefix, url, href } = getConfig();
		await driver.get(
			`${prefix}/html/install.html?i=${encodeURIComponent(href)}&s=${url}`
		);
		await wait(5000);
	});
	...
	it('should have been installed into the CRM', async function () {
		this.timeout(600000 * TIME_MODIFIER);
		this.slow(600000 * TIME_MODIFIER);
		const { prefix, href, title } = getConfig();
...
function installStylesheetFromInstallPage(
	getConfig: () => {
		prefix: string | void;
		href: string;
	}
	...
	it('should have been installed into the CRM', async function () {
		...
		try {
			const descriptor = await new webdriver.promise.Promise<string>(
				(resolve) => {
					request(
						href,
						(
	...
	describe('Installing from greasyfork', () => {
		...
		it('should be possible to click the install link', async function () {
			...
			const button = await findElement(
				webdriver.By.className('install-link')
			);
		...
		it('should be applied', async function () {
			...
			assert.strictEqual(
				...
				'rgb(51, 51, 51)',
				'background color changed (script is applied)'
			);
		});
	});
}
...
function doOpenUserJsTest(prefix: () => string | void) {
	describe('Installing from OpenUserJS', () => {
		...
		it('should be possible to click the install link', async function () {
			...
			this.slow(15000);
			const button = await findElement(
				webdriver.By.tagName('h2')
			).findElement(webdriver.By.tagName('a'));
			title = await findElement(
				webdriver.By.className('script-name')
			).getText();
...
function doUserScriptsOrgTest(prefix: () => string | void) {
	describe('installing from userscripts.org', () => {
		...
		it('should be possible to click the install link', async function () {
			...
			this.slow(15000);
			const button = await findElement(
				webdriver.By.id('install_script')
			).findElement(webdriver.By.tagName('a'));
			title = await findElement(
				webdriver.By.className('title')
			).getText();
...
function doUserStylesOrgTest(prefix: () => string | void) {
	describe('Userstyles.org', () => {
		...
		it('should be possible to click the install link', async function () {
			...
			const button = await findElement(
				webdriver.By.id('install_style_button')
			);
			assert.exists(button, 'Install link exists');
		...
		it('should be applied', async function () {
			...
			assert.strictEqual(
				...
				'rgb(27, 27, 27)',
				'background color changed (stylesheet is applied)'
			);
		});
	});
}
		...
		it('should be possible to click the install link', async function () {
			...
			for (let i = 0; i < 4; i++) {
				const exists = await driver.executeScript(
					inlineFn(() => {
						const els: HTMLDivElement[] = Array.prototype.slice.apply(
							document.querySelectorAll('div')
						);
						const all = els.filter((d) => {
							return (
								d.innerText.indexOf('Custom Right-Click Menu') >
								-1
							);
						});
...
(() => {
	...
	describe('Extension prefix', () => {
		it('should be extractable from the options page', async function () {
			this.timeout(60000);
			this.slow(20000);
			prefix = await getExtensionData().getExtensionURLPrefix(
				driver,
				browserCapabilities
			);
		});
