...
async function findExtensionElement(
	...
	if (version < 36) {
		console.error(
			"Chrome extension testing before chrome 36 won't work," +
	...
	if (version < 66) {
		...
		for (const extension of extensions) {
			...
			if (
				(await title.getText()).indexOf('Custom Right-Click Menu') > -1
			) {
				return extension;
			}
		}
		...
		const extensions = await findElement(
			webdriver.By.tagName('extensions-manager')
		)
		...
		for (const extension of extensions) {
			...
			if (
				...
				await extension
					.findElement(webdriver.By.id('detailsButton'))
					.click();
				await wait(1000);
				return driver;
			}
...
export async function getExtensionURLPrefix(
	...
	if (version < 66) {
		...
		return `chrome-extension://${await driver.executeScript(
			inlineFn(() => {
