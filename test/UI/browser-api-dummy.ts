...
class StorageGenerator {
	...
	set(
		...
		callback?: (storage: StorageContainer) => void
	) {
...
testWindow.chrome = {
	...
	commands: {
		...
		getAll: function (
			callback: (commands: _chrome.commands.Command[]) => void
		) {
	...
	contextMenus: {
		create: function (
			...
			if (
				data.contexts &&
				data.contexts.filter(function (element) {
			...
			if (data.parentId) {
				findItemWithId(
					testWindow.chrome._currentContextMenu,
					data.parentId,
					function (parent) {
		...
		update: function (
			...
			if (
				...
				//None found
				testWindow.chrome.runtime.lastError = new Error(
					'No contextMenu with id ' + id + ' exists'
				);
			}
			callback && callback();
	...
	downloads: {
		download: function (
			settings: _chrome.downloads.DownloadOptions,
			callback?: () => void
		) {
			TypeChecking.typeCheck(
				{
	...
	runtime: {
		...
		getPlatformInfo: function (
			callback: (info: _chrome.runtime.PlatformInfo) => void
		) {
			checkOnlyCallback(callback, false);
			callback({} as any);
		},
		...
		sendMessage: function (
			...
			onMessageListener &&
				onMessageListener(message, options, responseCallback);
		},
	...
	tabs: {
		...
		getCurrent: function (
			callback: (currentTab: _chrome.tabs.Tab) => void
		) {
			checkOnlyCallback(callback, false);
			...
			TypeChecking.typeCheck(
				...
				[
					...
					},
					{
						...
						type: 'string',
						optional:
							scriptSettings && scriptSettings.file !== void 0,
					},
					{
						val: 'details.file',
						type: 'string',
						optional:
							scriptSettings && scriptSettings.code !== void 0,
					},
					{
		...
		create: function (
			data: _chrome.tabs.CreateProperties,
		...
		update: function (
			id: number | _chrome.tabs.UpdateProperties,
			data: _chrome.tabs.UpdateProperties | (() => void),
			callback?: () => void
		) {
			if (typeof id !== 'number') {
		...
		query: function (
			...
			callback(
				Object.getOwnPropertyNames(testWindow.chrome._fakeTabs)
					.map(function (fakeTabId) {
						return testWindow
							.chrome._fakeTabs[fakeTabId] as _chrome.tabs.Tab;
					})
					.filter(function (tab) {
	...
	permissions: {
		getAll: function (callback: (res: { permissions: string[] }) => void) {
			checkOnlyCallback(callback, false);
...
window.onload = function () {
	var dummyContainer = (testWindow.dummyContainer = document.createElement(
		'div'
	));
