...
const REGISTER_SUBTASKS =
	process.argv.indexOf("-T") === -1 && process.argv.indexOf("--tasks") === -1;

const BANNERS = {
	html:
		"<!--Original can be found at https://www.github.com/SanderRonde" +
 ...
 */
function genRootTask<T extends ReturnFunction>(
	name: string,
	description: string,
	toRun: T
): T {
	if (!toRun && typeof description !== "string") {
		console.log(
			`Missing root task name for task with description ${description}`
		);
		process.exit(1);
	}
...
function cache(
	name: string,
	fallback: () => NodeJS.ReadWriteStream
): NodeJS.ReadWriteStream {
	if (!cacheExists(name)) {
...
class Tasks {
		...
		class Convenience {
			static Clean = (() => {
				...
				class Clean {
					...
					static cleanDist = series(Clean.dist);

					@rootTask(
						"clean",
						"Cleans the building and caching directories"
					)
				...
				class PrepareForHotReload {
					...
					static copyMonacoBeautiful() {
						return gulp
							...
							.pipe(
								replace(
									/document\.caretRangeFromPoint/g,
									"MonacoEditorHookManager.caretRangeFromPoint(arguments[0])"
								)
							)
							.pipe(
								replace(
									/this.target(\s)?=(\s)?e.target/g,
					...
					static async lessEmbedDev() {
						const less = await readFile(
							"./resources/buildresources/less.min.js"
						);
					...
					static async stylusEmbedDev() {
						const stylus = await readFile(
							"./resources/buildresources/stylus.min.js"
						);
				...
				return new Promise((resolve, reject) => {
					which("yarn", (err, cmdPath) => {
						if (err) {
							...
							const cmd = childProcess.spawn(cmdPath, [
								"--force",
								"--ignore-engines",
							]);
							cmd.on("close", (code) => {
								if (code !== 0) {
									reject(
										`Yarn failed with exit code ${code}`
									);
								} else {
									resolve();
								}
	...
	static I18N = (() => {
		...
		class I18N {
				...
				return new Promise<[any, string][]>((resolve, reject) => {
					glob(
						...
						async (err, matches) => {
							...
							resolve(
								matches.map((file) => {
									return [require(file).Messages, file] as [
										any,
										string
									];
								})
			...
			static compileTS() {
				return new Promise(async (resolve, reject) => {
					const files = await new Promise<string[]>(
						(resolve, reject) => {
			...
			static Compile = (() => {
				...
				class Compile {
					private static _removeMetadata(message: I18NMessage) {
						...
						for (const placeholder in message.placeholders || {}) {
							cleanedMessage.placeholders[placeholder] = {
								content:
									message.placeholders[placeholder].content,
							};
						}
					...
					static async _compileI18NFile(file: string, data?: any) {
						...
						await writeFile(
							path.join(path.dirname(file), "messages.json"),
					...
					static async watcher() {
						...
						watcher.on("change", (fileName) => {
							Compile._activeTask = (async () => {
								const fileData = I18N._getFreshFileExport(
									fileName
								);
								await I18N.Compile._compileI18NFile(
									fileName,
									fileData
								);
							})();
					...
					@subTask(
						"watch",
						"Compiles, then watches for file changes and compiles on change"
					)
					static watch = series(Compile.compile, Compile.watcher);
				}
				return Compile;
			})();
			...
			static Defs = (() => {
				...
				@taskClass("defs")
				class Defs {
					private static async _typeMessages(
						root: I18NRoot,
						typed: NestedObject = {}
					) {
						I18N._walkMessages(
							root,
							(_message, currentPath, finalKey) => {
					...
					@describe(
						"Generates the lang spec from input files, making sure all " +
					...
					static async genSpec() {
						...
						await writeFile(
							path.join(__dirname, "app/_locales/i18n.d.ts"),
							specFile
						);
					}

					private static _getMatches(
						a: string[],
						b: string[]
					): number {
					...
					private static _getDiffPath(
						a: string[],
						b: string[]
					): Change[] {
						let matches = Defs._getMatches(a, b);

						if (a.length === b.length && matches === a.length)
							return [];
						return [
							...a
								.slice(matches)
					...
					static genEnumMessages(root: I18NRoot) {
						...
						return `export namespace I18NKeys {\n${str
							.map((i) => Defs._indent(1) + i)
							.join("\n")}\n}`;
					}

					@describe(
						"Generates enums that can be used to reference some " +
					...
					static async genEnums() {
						...
						if (files.length === 0) {
							console.log(
								"No source files to generate enums from"
							);
							return;
						}
						const enums = await Defs.genEnumMessages(files[0][0]);
						await writeFile(
							path.join(__dirname, "app/_locales/i18n-keys.ts"),
							enums
						);
					}
					...
					@rootTask(
						"i18nDefs",
						"Generates definitions files based on i18n files"
					)
					static defs = series(I18N.compileTS, Defs.genEnums);

					@rootTask("i18nSpec", "Generates language spec")
					static spec = series(Defs.genSpec);
					...
					@describe(
						"Watches for file changes and updates enums on change"
					)
					static async watcher() {
						const watcher = gulp.watch(
							"./app/_locales/*/messages.js",
							Defs.watchFileChange
						);
						watcher.on("change", (fileName) => {
							Defs._activeTask = (async () => {
								...
								const enums = await I18N.Defs.genEnumMessages(
									fileData
								);
								await writeFile(
									path.join(
										__dirname,
										"app/_locales/i18n-keys.ts"
									),
									enums
								);
							})();
					...
					static watch = series(Defs.genEnums, Defs.watcher);
				}
				return Defs;
			})();
			...
			@describe(
				"Turns I18N TS files into messages.json files whenever they change"
			)
			static watcher() {
				const watcher = gulp.watch(
					"./app/_locales/*/messages.js",
					I18N.watchFileChange
				);
				watcher.on("change", (fileName) => {
			...
			static watch = series(I18N.Compile.compile, I18N.watcher);

			@rootTask(
				"i18n",
				"Compiles I18N files and generates spec and enum files"
			)
			static i18n = series(I18N.Defs.genEnums, I18N.Compile.compile);
		}
		return I18N;
	})();
	...
	static Compilation = (() => {
		class Compilation {
			@rootTask("fileIdMaps", "Updates the HTML to Typescript maps")
			static async fileIdMaps() {
				const pattern =
					"{app/elements/**/*.html,!app/elements/elements.html}";
			...
			static Compile = (() => {
				...
				class Compile {
					...
					static compile = series(
						Compilation.defs,
						parallel(Compile.app, Compile.test)
					);
				}
				return Compile;
			})();
	...
	static DocumentationWebsite = (() => {
		function typedocCloned() {
			return new Promise((resolve) => {
				fs.stat(
					path.join(__dirname, "typedoc", "package.json"),
					(err) => {
						if (err) {
							//Doesn't exist yet
							resolve(false);
		...
		async function cloneTypedoc() {
			...
			console.log(
				"Installing typedoc dependencies (this may take a few minutes)"
			);
			...
			console.log(
				"Installing this extension's typescript version in cloned typedoc"
			);
		...
		class Building {
			static Build = (() => {
				...
				class Build {
					static PrePolymer = (() => {
						...
						class PrePolymer {
							@describe(
								"Copies monaco files from node_modules to app/ and temp/"
							)
							static copyMonacoJs() {
							...
							@describe(
								"Copies non-js monaco files from node_modules to app/ and temp/"
							)
							...
							@describe(
								"Replaces CRM-dev with CRM in the manifest"
							)
							static devManifest() {
								return gulp
									.src("manifest.json", {
							...
							@describe(
								"Copies files necessary for building from app/ to temp/"
							)
							static copyBuild() {
								return gulp
									.src(
							...
							static copyInstalling() {
								return gulp
									...
									.pipe(
										gulp.dest("./temp/elements/installing")
									);
							}

							@describe(
								"Inline external CSS imports into HTML files"
							)
							...
							@describe(
								'Change where "this" points to in webcomponents-lite'
							)
							...
							@describe(
								"Embeds the typescript compilation library"
							)
							static embedTS() {
							...
							static Entrypoint = (() => {
								...
								class Entrypoint {
									@describe(
										"Run processhtml on the entrypoint"
									)
									static process() {
										return gulp
											.src(
									...
									static async crisp() {
										const entrypointPrefix = await readFile(
							...
							@describe(
								"Run processhtml on the generated files in the temp/ dir"
							)
					...
					static PostPolymer = (() => {
						...
						class PostPolymer {
							@describe(
								"Moves all files up a directory, from build/temp to build/"
							)
							static moveUpDir() {
								return gulp
									.src("**/*", {
							...
							@describe(
								"Copy the webcomponent libraries from temp/ to build/"
							)
							static copyWebComponentLibs() {
								return gulp
									...
									.pipe(
										gulp.dest("./build/bower_components")
									);
							}

							@describe(
								"Copies entrypointPrefix from temp/ to build/"
							)
							static copyPrefixJs() {
								return gulp
								...
								class EntrypointJS {
									static Crisp = (() => {
										...
										class Crisp {
											...
											@describe(
												"Crisps the background.html page"
											)
									...
									static Babel = (() => {
										...
										class Babel {
											...
											@describe(
												"Joins the options page with the prefix"
											)
											...
											@describe(
												"Joins the logging page with the prefix"
											)
											...
											@describe(
												"Joins the install page with the prefix"
											)
											...
											@subTask(
												"Babelify all entrypoints and join them with the prefix"
											)
									...
									@describe(
										"Fix some bugs in the background and options pages"
									)
									...
									@describe(
										"Fix some bugs in the webcomponents file that is used in the options page"
									)
									...
									@describe(
										"Don't defer loads in entrypoint HTML files"
									)
									static nodefer() {
										return gulp
											.src([
									...
									static Banners = (() => {
										...
										class Banners {
											@describe(
												"Creates banners in the HTML files"
											)
											static html() {
												return gulp
													.src(["build/html/**.html"])
											...
											@describe(
												"Creates banners in the JS files"
											)
											static js() {
												return gulp
													.src(
											...
											@subTask(
												"Creates banners in all HTML and JS files"
											)
									...
									@subTask(
										"Crisps, babels and joins all entrypoints"
									)
							...
							static Monaco = (() => {
								...
								class Monaco {
									@describe(
										"Copies JS files from monaco to build/ and uglifies them"
									)
									static js() {
									...
									@describe(
										"Copies non-JS files from monaco to build/"
									)
									static nonjs() {
										return gulp
											.src(["**/**", "!**/**/*.js"], {
									...
									@subTask(
										"Operations related to copying monaco"
									)
							...
							@subTask(
								"postpolymer",
								"All post-polymer tasks during building"
							)
					...
					@describe(
						"Copies extra dependencies not already copied by polymer"
					)
					...
					static Prod = (() => {
						...
						class Prod {
							...
							static uglify() {
								return gulp
									.src(["./build/html/entrypointPrefix.js"])
									.pipe(uglify())
									.pipe(gulp.dest("./build/html/"));
							}
						}
			...
			static BuildZip = (() => {
				...
				class BuildZip {
					...
					static zip() {
						return gulp
							.src(["**", "!Custom Right-Click Menu.zip"], {
								cwd: "./build/",
								base: "./build/",
							})
	...
	static DemoWebsite = (() => {
		class DemoWebsite {
			...
			@rootTask("noJekyll", "Copies the nojekyll file to the root")
			...
			static changeGitIgnore() {
				return gulp
					.src("gh-pages-gitignore.gitignore", {
						cwd: "./tools",
						base: "./tools",
					})
	...
	static Definitions = (() => {
				...
				await doReplace(
					...
					async (match) => {
						...
						return await readFile(
							path.join("./tools/definitions", file)
						);
					}
				)
			).join("\n");
		...
		class Definitions {
			...
			@rootTask(
				"genDefs",
				"Move all crmapi .d.ts files into a single file"
			)
			static async genDefs() {
				await writeFile(
					"./dist/defs/crmapi.d.ts",
					await Definitions._joinDefs()
				);
			}
		}
		return Definitions;
		...
		class Browser {
			...
			static genCRX = (() => {
				...
				class GenCRX {
					...
					static genCRX = parallel(GenCRX.firefox, GenCRX.chrome);
				}
				return GenCRX;
			})();

			@describe(
				"Generates an xpi file and places it in the /dist/packed folder"
			)
			static async genXPI() {
				await xpi(
					"./dist/packed/Custom Right-Click Menu.firefox.xpi",
					"./dist/firefox"
				);
			}
		}
	...
	static Distribution = (() => {
		class Distribution {
			static Zip = (() => {
				...
				class Zip {
					@describe(
						"Zips the build/ directory and places it in " +
							"artifacts.build.zip"
					)
					static build() {
						return gulp
					...
					@describe(
						"Zips the dist/ directory and places it in " +
							"artifacts.dist.zip"
					)
					static dist() {
						return gulp
					...
					static zipArtifacts = parallel(Zip.build, Zip.dist);
				}
				return Zip;
			})();
			...
			static Unzip = (() => {
				...
				class Unzip {
					...
					static unzipArtifacts = parallel(Unzip.build, Unzip.dist);
				}
				return Unzip;
			})();
			...
			static genAppxCopy() {
				return gulp
					...
					.pipe(gulp.dest("dist/CRM/edgeextension/manifest/Assets"));
			}
			...
			static async genAppxReplace() {
				 ...
				 * @type {{"version": string}}
				 */
				const edgeManifest: { version: string } = JSON.parse(
					await readFile("dist/edge/manifest.json")
				);
				...
				const secrets: {
					...
					await readFile("resources/buildresources/edge_secrets.json")
				);

				packageManifest = packageManifest
					.replace(
						/INSERT-YOUR-PACKAGE-IDENTITY-NAME-HERE/g,
						secrets.Name
					)
					.replace(
						/CN=INSERT-YOUR-PACKAGE-IDENTITY-PUBLISHER-HERE/g,
						secrets.Publisher
					)
					.replace(
						/INSERT-YOUR-PACKAGE-PROPERTIES-PUBLISHERDISPLAYNAME-HERE/g,
			...
			@rootTask("genAppx:clean", "Cleans the left over appx files")
	...
	static Meta = (() => {
		...
		class Meta {
			...
			static async tasks() {
				...
				const longest = Array.from(descriptions.keys())
					.map((k) => k.length)
					.reduce((prev, current) => {
			...
			static default = series(Tasks.Building.Build.build);
		}
		return Meta;
	})();
}
...
namespace TaskGeneration {
	...
	function collapseTask(
		root: TaskStructure | Function
	): Undertaker.TaskFunction {
		if (typeof root === "function") return root as Undertaker.TaskFunction;
		const fns = root.children.map(
			(child: ReturnFunction | TaskStructure) => {
	...
	export function createBasicTasks(taskTree: any, baseName: string = "") {
		...
		for (const key in root) {
			...
			if (
				Reflect.getOwnMetadata(taskClassMetaKey, value) ||
				...
				// Sub-task
				const [taskName] = Reflect.getOwnMetadata(
					subtaskMetaKey,
					taskTree,
					key
				);
				...
				const reflected = Reflect.getOwnMetadata(
					rootTaskMetaKey,
					taskTree,
					key
				);
	...
	export function createRootTasks(taskTree: any) {
		...
		for (const key in root) {
			...
			if (
				...
				gulp.task(
					taskName,
					genRootTask(taskName, description, collapseTask(value))
				);
			}
		}
	}
