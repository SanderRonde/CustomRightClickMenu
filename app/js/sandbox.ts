...
(() => {
	...
	_self.warn = (...args: any[]) => {
		...
		log(
			['Warning: ', ...args],
			errSplit.slice(1, errSplit.length).join('at')
		);
	};
	...
	function verifyKey(key: string): boolean {
		return (
			key ===
			handshakeData.secretKey.join('') + handshakeData.id + 'verified'
		);
	}
			...
			case 'init':
				...
				((script, log) => {
					try {
						...
						log(
							'Script boot failed, call window.debugBackgroundScript(node id) to' +
