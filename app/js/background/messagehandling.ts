...
export namespace MessageHandling {
	...
	export interface CRMAPICallMessage
		extends CRMAPIMessageInstance<'crmapi', CRMFunctionDataBase> {
		action: keyof typeof CRMAPIFunctions;
	}

	export interface BackgroundAPIMessage
		extends CRMAPIMessageInstance<
			'background',
			{
				[key: string]: any;
				[key: number]: any;
	...
	async function handleRuntimeMessage(
		message: CRMAPIMessageInstance<string, any>,
		...
		switch (message.type) {
			...
			case 'createLocalLogVariable':
				modules.Logging.LogExecution.executeCRMCode(
					message.data,
					message.type as
						| 'executeCRMCode'
						| 'getCRMHints'
						| 'createLocalLogVariable'
				);
				break;
			case 'displayHints':
				modules.Logging.LogExecution.displayHints(
					message as CRMAPIMessageInstance<
						'displayHints',
						{
							hints: string[];
							id: CRM.GenericNodeId;
			...
			case 'newTabCreated':
				if (messageSender && respond) {
					...
					let isReload = modules.crmValues.tabData.has(
						messageSender.tab.id
					);
					...
					response = await modules.CRMNodes.Running.executeScriptsForTab(
						messageSender.tab.id,
						isReload
					);
				}
				break;
			case 'styleInstall':
				await modules.CRMNodes.Stylesheet.Installing.installStylesheet(
					message.data
				);
				break;
			case 'updateStylesheet':
				await modules.CRMNodes.Stylesheet.Updating.updateStylesheet(
					message.data.id
				);
				break;
			case 'updateScripts':
	...
	export function handleRuntimeMessageInitial(
		message: CRMAPIMessageInstance<string, any>,
		messageSender?: _browser.runtime.MessageSender,
		respond?: (message: any) => void,
	...
	export async function handleCrmAPIMessage(
		message:
			| CRMAPICallMessage
