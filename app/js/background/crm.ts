...
export namespace CRMNodes.Script.Handler {
	async function genCodeOnPage({
		...
		const enableBackwardsCompatibility =
			(await modules.Util.getScriptNodeScript(node)).indexOf(
				'/*execute locally*/'
			) > -1 && node.isLocal;
		...
		const doDebug =
			modules.globalObject.globals.eventListeners.scriptDebugListeners.indexOf(
				node.id
			) > -1;
		if (doDebug) {
			modules.globalObject.globals.eventListeners.scriptDebugListeners.splice(
				modules.globalObject.globals.eventListeners.scriptDebugListeners.indexOf(
					node.id
				),
				1
		...
		modules.Util.setMapDefault(
			modules.storages.nodeStorageSync,
			node.id,
			{}
		);
		return [
			...
			modules.constants.templates.globalObjectWrapperCode(
				'window',
				'windowWrapper',
				node.isLocal && BrowserAPI.isBrowserAPISupported('chrome')
					? 'chrome'
					: 'void 0',
				node.isLocal && BrowserAPI.isBrowserAPISupported('browser')
					? 'browser'
					: 'void 0'
			),
			`${catchErrs ? 'try {' : ''}`,
			...
			script,
			'}',
			`crmAPI.onReady(function() {main.apply(this, [crmAPI, windowWrapper, ${
				node.isLocal && BrowserAPI.isBrowserAPISupported('chrome')
					? 'chrome'
					: 'void 0'
			}, ${
		...
		for (let i = 0; i < node.value.libraries.length; i++) {
			...
			if (globalLibraries) {
				for (let j = 0; j < globalLibraries.length; j++) {
					if (
						globalLibraries[j].name === node.value.libraries[i].name
					) {
						const currentLib = globalLibraries[j];
						if (currentLib.ts && currentLib.ts.enabled) {
							lib = {
								code: await modules.Util.getLibraryCode(
									currentLib
								),
							};
						} else {
			...
			if (!lib) {
				...
				if (!node.value.libraries[i].name) {
					...
					if (urlDataPairs.get(node.value.libraries[i].url as any)) {
						lib = {
							code: urlDataPairs.get(
								node.value.libraries[i].url as any
							).dataString,
						};
					}
				}
	...
	function getResourcesArrayForScript(
	...
	function ensureRunAt(
		...
		if (
			...
			window.logAsync(
				window.__(I18NKeys.background.crm.invalidRunat, id + '', runAt)
			);
		}
	...
	function executeScripts(
		nodeId: CRM.NodeId<CRM.ScriptNode>,
		tabId: TabId,
		scripts: {
			code?: string;
			file?: string;
	...
	export async function generateGreaseMonkeyData(
		metaData: {
			[key: string]: any;
		},
		node: CRM.ScriptNode,
		...
		const metaString = (
			MetaTags.getMetaLines(node.value.script) || []
		).join('\n');
		...
		return {
			info: {
				script: {
					...
					includes: (metaData['includes'] || []).concat(
						metaData['include']
					),
					...
					options: {
						...
						override: {
							...
							orig_includes: (metaData['includes'] || []).concat(
								metaData['include']
							),
	...
	export function genTabData(
		...
		modules.Util.setMapDefault(
			modules.crmValues.tabData.get(tabId).nodes,
			nodeId,
			[]
		);
		modules.crmValues.tabData
			.get(tabId)
			.nodes.get(nodeId)
			.push({
				secretKey: key,
			...
			if (err) {
				browserAPI.tabs
					.executeScript(tab.id, {
						code:
							'alert("Something went wrong very badly, please go to your Custom Right-Click Menu' +
				...
				const [
					contextData,
					{ greaseMonkeyData, runAt },
					script,
					tabIndex,
				] = (await window.Promise.all<any>([
					...
					modules.Util.iipe<{
						...
						let runAt: _browser.extensionTypes.RunAt =
							metaData['run-at'] ||
							metaData['run_at'] ||
							'document_end';
						...
						return {
							greaseMonkeyData: await generateGreaseMonkeyData(
								metaData,
								node,
								includes,
								excludes,
					...
					modules.Util.iipe<string>(async () => {
						return (await modules.Util.getScriptNodeScript(node))
							.split('\n')
							.map((line) => {
				...
				const usesUnsafeWindow =
					(await modules.Util.getScriptNodeScript(node)).indexOf(
						'unsafeWindow'
					) > -1;
				const scripts = await getScriptsToRun(
					code,
					runAt,
					node,
					usesUnsafeWindow
...
export namespace CRMNodes.Script.Background {
	async function loadBackgroundPageLibs(
		node: CRM.ScriptNode
	): Promise<{
		libraries: string[];
		code: string[];
	...
	async function genCodeBackground(
		code: string[],
		{
			key,
			node,
		...
		return [
			...
			[
				`var crmAPI = new (window._crmAPIRegistry.pop())(${[
					...
					greaseMonkeyData,
					true,
					fixOptionsErrors((node.value && node.value.options) || {}),
					enableBackwardsCompatibility,
					0,
					browserAPI.runtime.id,
					supportedBrowserAPIs.join(','),
	...
	export async function createBackgroundPage(
		...
		if (modules.background.byId.has(node.id)) {
			...
			await modules.Logging.backgroundPageLog(
				node.id,
				null,
				await window.__(
					I18NKeys.background.crm.restartingBackgroundPage
				)
			);
			modules.background.byId.get(node.id).terminate();
			modules.Logging.backgroundPageLog(
				node.id,
				null,
				await window.__(
					I18NKeys.background.crm.terminatedBackgroundPage
				)
			);
		}
		...
		if (
			modules.crmValues.tabData.has(0) &&
		...
		if (err) {
			window.logAsync(
				window.__(I18NKeys.background.crm.setupError, node.id),
				err
			);
			throw err;
		}
		...
		const indentUnit = '	';
		const [
			{ code: backgroundPageCode, libraries },
			script,
			greaseMonkeyData,
		] = (await window.Promise.all<any>([
			modules.Util.iipe<{
			...
			modules.Util.iipe<any>(async () => {
				const metaData = MetaTags.getMetaTags(
					await modules.Util.getScriptNodeScript(node)
				);
				const { excludes, includes } = Handler.getInExcludes(node);
				return await Handler.generateGreaseMonkeyData(
					metaData,
					node,
					includes,
					excludes,
		...
		const code = await genCodeBackground(
			backgroundPageCode,
			{
				key,
				node,
			...
			async (_nodeId, node) => {
				if (
					...
					if (isValidBackgroundPage(node)) {
						window.info(
							await window.__(
								I18NKeys.background.crm.createdBackgroundPage,
								node.id
							)
						);
					}
...
export namespace CRMNodes.MetaTags {
	export function getMetaIndexes(
		code: string
	): {
		start: number;
		end: number;
	...
	export function getlastMetaTagValue(
		metaTags: {
			[key: string]: any;
		},
		key: string
...
export namespace CRMNodes.Script.Updating {
	...
	function createUserscriptTriggers(metaTags: {
		...
		const includes: string[] = (metaTags['includes'] || []).concat(
			metaTags['include']
		);
		if (includes) {
		...
		triggers = triggers.filter(
			(trigger, index) => triggers.indexOf(trigger) === index
		);
		return {
			triggers,
	...
	async function createUserscriptScriptData(
		metaTags: {
			[key: string]: any;
		},
		code: string,
		node: Partial<CRM.Node>
	) {
		node.type = 'script';
		...
		const newLibs: CRM.InstalledLibrary[] = [
			...
			...(
				await Promise.all(
					libs.map(({ name, url }) => {
						return new Promise<CRM.InstalledLibrary>(
							async (resolve) => {
	...
	function createUserscriptStylesheetData(
		metaTags: {
			[key: string]: any;
		},
		code: string,
		...
		node.value = {
			...
			defaultOn: metaTags['CRM_defaultOn'] =
				...
				false,
			toggle: metaTags['CRM_toggle'] =
				MetaTags.getlastMetaTagValue(metaTags, 'CRM_toggle') || false,
			launchMode: CRMLaunchModes.ALWAYS_RUN,
			options: {},
			convertedStylesheet: null,
		};
	}
	async function createUserscriptTypeData(
		metaTags: {
			[key: string]: any;
		},
		code: string,
		...
		if (MetaTags.getlastMetaTagValue(metaTags, 'CRM_stylesheet')) {
	...
	export async function install(message: {
		...
		const oldTree = JSON.parse(
			JSON.stringify(modules.storages.settingsStorage.crm)
		);
		const { path, oldNodeId, node } = await Updating.installUserscript(
			message.metaTags,
			message.script,
			message.downloadURL,
	...
	export async function installUserscript(
		...
		const updateUrl =
			MetaTags.getlastMetaTagValue(metaTags, 'updateURL') ||
		...
		node.nodeInfo = {
			...
			source: {
				...
				url:
					updateUrl ||
					MetaTags.getlastMetaTagValue(metaTags, 'namespace') ||
					downloadURL,
				author:
					MetaTags.getlastMetaTagValue(metaTags, 'author') ||
					'Anonymous',
				autoUpdate: true,
			},
			isRoot: true,
		...
		if (MetaTags.getlastMetaTagValue(metaTags, 'CRM_contentTypes')) {
			try {
				node.onContentTypes = JSON.parse(
					MetaTags.getlastMetaTagValue(metaTags, 'CRM_contentTypes')
				);
			} catch (e) {}
		}
		...
		const { requestPermissions = [] } = await browserAPI.storage.local.get<
			CRM.StorageLocal
		>();
		const allPermissions = browserAPI.permissions
			? await browserAPI.permissions.getAll()
			: {
					permissions: [],
			  };
		const allowed = allPermissions.permissions || [];
		const joinedPermissions = [...requestPermissions, ...node.permissions]
			.filter((permission: _browser.permissions.Permission) => {
	...
	export async function updateScripts() {
		...
		await Promise.all(
			modules.Util.mapToArr(modules.crm.crmById).map(
				async ([_id, node]) => {
					if (node.type !== 'script') {
						return;
					}
	...
	async function onNodeUpdateDone(
		updated: {
			node: CRM.ScriptNode;
		}[],
		oldTree: CRM.Tree
		...
		const updatedData = updated.map(({ node: { id, name, nodeInfo } }) => {
			...
			return {
				...
				id: id,
				oldVersion:
					(oldNode && oldNode.nodeInfo && oldNode.nodeInfo.version) ||
					'',
		...
		const { updatedNodes = [] } = await browserAPI.storage.local.get<
			CRM.StorageLocal
		>();
					...
					modules.Util.convertFileToDataURI(
						...
						async (_dataURI, dataString) => {
							...
							try {
								...
								if (
									modules.Util.isNewer(
										metaTags['version'][0],
										node.nodeInfo.version
									)
								) {
									if (
										!modules.Util.compareArray(
										...
										addedPermissions.push({
											node: node.id,
											permissions: metaTags[
												'grant'
											].filter(
												(
													newPermission: CRM.Permission
												) => {
													return (
														node.nodeInfo.permissions.indexOf(
															newPermission
														) === -1
													);
												}
						...
						() => {
							window.logAsync(
								window.__(
									I18NKeys.background.crm.updateDownload404,
									'script',
									node.id,
									node.name
								)
...
export namespace CRMNodes.Running {
	...
	export async function executeScriptsForTab(
		tabId: TabId,
		isReload: boolean
	) {
		try {
			...
			if (tab.url && modules.Util.canRunOnUrl(tab.url)) {
				...
				if (!urlIsGlobalExcluded(tab.url)) {
					...
					const toExecute = toExecuteNodes.onUrl.documentEnd.filter(
						({ triggers }) => {
					...
					for (const {
						id,
					} of toExecuteNodes.always.documentEnd.concat(toExecute)) {
...
export namespace CRMNodes.Stylesheet.Updating {
	...
	export async function updateStylesheet(
		nodeId: CRM.NodeId<CRM.StylesheetNode>
	) {
		const node = modules.crm.crmById.get(nodeId) as CRM.StylesheetNode;
		const url = getDownloadURL(node);
		const updateData = (
			await CRMNodes.Stylesheet.Installing.getUserstyleMeta(url)
		).sections[
			node.nodeInfo.source !== 'local' &&
				node.nodeInfo.source.sectionIndex
		];
					...
					try {
						...
						for (let i = 0; i < parsed.sections.length; i++) {
							...
							if (
								...
								node.triggers = JSON.parse(
									JSON.stringify(launchData.triggers)
								);
								wasUpdated = true;
							}
	...
	async function onNodeUpdateDone(
		...
		const updatedData = updated.map(({ node: { id, name, nodeInfo } }) => {
			...
			return {
				...
				id: id,
				oldVersion:
					modules.Util.accessPath(oldNode, 'nodeInfo', 'version') ||
					undefined,
				newVersion: nodeInfo.version,
			};
		});
...
export namespace CRMNodes.Stylesheet.Options {
	...
	function preprocessUSO(
		...
		while ((match = _variableRegex.exec(stylesheet))) {
			...
			if (!(name in options)) {
				window.logAsync(
					window.__(I18NKeys.background.crm.optionNotFound, name, id)
				);
	...
	function parseVar(value: string) {
		...
		lastLabelChar = type.length + 1 + name.length + 1 + label.length + 2;
	...
	function metaTagVarsToCodeOptions(
		stylesheet: string,
		...
		if (vars.length === 0) {
			...
			vars.forEach((value: string) => {
				...
				const descriptor = {
					...((typeof options !== 'string' && options[name]) || {}),
					...({
				...
				switch (type) {
					...
					case 'checkbox':
						option =
							...
							(options[name] as
								| CRM.OptionString
								| CRM.OptionColorPicker
								| CRM.OptionCheckbox);
						if (option && option.value === null) {
							(descriptor as
								| CRM.OptionString
								| CRM.OptionColorPicker
								| CRM.OptionCheckbox).value = defaultValue;
						}
						break;
					case 'select':
						try {
	...
	function getPreprocessorFormat(
		...
		if (preprocessor === 'less' || preprocessor === 'stylus') {
			return modules.Util.toArray<CRM.OptionsValue>(options || {}).map(
				([key, value]) => {
					switch (value.type) {
						case 'string':
						case 'color':
			...
			modules.Util.toArray<CRM.OptionsValue>(options || {}).map(
				([key, value]) => {
					switch (value.type) {
						case 'string':
						case 'color':
		...
		return new Promise<string>((resolve) => {
			window.less.Parser().parse(stylesheet, (err, result) => {
				if (!err) {
					...
					window.logAsync(
						`${window.__(
							I18NKeys.background.crm.cssCompileError,
							'less',
							id
						)}:`,
						err.name,
	...
	async function convertStylesheet(
		id: CRM.NodeId<CRM.StylesheetNode>,
		stylesheet: string,
		options: CRM.Options
	): Promise<string> {
		...
		const preprocessor =
			(meta['preprocessor'] && meta['preprocessor'][0]) ||
			(vars.length ? 'uso' : 'default');
		return await applyPreprocessor(
			compileVars(stylesheet, options, preprocessor),
			id,
			preprocessor,
			options
		);
	}
	export async function getConvertedStylesheet(
		node: CRM.StylesheetNode
	): Promise<string> {
		if (
			node.value.convertedStylesheet &&
			node.value.convertedStylesheet.options ===
				JSON.stringify(node.value.options)
		) {
			return node.value.convertedStylesheet.stylesheet;
		}
		node.value.convertedStylesheet = {
			options: JSON.stringify(node.value.options),
			stylesheet: await convertStylesheet(
				node.id,
				node.value.stylesheet,
				typeof node.value.options === 'string' ? {} : node.value.options
			),
		};
		return node.value.convertedStylesheet.stylesheet;
	}
...
export namespace CRMNodes.Stylesheet.Installing {
	function triggerify(url: string): string {
		const match = /((http|https|file|ftp):\/\/)?(www\.)?((\w+)\.)*((\w+)?|(\w+)?(\/(.*)))?/g.exec(
			url
		);
	...
	function canBeUpdated(
		...
		if (data.version && node.nodeInfo.version) {
			return (
				modules.Storages.getVersionDiff(
					node.nodeInfo.version,
					data.version
				) === 1
			);
	...
	export async function getInstalledStatus(url: string) {
		...
		await modules.Util.crmForEachAsync(
			modules.crm.crmTree,
			async (node) => {
				if (node.type !== 'stylesheet') {
					return;
	...
	function findDocDeclarations(
		...
		for (let i = 0; i < code.length; i++) {
			const char = code[i];
			if (
				stringStartsFromHere(code, i, '@-moz-document') ||
	...
	export function getUserstyleMeta(
		...
		if (parsable) {
			...
			return {
				...
				updateUrl:
					MetaTags.getMetaTag(metaTags, 'updateURL') ||
					MetaTags.getMetaTag(metaTags, 'homepageURL') ||
					undefined,
	...
	export async function installStylesheet(data: {
		...
		const ids = await modules.Util.getMultipleItemIds(
			stylesheetData.sections.length
		);
		await modules.Util.promiseChain(
			stylesheetData.sections.map((section, index) => {
				return async () => {
					const sectionData = extractStylesheetData(section);
					const node = modules.constants.templates.getDefaultStylesheetNode(
						{
							isLocal: false,
							...
							nodeInfo: {
								...
								source: {
									...
									author:
										stylesheetData.author || data.author,
									sectionIndex: index,
...
export namespace CRMNodes.Stylesheet {
	export function createToggleHandler(
		...
		return async (
			...
			if (
				!modules.crmValues.nodeTabStatuses.get(node.id).tabs.has(tab.id)
			) {
			...
			modules.crmValues.nodeTabStatuses
				.get(node.id)
				.tabs.get(tab.id).checked = info.checked;
	...
	export function createClickHandler(node: CRM.StylesheetNode): ClickHandler {
		return async (
			_info: _browser.contextMenus.OnClickData,
			tab: _browser.tabs.Tab
		) => {
			const className = node.id + '' + tab.id;
...
export namespace CRMNodes.NodeCreation {
	function getStylesheetReplacementTabs(
		node: CRM.Node
	): {
		id: TabId;
	}[] {
		...
		const crmNode: CRM.Node = modules.crm.crmById.get(
			node.id as CRM.GenericNodeId
		);
		if (
			modules.crmValues.contextMenuIds.get(node.id) && //Node already exists
	...
	function pushToGlobalToExecute(node: CRM.Node, launchMode: CRMLaunchModes) {
		...
		if (
			...
			if (
				...
				modules.toExecuteNodes.always.documentEnd.push(
					extractToExecuteNode(node)
				);
			} else if (
				launchMode === CRMLaunchModes.RUN_ON_SPECIFIED ||
				launchMode === CRMLaunchModes.SHOW_ON_SPECIFIED
			) {
				modules.toExecuteNodes.onUrl.documentEnd.push(
					extractToExecuteNode(node)
				);
			}
		}
	}
	function handleHideOnPages(
		node: CRM.Node,
		launchMode: CRMLaunchModes,
		...
		if (
			...
			for (let i = 0; i < node.triggers.length; i++) {
				const prepared = modules.URLParsing.prepareTrigger(
					node.triggers[i].url
				);
				if (prepared) {
					if (node.triggers[i].not) {
						modules.crmValues.hideNodesOnPagesData
							.get(node.id)
							.push({
								not: false,
								url: prepared,
						...
						rightClickItemOptions.documentUrlPatterns.push(
							prepared
						);
					}
				}
	...
	function generateClickHandler(
		...
		switch (node.type) {
			...
			case 'link':
				...
				break;
			case 'script':
				rightClickItemOptions.onclick = Script.Handler.createHandler(
					node
				);
				break;
			case 'stylesheet':
				if (node.value.toggle) {
					...
					rightClickItemOptions.onclick = Stylesheet.createToggleHandler(
						node
					);
					rightClickItemOptions.checked = node.value.defaultOn;
				} else {
					rightClickItemOptions.onclick = Stylesheet.createClickHandler(
						node
					);
				}
	...
	async function handleContextMenuError(
		options: ContextMenuCreateProperties,
		e: _chrome.runtime.LastError | string,
		idHolder: {
			id: number | string;
		}
	) {
		if (options.documentUrlPatterns) {
			window.logAsync(
				window.__(I18NKeys.background.crm.contextmenuErrorRetry),
				e
			);
			...
			idHolder.id = await browserAPI.contextMenus.create(
				...
				async () => {
					...
					window.logAsync(
						window.__(I18NKeys.background.crm.contextmenuError),
						e
					);
				}
			);
	...
	async function generateContextMenuItem(
		rightClickItemOptions: ContextMenuCreateProperties,
		idHolder: {
			id: number | string;
		}
	...
	function getContextmenuGlobalOverrides(
		node: CRM.Node
	): ContextMenuOverrides {
		return modules.crmValues.contextMenuGlobalOverrides.get(node.id);
	}
	async function addRightClickItemClick(
		node: CRM.Node,
		launchMode: CRMLaunchModes,
		rightClickItemOptions: ContextMenuCreateProperties,
		idHolder: {
		...
		modules.Util.applyContextmenuOverride(
			rightClickItemOptions,
			getContextmenuGlobalOverrides(node)
		);
	...
	async function hasDocumentStartMetaTag(node: CRM.Node) {
		if (node.type === 'script') {
			const meta = MetaTags.getMetaTags(
				await modules.Util.getScriptNodeScript(node)
			);
	...
	async function setupUserInteraction(
		node: CRM.Node,
		rightClickItemOptions: ContextMenuCreateProperties,
		idHolder: {
			id: number | string;
		...
		const launchMode =
			...
			CRMLaunchModes.RUN_ON_CLICKING;
		...
		if (
			...
			pushToToExecute(
				...
				launchMode === CRMLaunchModes.ALWAYS_RUN,
				node.type === 'stylesheet' ||
					(await hasDocumentStartMetaTag(node))
			);
		} else if (launchMode !== CRMLaunchModes.DISABLED) {
			await addRightClickItemClick(
				node,
				launchMode,
				rightClickItemOptions,
				idHolder
	...
	export async function createNode(
		...
		if (replaceStylesheetTabs.length !== 0) {
			...
			for (let i = 0; i < replaceStylesheetTabs.length; i++) {
				...
				let code = `var nodes = document.querySelectorAll(".styleNodes${className}");var i;for (i = 0; i < nodes.length; i++) {nodes[i].remove();}`;
				const css = node.value.stylesheet.replace(/[ |\n]/g, '');
				code += `var CRMSSInsert=document.createElement("style");CRMSSInsert.className="styleNodes${className}";CRMSSInsert.type="text/css";CRMSSInsert.appendChild(document.createTextNode(${JSON.stringify(
					css
				)}));document.head.appendChild(CRMSSInsert);`;
				...
				statusses
					.get(node.id)
					.tabs.get(replaceStylesheetTabs[i].id).checked = true;
			}
		}
...
export namespace CRMNodes.TS {
	...
	export async function compileNode(
		node: CRM.ScriptNode | CRM.SafeScriptNode
	) {
		if (node.value.ts && node.value.ts.enabled) {
			node.value.ts.script = await compileChangedScript(
				node.value.script,
				node.value.ts.script
			);
			node.value.ts.backgroundScript = await compileChangedScript(
				modules.Util.getScriptNodeJS(node, 'background'),
				node.value.ts.backgroundScript
			);
		}
	}
	export async function compileLibrary(
		library: CRM.InstalledLibrary
	): Promise<CRM.InstalledLibrary> {
		if (library.ts && library.ts.enabled) {
			library.ts.code = await compileChangedScript(
				library.code,
				library.ts.code
			);
		}
	...
	export async function compileAllLibraries(
	...
	async function compileChangedScript(
		script: string,
		compilationData: CRM.TypescriptCompilationData
	): Promise<CRM.TypescriptCompilationData> {
...
export namespace CRMNodes {
	...
	export async function updateCrm(toUpdate?: CRM.GenericNodeId[]) {
		...
		toUpdate &&
			(await modules.Storages.checkBackgroundPagesForChange({
				toUpdate,
			}));
	}
	async function _assertItemIds() {
		...
		return (
			crmBefore !== JSON.stringify(modules.storages.settingsStorage.crm)
		);
	}
	...
	export async function updateCRMValues() {
		...
		changed =
			_assertCRMShape(modules.storages.settingsStorage.crm) || changed;

		modules.crm.crmTree = modules.storages.settingsStorage.crm;
		modules.crm.safeTree = buildSafeTree(
			modules.storages.settingsStorage.crm
		);
	...
	export function handleUserAddedContextMenuErrors() {
		...
		if (__window.chrome && __window.chrome.runtime) {
			...
			if (__chrome && __chrome.runtime && __chrome.runtime.lastError) {
				window.logAsync(
					window.__(I18NKeys.background.crm.userContextmenuError),
					__chrome.runtime.lastError
				);
			}
		} else {
	...
	export function buildPageCRM() {
		return new Promise<void>((resolve) => {
			...
			browserAPI.contextMenus.removeAll().then(async () => {
				...
				const done = await modules.Util.lock(
					modules.Util.LOCK.ROOT_CONTEXTMENU_NODE
				);
				modules.crmValues.rootId = await browserAPI.contextMenus.create(
					{
						title:
							modules.storages.settingsStorage.rootName ||
							'Custom Menu',
				...
				modules.Util.promiseChain(
					modules.crm.crmTree.map((node, index) => {
						return () => {
							return new Promise<void>((resolveInner) => {
								buildPageCRMTree(
									node,
									modules.crmValues.rootId,
									[index],
									modules.crmValues.contextMenuItemTree
								).then((result) => {
									if (result) {
										modules.crmValues.contextMenuItemTree[
											index
	...
	export function getContexts(
		contexts: CRM.ContentTypes
	): _browser.contextMenus.ContextType[] {
		const newContexts: _browser.contextMenus.ContextType[] = [
			'browser_action',
		];
	...
	export function getJoinedContexts(
		contexts: CRM.ContentTypes[]
	): _browser.contextMenus.ContextType[] {
		const newContexts: {
			[key: string]: boolean;
		...
		return Object.getOwnPropertyNames(
			newContexts
		) as _browser.contextMenus.ContextType[];
	}
	...
	export function fixOptionsErrors(
		options: CRM.Options | string
	): CRM.Options | string;
	export function fixOptionsErrors(
		options: CRM.Options | string
		...
		switch (node.type) {
			...
			case 'stylesheet':
				return [
					node.name,
					'Script',
					[node.value.launchMode, node.value.stylesheet].join('%124'),
				].join('%123');
		}
	}
	function walkCRM(
			...
			props.forEach((prop) => {
				if (prop in obj) {
					if (typeof obj[prop as keyof CRM.Node] === 'object') {
						(target as any)[
							prop as keyof CRM.SafeNode
						] = JSON.parse(
							JSON.stringify(obj[prop as keyof CRM.Node])
						);
					} else {
	...
	async function buildPageCRMTree(
		node: CRM.Node,
		parentId: string | number,
		path: number[],
		...
		if (id !== null) {
			...
			if (node.children) {
				...
				for (let i = 0; i < node.children.length; i++) {
					...
					const result: any = await buildPageCRMTree(
						node.children[i],
						id,
						newPath,
						children
