...
export namespace Init {
	...
	async function genStorageModules(): Promise<StorageModules> {
		const isDev =
			...
			-1;
		const globalObject: GlobalObject = isNode() || isDev ? window : {};
	...
	async function initRoutine() {
		...
		window.console.group(
			await window.__(I18NKeys.background.init.initialization)
		);
		...
		await Util.iipe(async () => {
			...
			try {
				...
				window.info(
					await window.__(I18NKeys.background.init.registeringHandler)
				);
				...
				window.info(
					await window.__(I18NKeys.background.init.buildingCrm)
				);
				await CRMNodes.buildPageCRM();
				window.info(
					await window.__(I18NKeys.background.init.compilingTs)
				);
				...
				window.info(
					await window.__(
						I18NKeys.background.init.registeringHandlers
					)
				);
				...
				window.console.group(
					await window.__(I18NKeys.background.init.resources)
				);
				window.info(
					await window.__(I18NKeys.background.init.updatingResources)
				);
				Resources.updateResourceValues();
				window.info(
					await window.__(I18NKeys.background.init.updatingNodes)
				);
				...
				window.info(
					await window.__(I18NKeys.background.init.debugInfo)
				);
				window.info(
					await window.__(I18NKeys.background.init.invalidatedTabs),
				...
				if (!isNode()) {
					...
					window.logAsync(
						window.__(I18NKeys.background.init.debugExplanation)
					);
				}
			} catch (e) {
				for (let i = 0; i < 10; i++) {
