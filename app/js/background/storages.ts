...
export namespace Storages.SetupHandling.TransferFromOld.LegacyScriptReplace.LocalStorageReplace {
	...
	function findLocalStorageExpression(
		...
		switch (expression.type) {
			...
			case 'VariableDeclaration':
				for (let i = 0; i < expression.declarations.length; i++) {
					...
					if (declaration.init) {
						if (
							findLocalStorageExpression(declaration.init, data)
						) {
							return true;
						}
					}
			...
			case 'MemberExpression':
				...
				return findLocalStorageExpression(
...
export namespace Storages.SetupHandling.TransferFromOld.LegacyScriptReplace.ChromeCallsReplace {
	...
	function getFunctionCallExpressions(
	...
	function getLineIndexFromTotalIndex(
		lines: string[],
		line: number,
		index: number
	): number {
	...
	function replaceChromeFunction(
		data: ChromePersistentData,
		expr: Tern.Expression,
		callLine: {
			from: {
		...
		var lineExprStart = getLineIndexFromTotalIndex(
			data.persistent.lines,
			callLine.from.line,
			(data.returnExpr && data.returnExpr.start) || expr.callee.start
		);
		var lineExprEnd = getLineIndexFromTotalIndex(
			data.persistent.lines,
			callLine.from.line,
			expr.callee.end
		);
		...
		if (data.isReturn) {
			...
			for (
				...
				if (
					data.parentExpressions[i].type === 'BlockStatement' ||
					(data.parentExpressions[i].type === 'FunctionExpression' &&
						(data.parentExpressions[i].body as Tern.BlockStatement)
							.type === 'BlockStatement')
				) {
					scopeLength = getLineIndexFromTotalIndex(
						data.persistent.lines,
						callLine.from.line,
	...
	function callsChromeFunction(
		callee: Tern.FunctionCallExpression,
		data: ChromePersistentData,
		onError: TransferOnErrorHandler
	): boolean {
		...
		if (callee.object && callee.object.name) {
			//First object
			const isWindowCall =
				isProperty(callee.object.name, 'window') &&
				isProperty(
					callee.property.name || (callee.property as any).raw,
					'chrome'
				);
			if (isWindowCall || isProperty(callee.object.name, 'chrome')) {
				...
				const callLines = getCallLines(
					data.persistent.lineSeperators,
					expr.start,
					expr.end
				);
				if (data.isReturn && !data.isValidReturn) {
					callLines.from.index = getLineIndexFromTotalIndex(
						data.persistent.lines,
						callLines.from.line,
						callLines.from.index
					);
					callLines.to.index = getLineIndexFromTotalIndex(
						data.persistent.lines,
						callLines.to.line,
						callLines.to.index
					);
					...
					for (let i = 0; i < expression.arguments.length; i++) {
						if (
							expression.arguments[i].type !==
								'MemberExpression' &&
							expression.arguments[i].type !== 'CallExpression'
						) {
	...
	function removePositionDuplicates(
	...
	export function replace(
		...
		if (finalPassErrors) {
			onError(
				removePositionDuplicates(firstPassErrors),
...
export namespace Storages.SetupHandling.TransferFromOld.LegacyScriptReplace {
	...
	export function generateScriptUpgradeErrorHandler(
		id: number
	): UpgradeErrorHandler {
		return async (oldScriptErrors, newScriptErrors, parseError) => {
			const { upgradeErrors = {} } = await browserAPI.storage.local.get<
				CRM.StorageLocal
			>();
			upgradeErrors[id] = modules.storages.storageLocal.upgradeErrors[
				id
			] = {
	...
	export function convertScriptFromLegacy(
		script: string,
		...
		try {
			switch (method) {
				case SCRIPT_CONVERSION_TYPE.CHROME:
					script = ChromeCallsReplace.replace(
						script,
...
export namespace Storages.SetupHandling.TransferFromOld {
	...
	export async function transferCRMFromOld(
		...
		for (let i = 1; i < amount; i++) {
			nodes.push(
				await parseOldCRMNode(
					storageSource.getItem(String(i)),
					openInNewTab,
					method
				)
			);
	...
	async function parseOldCRMNode(
		string: string,
		openInNewTab: boolean,
		method: SCRIPT_CONVERSION_TYPE
	): Promise<CRM.Node> {
		...
		switch (type.toLowerCase()) {
			...
			case 'link':
				...
				node = modules.constants.templates.getDefaultLinkNode({
					name: name,
					id: (await modules.Util.generateItemId()) as CRM.NodeId<
						CRM.LinkNode
					>,
			...
			case 'script':
				...
				node = modules.constants.templates.getDefaultScriptNode({
					...
					value: {
						...
						script: Storages.SetupHandling.TransferFromOld.LegacyScriptReplace.convertScriptFromLegacy(
							scriptData,
							id,
							method
						),
...
export namespace Storages.SetupHandling {
	//Local storage
	async function getDefaultStorages(): Promise<
		[CRM.StorageLocal, CRM.SettingsStorage]
	> {
	...
	export async function handleFirstRun(
		...
		const prevSync = await (async () => {
			...
			const { data, syncEnabled } = parseCutData<CRM.SettingsStorage>(
				sync as any
			);
			if (!syncEnabled) {
				return {};
		...
		storageLocal.settingsVersionData.current.hash = storageLocal.settingsVersionData.latest.hash = window.md5(
			JSON.stringify(syncStorage)
		);
		storageLocal.settingsVersionData.current.date = storageLocal.settingsVersionData.latest.date =
	...
	export async function handleTransfer(): Promise<{
		...
		return await handleFirstRun(
			await TransferFromOld.transferCRMFromOld(whatPage)
		);
	}
			...
			promiseInitializers[index]().then(
				(value) => {
					if (index + 1 >= promiseInitializers.length) {
						...
						chainPromise(promiseInitializers, index + 1).then(
							(value) => {
								resolve(value);
							},
							(err) => {
		...
		if (settingsJson.length >= 101400 || !supportsStorageSync()) {
			...
			await browserAPI.storage.sync
				...
				.catch(async (err) => {
					...
					window.logAsync(
						window.__(I18NKeys.background.storages.syncUploadError),
						err
					);
...
export namespace Storages {
	...
	type StorageLocalChange<
		K extends keyof CRM.StorageLocal = keyof CRM.StorageLocal
	> = {
		key: K;
	...
	type StorageSyncChange<
		K extends Extract<keyof CRM.SettingsStorage, string> = Extract<
			keyof CRM.SettingsStorage,
			string
		>
	> = {
		key: K;
	...
	export async function checkBackgroundPagesForChange({
		change,
		toUpdate,
	}: {
		change?: {
		...
		const { same, additions, removals } = diffCRM(
			change.oldValue,
			change.newValue
		);
		await window.Promise.all([
			...same.map(async ({ id }) => {
				...
				const currentNode = modules.crm.crmById.get(
					id
				) as CRM.ScriptNode;
				...
				if (
					...
					const [
						...
						modules.Util.getScriptNodeScript(
							currentNode,
							'background'
						),
					...
					if (
						oldBgScript !== newBgScript ||
						currentBgScript !== currentBgScript
					) {
						await modules.CRMNodes.Script.Background.createBackgroundPage(
							newNode
						);
	...
	async function uploadSync(changes: StorageChange[]) {
		...
		await browserAPI.storage.local.set({
			settingsVersionData: {
				...
				latest:
					modules.storages.storageLocal.settingsVersionData.latest,
				wasUpdated:
					modules.storages.storageLocal.settingsVersionData
						.wasUpdated,
			},
		});
		if (
			!modules.storages.storageLocal.useStorageSync ||
			!supportsStorageSync()
		) {
			if (cachedWriteTimer) {
			...
			await browserAPI.storage.local
				...
				.catch((e) => {
					window.logAsync(
						window.__(
							I18NKeys.background.storages.localUploadError
						),
						e
					);

					if (
						e.message.indexOf('MAX_WRITE_OPERATIONS_PER_MINUTE') >
							-1 ||
						e.message.indexOf('MAX_WRITE_OPERATIONS_PER_HOUR') > -1
					) {
						cachedWrite = JSON.stringify(
							modules.storages.settingsStorage
						);
						if (cachedWriteTimer) {
							window.clearTimeout(cachedWriteTimer);
						}
						cachedWriteTimer = window.setTimeout(
							async () => {
								await browserAPI.storage.local.set({
							...
							e.message.indexOf('MAX_WRITE_OPERATIONS_PER_HOUR') >
								-1
								? 1000 * 60 * 60
								: 1000 * 60 * 60
						);
			...
			if (settingsJson.length >= 101400 || !supportsStorageSync()) {
				...
				await browserAPI.storage.sync
					...
					.catch(async (err) => {
						window.logAsync(
							window.__(
								I18NKeys.background.storages.syncUploadError
							),
							err
						);
						modules.storages.storageLocal.useStorageSync = false;
	...
	export async function uploadChanges(
		...
		switch (type) {
			case 'local':
				await browserAPI.storage.local.set(
					modules.storages.storageLocal
				);
				for (let i = 0; i < changes.length; i++) {
					if (changes[i].key === 'useStorageSync') {
						const change = changes[i] as StorageLocalChange<
							'useStorageSync'
						>;
	...
	export async function applyChanges(data: {
		...
		switch (data.type) {
			case 'optionsPage':
				...
				break;
			case 'libraries':
				const compiled = await modules.CRMNodes.TS.compileAllLibraries(
					data.libraries
				);
			...
			case 'nodeStorage':
				...
				if (data.isSync) {
					applyChangeForStorageType(
						modules.storages.nodeStorageSync.get(data.id),
						data.nodeStorageChanges,
						true
					);
					modules.storages.settingsStorage.nodeStorageSync = modules.Util.fromMap(
						modules.storages.nodeStorageSync
					);
				} else {
					applyChangeForStorageType(
						modules.storages.nodeStorage.get(data.id),
						data.nodeStorageChanges,
						true
					);
					modules.storages.storageLocal.nodeStorage = modules.Util.fromMap(
						modules.storages.nodeStorage
					);
				}
				notifyNodeStorageChanges(
					data.id,
					data.tabId,
					data.nodeStorageChanges,
					data.isSync
	...
	export async function setStorages(
		...
		window.info(
			await window.__(I18NKeys.background.storages.settingGlobalData)
		);
		modules.storages.storageLocal = storageLocalCopy;
		modules.storages.settingsStorage = settingsStorage;

		modules.storages.globalExcludes = setIfNotSet(
			chromeStorageLocal,
			'globalExcludes',
			[] as string[]
		)
			.map(modules.URLParsing.validatePatternUrl)
			.filter((pattern) => {
				return pattern !== null;
			}) as MatchPattern[];
		const toMap = modules.Util.toMap;
		modules.storages.resources = toMap(
			setIfNotSet(chromeStorageLocal, 'resources', {})
		);
		modules.storages.nodeStorage = toMap(
			setIfNotSet(
				chromeStorageLocal,
				'nodeStorage',
				{} as {
		...
		modules.storages.nodeStorageSync = toMap(
			setIfNotSet(
				settingsStorage,
				'nodeStorageSync',
				{} as {
		...
		modules.storages.resourceKeys = setIfNotSet(
			chromeStorageLocal,
			'resourceKeys',
			[]
		) as {
		...
		modules.storages.urlDataPairs = toMap(
			setIfNotSet(
				chromeStorageLocal,
				'urlDataPairs',
				{} as {
	...
	export function parseCutData<T>(
		data: {
			[key: string]: string;
		} & {
	...
	export function loadStorages() {
		return new Promise<void>(async (resolve) => {
			window.info(
				await window.__(I18NKeys.background.storages.loadingSync)
			);
			const storageSync: {
				[key: string]: string;
			} & {
			...
			if (result.type === 'firstTimeCallback') {
				...
				window.info(
					await window.__(I18NKeys.background.storages.parsingData)
				);
				const storageLocalCopy = JSON.parse(
				...
				if (storageLocal['useStorageSync']) {
					...
					if (!storageLocal['settings']) {
						...
						settingsStorage = parseCutData(storageSync)
							.data as CRM.SettingsStorage;
					} else {
				...
				window.info(
					await window.__(
						I18NKeys.background.storages.checkingUpdates
					)
				);
	...
	async function changeCRMValuesIfSettingsChanged(changes: StorageChange[]) {
		for (let i = 0; i < changes.length; i++) {
			if (changes[i].key === 'crm' || changes[i].key === 'showOptions') {
				...
				const rootNameChange = changes[i] as StorageSyncChange<
					'rootName'
				>;
	...
	function setIfNotSet<U, K extends keyof U>(
		obj: U,
		key: K,
		defaultValue: U[K]
	): U[K] {
	...
	function applyChangeForStorageType(
		storageObj: {
			[key: string]: any;
			[key: number]: any;
		},
		changes: StorageChange[],
		usesDots: boolean = false
	) {
		for (let i = 0; i < changes.length; i++) {
			if (usesDots) {
	...
	function notifyNodeStorageChanges(
		id: CRM.GenericNodeId,
		tabId: TabId,
		changes: StorageChange[],
		isSync: boolean
	...
	function isVersionInRange(
		...
		return (
			getVersionDiff(min, target) === 1 &&
			getVersionDiff(target, max) === 1
		);
	}
		...
		if (isVersionInRange(oldVersion, newVersion, '2.0.4')) {
			fns.afterSync.push(async () => {
				await modules.Util.crmForEachAsync(
					...
					async (node) => {
						if (node.type === 'script') {
							node.value.oldScript = await modules.Util.getScriptNodeScript(
								node
							);
							const legacyScriptReplace =
								SetupHandling.TransferFromOld
									.LegacyScriptReplace;
							node.value.script = legacyScriptReplace.ChromeCallsReplace.replace(
								await modules.Util.getScriptNodeScript(node),
								legacyScriptReplace.generateScriptUpgradeErrorHandler(
									node.id
								)
							);
						}
						if (node.isLocal) {
							...
							if (
								node.onContentTypes[0] &&
								node.onContentTypes[1] &&
								node.onContentTypes[2] &&
								!node.onContentTypes[3] &&
								!node.onContentTypes[4] &&
								!node.onContentTypes[5]
							) {
								node.onContentTypes = [
									true,
									true,
		...
		if (isVersionInRange(oldVersion, newVersion, '2.1.0')) {
			fns.beforeSyncLoad.push((local) => {
				...
				if (typeof local.selectedCrmType === 'number') {
					local.selectedCrmType = crmTypeNumberToArr(
						local.selectedCrmType
					);
					browserAPI.storage.local.set({
				...
				if (
					local.editing &&
					typeof local.editing.crmType === 'number'
				) {
					local.editing.crmType = crmTypeNumberToArr(
						local.editing.crmType
					);
					browserAPI.storage.local.set({
...
window.open(url.replace(/%s/g,query), \'_blank\');
				...
				modules.Util.crmForEach(modules.crm.crmTree, (node) => {
					if (node.type === 'script') {
						...
						if (
							script.length !== searchEngineScript.length ||
	...
	> {
		...
		if (
			localStorage.getItem('transferToVersion2') &&
			...
			if (
				localStorage.getItem('transferToVersion2') &&
				storageLocal.lastUpdatedAt
			) {
				window.logAsync(
					window.__(
						I18NKeys.background.storages.upgrading,
						storageLocal.lastUpdatedAt,
						currentVersion
					)
				);
				const fns = await upgradeVersion(
					storageLocal.lastUpdatedAt,
					currentVersion
				);
			...
			if (
				!window.localStorage.getItem('transferToVersion2') &&
				...
				window.info(
					await window.__(
						I18NKeys.background.storages.initializingFirst
					)
				);
				return {
					type: 'firstTimeCallback',
	...
	function checkForStorageSyncUpdates(
		storageSync: CRM.SettingsStorage,
		storageLocal: CRM.StorageLocal
	) {
		...
		if (
			...
			storageLocal.settingsVersionData.current.hash !== hash
		) {
