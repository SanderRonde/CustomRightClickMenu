...
export namespace CRMAPIFunctions {
	...
	export function initModule(
		__this: CRMAPICall.Instance,
		_modules?: ModuleData
	) {
		modules = _modules;
	}

	export function _doesLibraryExist(lib: { name: string }): string | boolean {
	...
	export function _isAlreadyUsed(
		script: CRM.ScriptNode,
		lib: {
			name: string;
		}
	): boolean {
...
export namespace CRMAPIFunctions.contextMenuItem {
	function applyContextmenuChange(
		nodeId: CRM.GenericNodeId,
		tabId: TabId,
		override: ContextMenuOverrides,
		update: ContextMenuUpdateProperties,
		allTabs: boolean
	) {
		if (!allTabs) {
			if (!modules.crmValues.nodeTabStatuses.has(nodeId)) {
				...
				!modules.crmValues.nodeTabStatuses.get(nodeId).tabs.has(tabId)
			) {
				modules.crmValues.nodeTabStatuses.get(nodeId).tabs.set(tabId, {
					overrides: {},
				});
			} else if (
				!modules.crmValues.nodeTabStatuses.get(nodeId).tabs.get(tabId)
					.overrides
			) {
				modules.crmValues.nodeTabStatuses
					.get(nodeId)
					.tabs.get(tabId).overrides = {};
			}
		} else {
			modules.Util.setMapDefault(
				modules.crmValues.contextMenuGlobalOverrides,
				nodeId,
				{}
			);
		}
	...
	export function setType(__this: CRMAPICall.Instance) {
		__this.typeCheck(
			...
			() => {
				const { itemType, allTabs = false } = __this.message
					.data as MessageHandling.CRMFunctionDataBase & {
				...
				if (validTypes.indexOf(itemType) === -1) {
					__this.respondError(
						'Item type is not one of "normal", "checkbox", "radio" or"separator"'
					);
					return;
				}

				applyContextmenuChange(
					__this.message.id,
					__this.message.tabId,
					{
						type: itemType,
					},
					{
	...
	export function setChecked(__this: CRMAPICall.Instance) {
		__this.typeCheck(
			...
			() => {
				const { checked, allTabs = false } = __this.message
					.data as MessageHandling.CRMFunctionDataBase & {
					checked: boolean;
					allTabs?: boolean;
				};

				const globalOverride = modules.crmValues.contextMenuGlobalOverrides.get(
					__this.message.id
				);
				const tabOverride =
					(modules.crmValues.nodeTabStatuses.get(__this.message.id) &&
						modules.crmValues.nodeTabStatuses
							.get(__this.message.id)
							.tabs.get(__this.message.tabId) &&
						modules.crmValues.nodeTabStatuses
							.get(__this.message.id)
							.tabs.get(__this.message.tabId).overrides) ||
					{};
				const joinedOverrides = { ...globalOverride, ...tabOverride };

				const shouldConvertToCheckbox =
					joinedOverrides.type !== 'checkbox' &&
	...
	export function setContentTypes(__this: CRMAPICall.Instance) {
		__this.typeCheck(
			...
			() => {
				const { contentTypes, allTabs = false } = __this.message
					.data as MessageHandling.CRMFunctionDataBase & {
				...
				for (const contentType of contentTypes) {
					if (validContentTypes.indexOf(contentType) === -1) {
						__this.respondError(
							'Not all content types are one of "page", "link", ' +
	...
	export function setVisibility(__this: CRMAPICall.Instance) {
		__this.typeCheck(
			...
			() => {
				__this
					.getNodeFromId(__this.message.id)
					.run((node: CRM.ScriptNode) => {
						const { isVisible, allTabs = false } = __this.message
							.data as MessageHandling.CRMFunctionDataBase & {
							isVisible: boolean;
							allTabs?: boolean;
						};
	...
	export function setDisabled(__this: CRMAPICall.Instance) {
		__this.typeCheck(
			...
			() => {
				const { isDisabled, allTabs = false } = __this.message
					.data as MessageHandling.CRMFunctionDataBase & {
					isDisabled: boolean;
					allTabs?: boolean;
				};
	...
	export function setName(__this: CRMAPICall.Instance) {
		__this.checkPermissions(['crmContextmenu'], () => {
			__this.typeCheck(
				...
				() => {
					const { name, allTabs = false } = __this.message
						.data as MessageHandling.CRMFunctionDataBase & {
						name: string;
						allTabs?: boolean;
					};
	...
	export function resetName(__this: CRMAPICall.Instance) {
		__this.typeCheck(
			...
			() => {
				__this
					.getNodeFromId(__this.message.data.nodeId)
					.run((node: CRM.ScriptNode) => {
						const { allTabs = false } = __this.message
							.data as MessageHandling.CRMFunctionDataBase & {
							allTabs?: boolean;
						};
						applyContextmenuChange(
...
export namespace CRMAPIFunctions.crm {
	...
	export function getSubTree(__this: CRMAPICall.Instance) {
		__this.checkPermissions(['crmGet'], () => {
			...
			if (typeof nodeId === 'number') {
				const node = modules.crm.crmByIdSafe.get(
					nodeId as CRM.GenericNodeId
				);
				if (node) {
	...
	export function getNodeIdFromPath(__this: CRMAPICall.Instance) {
		__this.checkPermissions(['crmGet'], () => {
			...
			const lookedUp = __this.lookup(
				pathToSearch,
				modules.crm.safeTree,
				false
			);
			if (lookedUp === true) {
				return false;
				...
				(optionals) => {
					...
					if (optionals['query.inSubTree']) {
						...
						if (searchScopeObj) {
							...
							searchScopeObjChildren =
								menuSearchScopeObj.children;
						}
		...
		__this.checkPermissions(['crmGet'], () => {
			__this.getNodeFromId(__this.message.data.nodeId).run((node) => {
				...
				if (pathToSearch.length === 0) {
					...
					const lookedUp = __this.lookup<CRM.SafeNode>(
						pathToSearch,
						modules.crm.safeTree,
						false
					);
	...
	export function getNodeType(__this: CRMAPICall.Instance) {
		__this.checkPermissions(['crmGet'], () => {
			__this
				.getNodeFromId(__this.message.data.nodeId, true)
				.run((node) => {
		...
		__this.checkPermissions(['crmGet', 'crmWrite'], () => {
			__this.typeCheck(
				...
				async () => {
					...
					const sourceNode = __this.getNodeFromId(
						__this.message.id,
						false,
						true
					);
					...
					const baseNode = {
						...modules.CRMNodes.makeSafe(
							__this.message.data.options
						),
						...{
							id,
							isLocal,
					...
					switch (__this.message.data.options.type) {
						case 'script':
							...
							break;
						case 'stylesheet':
							newNode = templates.getDefaultStylesheetNode(
								baseNode
							);
					...
					if (
						(newNode = (await __this.moveNode(
							newNode,
							__this.message.data.options.position
						)) as CRM.Node)
					) {
						await modules.CRMNodes.updateCrm([newNode.id]);
						__this.respondSuccess(
							__this.getNodeFromId(newNode.id, true, true)
						);
					} else {
				...
				(optionals) => {
					__this
						...
						.run(async (copiedNode: CRM.Node) => {
							...
							const executingNode = __this.getNodeFromId(
								__this.message.id,
								false,
								true,
								true
							...
							if (
								executingNode.isLocal === true &&
								copiedNode.isLocal === true
							) {
								newNode.isLocal = true;
								modules.Util.crmForEach(
									newNode.children || [],
									(node) => {
										node.isLocal = true;
									}
							...
							const moved = await __this.moveNode(
								newNode,
								__this.message.data.options.position
							);
							if (moved) {
		...
		__this.checkPermissions(['crmGet', 'crmWrite'], () => {
			__this
				...
				.run(async (node) => {
					...
					const parentChildren = __this.lookup(
						node.path,
						modules.crm.crmTree,
						true
					);
					if (
						typeof parentChildren === 'boolean' ||
						!parentChildren
					) {
						__this.respondError(
							'Something went wrong removing the source node'
						);
						return;
					}

					if (
						(node = (await __this.moveNode(
							node,
							__this.message.data.position as {
						...
						modules.CRMNodes.updateCrm().then(() => {
							__this.respondSuccess(
								__this.getNodeFromId(node.id, true, true)
							);
						});
					}
				});
			...
			__this
				...
				.run(async (node) => {
					...
					if (
						...
						await browserAPI.contextMenus.remove(
							modules.crmValues.contextMenuIds.get(node.id)
						);
						await modules.CRMNodes.updateCrm([
							__this.message.data.nodeId,
						]);
					...
					__this
						...
						.run((node) => {
							...
							if (optionals['options.type']) {
								if (
									__this.message.data.options.type !==
										'link' &&
									__this.message.data.options.type !==
										'script' &&
									__this.message.data.options.type !==
									...
									if (oldType === 'menu') {
										...
										node.value =
											(node as any)[
												msg.options.type + 'Val'
											] || {};
									} else {
										(node as any)[oldType + 'Val'] =
											node.value;
										node.value =
											(node as any)[
												msg.options.type + 'Val'
				...
				() => {
					__this
						...
						.run(async (node) => {
							...
							modules.crmValues.hideNodesOnPagesData.set(
								node.id,
								[]
							);
							if (
								...
								for (const trigger of triggers) {
									const pattern = modules.URLParsing.validatePatternUrl(
										trigger.url
									);
									if (!pattern) {
										__this.respondSuccess(
								...
								for (let { url, not } of triggers) {
									if (
										!modules.URLParsing.triggerMatchesScheme(
											url
										)
									) {
										__this.respondError(
									...
									url = modules.URLParsing.prepareTrigger(
										url
									);
							...
							if (modules.crmValues.contextMenuIds.has(node.id)) {
								await browserAPI.contextMenus.update(
									modules.crmValues.contextMenuIds.get(
										node.id
									),
									{
					...
					__this
						...
						.run(async (node) => {
							if (
								...
								await modules.CRMNodes.updateCrm();
								if (
									modules.crmValues.contextMenuIds.has(
										node.id
									)
								) {
				...
				() => {
					...
					if (typeof msg.index === 'string') {
						if (options.indexOf(msg.index) === -1) {
							__this.respondError(
								'Index is not in index array ([page, link, selection, image, video, audio])'
							);
							return;
						} else {
					...
					__this
						.getNodeFromId(__this.message.data.nodeId)
						.run(async (node) => {
							node.onContentTypes[msg.index as number] =
								msg.value;
							await modules.CRMNodes.updateCrm();
							await browserAPI.contextMenus.update(
								modules.crmValues.contextMenuIds.get(node.id),
								{
									contexts: modules.CRMNodes.getContexts(
										node.onContentTypes
									),
								}
							);
			...
			__this.typeCheck(
				...
				() => {
					__this
						.getNodeFromId(__this.message.data.nodeId)
						.run(async (node) => {
							const { stylesheet } = __this.message
								.data as MessageHandling.CRMFunctionDataBase & {
								stylesheet: string;
							};
					...
					__this
						...
						.run((node) => {
							...
							modules.CRMNodes.updateCrm().then(() => {
								if (node.type === 'link') {
									__this.respondSuccess(
										modules.Util.safe(node).value
									);
								} else {
									__this.respondSuccess(
										modules.Util.safe(node).linkVal
									);
								}
							});
							return true;
		...
		__this.checkPermissions(['crmGet'], () => {
			__this
				...
				.run(async (node) => {
					if (node.type === 'script') {
						__this.respondSuccess(
							await modules.Util.getScriptNodeScript(
								node,
								'background'
							)
						);
						...
						.run(async (node) => {
							...
							if (Array.isArray(libraries)) {
								for (const library of libraries) {
									...
									if (
										!(library.name = _doesLibraryExist(
											library
										) as string)
									) {
										__this.respondError(
									...
									if (!_isAlreadyUsed(node, library)) {
										node.value.libraries.push(
											library as CRM.Library
										);
									}
								}
							} else {
								...
								if (
									!(libraries.name = _doesLibraryExist(
										libraries
									) as string)
								) {
									__this.respondError(
								...
								if (!_isAlreadyUsed(node, libraries)) {
									node.value.libraries.push(
										libraries as CRM.Library
									);
								}
							}
							await modules.CRMNodes.updateCrm();
							__this.respondSuccess(
								modules.Util.safe(node).value.libraries
							);
							return true;
						});
				}
			...
			__this.typeCheck(
				...
				() => {
					__this
						.getNodeFromId(__this.message.data.nodeId)
						.run(async (node) => {
							const { start, amount } = __this.message
								.data as MessageHandling.CRMFunctionDataBase & {
								start: number;
								amount: number;
							};
						...
						.run(async (node) => {
							...
							if (Array.isArray(libraries)) {
								...
								for (const library of libraries) {
									...
									if (!_isAlreadyUsed(node, library)) {
										node.value.backgroundLibraries.push(
											library as CRM.Library
										);
									}
								}
			...
			__this.typeCheck(
				...
				() => {
					...
					__this
						...
						.run(async (node) => {
							if (node.type === 'script') {
								const backgroundLibs =
									node.value.backgroundLibraries;
								const spliced = backgroundLibs.splice(
									start,
									amount
								);
								await modules.CRMNodes.updateCrm([
			...
			__this.typeCheck(
				...
				() => {
					__this
						...
						.run(async (node) => {
							//Force path recalculation
							modules.CRMNodes.buildNodePaths(
								modules.crm.crmTree
							);

							const { childrenIds } = __this.message
								.data as MessageHandling.CRMFunctionDataBase & {
							...
							for (const childId of childrenIds) {
								const toMove = __this.getNodeFromId(
									childId,
									false,
									true
								);
					...
					__this
						...
						.run(async (node) => {
							...
							for (const childId of childrenIds) {
								if (typeof childId !== 'number') {
									__this.respondError(
										'Not all values in array childrenIds are of type number'
									);
									return false;
								}
							}
				...
				() => {
					...
					__this
						...
						.run(async (node) => {
							...
							const { children } = __this.getNodeFromId(
								node.id,
								true,
								true
							) as CRM.MenuNode;
...
export namespace CRMAPIFunctions.crm.background {
	...
	async function doRunScript(
		...
		const tabIdTabs = await window.Promise.all(
			(tabIds || []).map((tabId) => {
	...
	export function runScript(__this: CRMAPICall.Instance) {
		__this.checkPermissions(['crmRun'], () => {
			__this.typeCheck(
				...
				async () => {
					const { options, id } = __this.message
						.data as MessageHandling.CRMFunctionDataBase & {
						id: CRM.GenericNodeId;
						options: BrowserTabsQueryInfo & {
							tabId?: MaybeArray<TabId>;
	...
	export function addKeyboardListener(__this: CRMAPICall.Instance) {
		__this.typeCheck(
			...
			() => {
				...
				const shortcuts =
					modules.globalObject.globals.eventListeners
						.shortcutListeners;
	...
	export function register(__this: CRMAPICall.Instance) {
		__this.checkPermissions(['crmWrite'], () => {
			__this.typeCheck(
				...
				async (optionals) => {
					const { name, url, ts, code } = __this.message
						.data as MessageHandling.CRMFunctionDataBase & {
						url?: string;
						name: string;
						code?: string;
					...
					if (optionals['url']) {
						if (url.indexOf('.js') === url.length - 3) {
							...
							if (res === null) {
								...
								__this.respondError(
									`Request failed with status code ${res}`
								);
							} else {
								newLibrary = {
									name,
								...
								const compiled = await modules.CRMNodes.TS.compileLibrary(
									newLibrary
								);
								modules.storages.storageLocal.libraries.push(
									compiled
								);
								await browserAPI.storage.local.set({
									libraries:
										modules.storages.storageLocal.libraries,
								});
