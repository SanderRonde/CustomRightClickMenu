...
export namespace CRMAPICall {
	...
	export class Instance {
		constructor(
			public message: MessageHandling.CRMAPICallMessage,
		...
		lookup<T>(path: number[], data: CRMParent<T>[]): T | boolean;
		lookup<T>(
			path: number[],
			data: CRMParent<T>[],
			hold: true
		...
		checkType(
			...
			if (toCheck === undefined || toCheck === null) {
				if (optional) {
					...
					if (isArray) {
						this.respondError(
							`Not all values for ${name} are defined`
						);
					} else {
				...
				if (type === 'array') {
					if (typeof toCheck !== 'object' || Array.isArray(toCheck)) {
						if (isArray) {
							this.respondError(
								`Not all values for ${name} are of type ${type},` +
									` they are instead of type ${typeof toCheck}`
							);
						} else {
							this.respondError(
								`Value for ${name} is not of type ${type},` +
		...
		private static readonly MoveNode = class MoveNode {
			...
			private static _genMoveTargetReturn(
				index: number,
				placedAfter: boolean
			): MoveTarget {
				if (index !== -1) {
			...
			static before(
				...
				if (isRoot) {
					...
					modules.Util.pushIntoArray(
						node,
						relativeNode.path[relativeNode.path.length - 1],
						parentChildren
					);
					return this._genMoveTargetReturn(
						targetIndex,
						relativeNode.path[relativeNode.path.length - 1] >
							targetIndex
					);
		...
		async moveNode(
			...
			if (
				!this.checkType(
					...
					false,
					() => {
						relativeNode = crmFunction.getNodeFromId(
							position.node,
							false,
							true
						);
			...
			switch (position.relation) {
				case 'before':
					target = Instance.MoveNode.before(
						isRoot,
						node,
						relativeNode,
						this
					...
					break;
				case 'firstSibling':
					target = Instance.MoveNode.firstSibling(
						isRoot,
						node,
						relativeNode,
						this
					...
					break;
				case 'after':
					target = Instance.MoveNode.after(
						isRoot,
						node,
						relativeNode,
						this
					...
					break;
				case 'lastSibling':
					target = Instance.MoveNode.lastSibling(
						isRoot,
						node,
						relativeNode,
						this
			...
			if (removeOld && target !== MoveTarget.NOT_PLACED) {
				...
				for (let i = 0; i < removeOld.children.length; i++) {
					if (removeOld.children[i].id === removeOld.id) {
						if (
							target === MoveTarget.NOT_SAME_TREE ||
				...
				settingsChanges: [
					{
						...
						oldValue: oldCrmTree,
						newValue: JSON.parse(
							JSON.stringify(modules.crm.crmTree)
						),
					},
				],
			});
		...
		getNodeFromId(
			id: CRM.GenericNodeId | CRM.GenericSafeNodeId,
			makeSafe: true,
			synchronous: true,
			_forceValid: true
		): CRM.SafeNode;
		...
		getNodeFromId(
			...
			makeSafe: true,
			synchronous: false
		): GetNodeFromIdCallback<CRM.SafeNode>;
		getNodeFromId(
			id: CRM.GenericNodeId | CRM.GenericSafeNodeId,
			makeSafe: false,
			synchronous: true,
		...
		getNodeFromId(
			...
			| GetNodeFromIdCallback<CRM.Node>
			| GetNodeFromIdCallback<CRM.SafeNode>
			| CRM.Node
			| CRM.SafeNode
			| false;
		getNodeFromId(
			...
			synchronous: true,
			_forceValid: true
		): CRM.Node | CRM.SafeNode;
		getNodeFromId(
			id: CRM.GenericNodeId | CRM.GenericSafeNodeId,
			makeSafe: boolean,
			synchronous: true,
			_forceValid: boolean
		): CRM.Node | CRM.SafeNode | false;
		getNodeFromId(
			id: CRM.GenericNodeId | CRM.GenericSafeNodeId,
			makeSafe: boolean,
			synchronous: true
		...
		getNodeFromId(
			...
			if (node) {
				...
				this.respondError(
					`There is no node with the id you supplied (${id})`
				);
				if (synchronous) {
					return false;
				}
		...
		private _typesMatch(data: TypeCheckConfig, value: any): string {
			...
			this.respondError(
				`Value for ${data.val} is not of type ${types.join(' or ')}`
			);
			return null;
		}

		private _checkNumberConstraints(
			data: TypeCheckConfig,
			value: number
		): boolean {
			if (data.min !== undefined) {
				if (data.min > value) {
					this.respondError(
						`Value for ${data.val} is smaller than ${data.min}`
					);
					return false;
				}
			}
			if (data.max !== undefined) {
				if (data.max < value) {
					this.respondError(
						`Value for ${data.val} is bigger than ${data.max}`
					);
					return false;
				}
			}
		...
		private _checkArrayChildType(
			...
			this.respondError(
				`For not all values in the array ${data.val} is the property ${
					forChild.val
				} of type ${types.join(' or ')}`
			);
			return false;
		}

		private _checkArrayChildrenConstraints<
			T extends {
				[key: string]: any;
			}
				...
				for (const forChild of data.forChildren) {
					...
					if (childValue === undefined || childValue === null) {
						if (!forChild.optional) {
							this.respondError(
								`For not all values in the array ${data.val} is the property ${forChild.val} defined`
							);
							return false;
						}
					} else if (
						!this._checkArrayChildType(data, childValue, forChild)
					) {
						return false;
					}
				}
		...
		typeCheck(
			...
			callback: (optionals?: { [key: string]: any }) => void
		) {
			const optionals: {
				[key: string]: any;
				[key: number]: any;
			...
			for (const data of toCheck) {
				...
				const value = Instance._getDotValue(
					this.message.data,
					data.val
				);
				//Check if it's defined
		...
		checkPermissions(
			...
			if (!(node = modules.crm.crmById.get(this.message.id))) {
				this.respondError(
					'The node you are running this script from no longer exist, no CRM API calls are allowed'
				);
				return false;
			}
			...
			if (node.isLocal) {
				...
				if (
					!node.permissions ||
					modules.Util.compareArray(node.permissions, [])
				) {
					if (toCheck.length > 0) {
						permitted = false;
				...
				if (!permitted) {
					if (
						modules.storages.insufficientPermissions.length > 1000
					) {
					...
					modules.storages.insufficientPermissions.push(
						`Script id ${
							this.message.id
						} asked for and was rejected permission${
