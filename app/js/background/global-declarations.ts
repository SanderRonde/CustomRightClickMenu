...
export namespace GlobalDeclarations {
	...
	export function initGlobalFunctions() {
		...
		window.debugNextScriptCall = (id: CRM.NodeId<CRM.ScriptNode>) => {
			if ((id !== 0 && !id) || typeof id !== 'number') {
				throw new Error(
					`Please supply a valid node ID, ${findNodeMsg}`
				);
			}
			const node = modules.crm.crmByIdSafe.get(id);
			if (!node) {
				throw new Error(
					`There is no node with the node ID you supplied, ${findNodeMsg}`
				);
			}
			if (node.type !== 'script') {
			...
			if (
				...
				modules.globalObject.globals.eventListeners.scriptDebugListeners.push(
					id
				);
			}
		};
		...
		window.debugBackgroundScript = (id: CRM.NodeId<CRM.ScriptNode>) => {
			...
			if (node.value.backgroundScript === '') {
				throw new Error(
					'Backgroundscript is empty (code is empty string)'
				);
			}
			modules.CRMNodes.Script.Background.createBackgroundPage(
				modules.crm.crmById.get(id),
				true
			);
		};
		...
		window.getID = (searchedName: string) => {
			...
			if (matches.length === 0) {
				window.logAsync(
					window.__(
						I18NKeys.background.globalDeclarations.getID.noMatches
					)
				);
			} else if (matches.length === 1) {
				window.logAsync(
					window.__(
						I18NKeys.background.globalDeclarations.getID.oneMatch,
						matches[0].id
					),
					matches[0].node
				);
				...
				window.logAsync(
					window.__(
						I18NKeys.background.globalDeclarations.getID
							.multipleMatches
					)
				);
				matches.forEach((match) => {
					window.logAsync(
						`${window.__(I18NKeys.crm.id)}:`,
						match.id,
						`, ${window.__(I18NKeys.crm.node)}:`,
						match.node
					);
				});
			}
		...
		window.filter = (
			nodeId: CRM.GenericNodeId | string,
			tabId: string | TabId | void
		) => {
		...
		function filterMessageText(
			messages: LogListenerLine[],
			filter: string
		): LogListenerLine[] {
			if (filter === '') {
				return messages;
			...
			return messages.filter((message: LogListenerLine) => {
				for (let i = 0; i < message.data.length; i++) {
					if (
						typeof message.data[i] !== 'function' &&
		...
		function updateLog(
			this: LogListenerObject,
			id: CRM.GenericNodeId | 'ALL',
		...
		window._getIdsAndTabs = async (
			...
			callback({
				ids: modules.Logging.Listeners.getIds(
					selectedTab === 'background' ? 0 : selectedTab
				),
				tabs: await modules.Logging.Listeners.getTabs(selectedId),
			});
		};
		window._getCurrentTabIndex = (
			id: CRM.GenericNodeId,
	...
	function permissionsChanged(available: _browser.permissions.Permissions) {
		modules.globalObject.globals.availablePermissions =
			available.permissions;
	}
	export async function refreshPermissions() {
		if ((window as any).chrome && (window as any).chrome.permissions) {
			const chromePermissions: typeof _chrome.permissions = (window as any)
				.chrome.permissions;
			if (
				'onRemoved' in chromePermissions &&
				'onAdded' in chromePermissions
			) {
				chromePermissions.onRemoved.addListener(permissionsChanged);
				chromePermissions.onAdded.addListener(permissionsChanged);
			}
		}
		const available = browserAPI.permissions
				...
				if (!tabNodeData.port) {
					if (
						...
						modules.Util.iterateMap(
							nodeInstances.get(message.id),
							(tabId) => {
								try {
									tabData
										.get(tabId)
										.nodes.get(message.id)
										.forEach((tabInstance, index, arr) => {
											if (
												tabId === message.tabId &&
												index === arr.length - 1
											) {
					...
					await modules.MessageHandling.handleCrmAPIMessage(
						(message as any) as
							| MessageHandling.CRMAPICallMessage
	...
	export function init() {
		...
		function setStatusForTree(
			tree: ContextMenuItemTreeItem[],
			enabled: boolean
		) {
		...
		function getFirstRowChange(
			row: ContextMenuItemTreeItem[],
			changes: {
		...
		async function reCreateNode(
			...
			const oldId = item.id;
			item.enabled = true;
			const { settings } = modules.crmValues.contextMenuInfoById.get(
				item.id
			);
			if (
				item.node &&
				item.node.type === 'stylesheet' &&
				item.node.value.toggle
			) {
			...
			modules.crmValues.contextMenuInfoById.set(
				id,
		...
		async function buildSubTreeFromNothing(
			parentId: string | number,
			tree: ContextMenuItemTreeItem[],
			changes: {
				[contextMenuId: string]: {
			...
			for (const item of tree) {
				if (
					(changes[item.id] && changes[item.id].type === 'show') ||
					!changes[item.id]
				) {
					await reCreateNode(parentId, item, changes);
				} else {
					modules.crmValues.contextMenuInfoById.get(
						item.id
					).enabled = false;
				}
			}
		...
		async function applyNodeChangesOntree(
			parentId: string | number,
			tree: ContextMenuItemTreeItem[],
			changes: {
				[contextMenuId: string]: {
		...
		function getNodeStatusses(
			subtree: ContextMenuItemTreeItem[],
			hiddenNodes: ContextMenuItemTreeItem[] = [],
			shownNodes: ContextMenuItemTreeItem[] = []
		) {
			for (let i = 0; i < subtree.length; i++) {
				if (subtree[i]) {
					(subtree[i].enabled ? shownNodes : hiddenNodes).push(
						subtree[i]
					);
					getNodeStatusses(
						subtree[i].children,
		...
		function getToHide(
			...
			return shown
				.map(({ node, id }) => {
					...
					if (
						modules.crmValues.hideNodesOnPagesData.has(node.id) &&
		...
		function getToEnable(
			tab: _browser.tabs.Tab,
			hidden: ContextMenuItemTreeItem[]
		): {
			node:
				| CRM.DividerNode
				| CRM.MenuNode
		...
		function getContextmenuTabOverrides(
			...
			if (
				!statuses.get(nodeId).tabs.has(tabId) ||
		...
		async function tabChangeListener(changeInfo: {
			...
			const currentTabId =
				changeInfo.tabIds[changeInfo.tabIds.length - 1];
			const tab = await browserAPI.tabs.get(currentTabId).catch((err) => {
				if (err.message.indexOf('No tab with id:') > -1) {
					if (modules.storages.failedLookups.length > 1000) {
						...
						modules.storages.failedLookups.push(
							'Cleaning up last 500 array items because size exceeded 1000...'
						);
					}
			...
			const { shownNodes, hiddenNodes } = getNodeStatusses(
				modules.crmValues.contextMenuItemTree
			);
			[
				...getToHide(tab, shownNodes),
				...getToEnable(tab, hiddenNodes),
			].forEach(({ node, id, type }) => {
				changes[id] = {
					node,
					type,
				};
			...
			modules.Util.asyncIterateMap(
				statuses,
				async (nodeId, { tabs, defaultCheckedValue }) => {
					const isStylesheet =
						modules.crm.crmById.get(nodeId).type === 'stylesheet';
					...
					const base = isStylesheet
						...
						: null;
					const overrides = getContextmenuTabOverrides(
						nodeId,
						currentTabId
					);
		...
		function onTabUpdated(
			...
			if (
				...
				changeInfo.url &&
				modules.Util.canRunOnUrl(changeInfo.url)
			) {
		...
		function onTabsRemoved(tabId: TabId) {
			...
			modules.Util.iterateMap(
				modules.crmValues.nodeInstances,
				(nodeId, nodeStatus) => {
			...
			for (let i = 0; i < deleted.length; i++) {
				if (
					(deleted[i] as any).node &&
					(deleted[i] as any).node.id !== undefined
				) {
					modules.crmValues.tabData
						.get(tabId)
						.nodes.get((deleted[i] as any).node.id)
						.forEach((tabInstance) => {
			...
			if (browserAPI.notifications) {
				browserAPI.notifications.onClicked.addListener(
					(notificationId: string) => {
						const notification = notificationListeners.get(
							notificationId
						);
						if (
							notification &&
							...
							modules.globalObject.globals.sendCallbackMessage(
								notification.tabId,
								notification.tabIndex,
								notification.id,
								{
									err: false,
									args: [notificationId],
		...
		function listenKeyCommands() {
			...
			const shortcutListeners =
				modules.globalObject.globals.eventListeners.shortcutListeners;
				...
				commands.forEach((registerCommand) => {
					if (registerCommand.name === command) {
						...
						permutations.forEach((permutation) => {
							...
							if (
								shortcutListeners.has(permutationKey) &&
	...
	export function runAlwaysRunNodes(tab: _browser.tabs.Tab) {
		...
		for (const { id, triggers } of modules.toExecuteNodes.onUrl
			.documentStart) {
			...
			tabs.map(async (tab) => {
				...
				switch (state) {
					case RestoreTabStatus.SUCCESS:
						window.logAsync(
							window.__(
								I18NKeys.background.globalDeclarations
									.tabRestore.success,
								tab.id
							)
						);
