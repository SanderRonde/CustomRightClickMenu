...
(() => {
		...
		static chain<T>(initializers: (() => RoughPromise<any>)[]) {
			return new RoughPromise<T>((resolve) => {
				...
				initializers[0]().then((result) => {
					if (initializers[1]) {
						RoughPromise.chain<T>(initializers.slice(1)).then(
							(result) => {
								resolve(result);
							}
						);
	...
	window.onExists = <T extends keyof C, C = Window>(
		key: T,
		container?: C
	): PromiseLike<C[T]> => {
		if (!container) {
	...
	const ElementI18nManager = (() => {
		...
		class Lang {
			...
			Files = class LangFiles {
				...
				static async loadLang(lang: LANGS) {
					...
					try {
						const langData = await this._loadFile(
			...
			private async _getDefaultLang() {
				...
				const availableLangs = acceptLangs.filter(
					(i) => Lang.SUPPORTED_LANGS.indexOf(i as LANGS) !== -1
				);
			...
			async fetchLang() {
				...
				const { lang } = (await browserAPI.storage.local.get(
					'lang'
				)) as {
					lang?: LANGS;
				};
			...
			async setLang(lang: LANGS) {
				...
				this._listeners.forEach(
					(l) =>
						l.onLangChange && l.onLangChange.call(l, lang, prevLang)
				);
				this.ready = (async () => {
			...
			private _getMessage(
				...
				for (let i = 0; i < replacements.length; i++) {
					...
					placeholderContents = placeholderContents.map(
						(placeholder) => {
		...
		__.addListener = langInstance.addLanguageChangeListener.bind(
			langInstance
		);
	...
	const register = (fn: any) => {
		...
		window.Polymer({
			...
			...{
				ready(
					this: Polymer.InitializerProperties & Polymer.PolymerElement
				) {
	...
	function propertyPersists<T extends keyof CSSStyleDeclaration>(
		property: T,
	...
	window.setDisplayFlex = (el: HTMLElement | SVGElement) => {
		if (_supportsFlexUnprefixed === null) {
			_supportsFlexUnprefixed = propertyPersists('display', 'flex');
		}
	...
	if (
		...
		location.href.indexOf('background.html') === -1
	) {
