...
(function(module, exports) {
	(function(mod) {
		...
		function CodeMirror(place, options) {
			...
			function getColorHex(colorName, colors) {
			...
			window.colorFunction = {
				...
				func: function(change, cm) {
					setTimeout(function () {
						...
						var match;
						var childNodes;
						var lines = $(cm.getDoc().cm.display.wrapper).find('.CodeMirror-code').children().toArray();
						for (var k = 0; k < lines.length + 1; k++) {
							line = $(lines[k]).children('pre').children('span');
							childNodes = line.find('span').toArray();
							for (var j = 0; j < childNodes.length; j++) {
								node = childNodes[j];
								if (node.className === 'cm-atom') {
									if (j <= childNodes.length - 3 && node.innerHTML === 'rgb' && childNodes[j + 1].className === 'cm-number' && childNodes[j + 2].className === 'cm-number' && childNodes[j + 3].className === 'cm-number') {
										node.style.borderBottom = '3px solid rgb(' +
											childNodes[j + 1].innerHTML + ', ' +
											childNodes[j + 2].innerHTML + ', ' +
											childNodes[j + 3].innerHTML + ')';
									} else {
										match = node.innerHTML.match(/#((\d|[a-f]|[A-F]){6}|(\d|[a-f]|[A-F]){3})/);
										if (match && match[0]) {
											node.style.borderBottom = '3px solid ' + match[0];
										}
									}
								} else if (node.className === 'cm-keyword') {
									var colors = {
										"aqua": '#00ffff',
										"aliceblue": '#f0f8ff',
										"antiquewhite": '#faebd7',
									...
									var color = getColorHex(node.innerHTML, colors);
									if (color) {
			...
			if (options.messageExternal) {
				...
				cm.on('change', function (instance, change) {
					window.colorFunction.func(change, cm);
				});
			...
			else if (options.messageTryEditor) {
				window.app.tryEditorLoaded.apply(window.app, [this]);
			}
		}
						...
						var i = 0,
							poll = function() {
								if (display.selForContextMenu == cm.doc.sel && te.selectionStart == 0 &&
									te.selectionEnd > 0 && input.prevInput == '\u200b') operation(cm, commands.selectAll)(cm);
