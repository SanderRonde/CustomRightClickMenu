...
(function (global, factory) {
	...
	function clone(el) {
		if (Polymer && Polymer.dom) {
			var clone = Polymer.dom(el).cloneNode(true);
			//Clone properties as well
			var srcClass = el.prototype && el.prototype.constructor;
			var props = srcClass && srcClass.properties;
			if (props) {
				var keys = Object.getOwnPropertyNames(props);
				for (var i = 0; i < keys.length; i++) {
					clone[keys[i]] = el[keys[i]];
				}
			} else {
				clone.classList.add('fallbackFiller');
			}
			return clone;
		}
		else if ($) {
	...
	/** @lends Sortable.prototype */
	{
	  ...
	  _disableDelayedDragEvents: function _disableDelayedDragEvents() {
		var ownerDocument = getOwner(this.el);
	  ...
	  _emulateDragOver: function _emulateDragOver() {
		if (touchEvt) {
		  ...
		  var target = getOwner(dragEl).elementFromPoint(touchEvt.clientX, touchEvt.clientY);
	  ...
	  _offMoveEvents: function _offMoveEvents() {
		  const _this = this;
		  [getOwner(this.el), document].forEach(function(owner) {
			off(owner, 'mousemove', _this._onTouchMove);
			off(owner, 'touchmove', _this._onTouchMove);
			off(owner, 'pointermove', _this._onTouchMove);
			off(owner, 'dragover', nearestEmptyInsertDetectEvent);
			off(owner, 'mousemove', nearestEmptyInsertDetectEvent);
			off(owner, 'touchmove', nearestEmptyInsertDetectEvent);
		  });
	  },
	  _offUpEvents: function _offUpEvents() {
		const _this = this;
		[getOwner(this.el), document].forEach(function(owner) {
			off(owner, 'mouseup', _this._onDrop);
			off(owner, 'touchend', _this._onDrop);
			off(owner, 'pointerup', _this._onDrop);
			off(owner, 'touchcancel', _this._onDrop);
			off(owner, 'selectstart', _this);
		});
	  },
