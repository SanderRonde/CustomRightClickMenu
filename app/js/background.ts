...
((extensionId: string, globalObject: GlobalObject, sandboxes: {
	...
	const Helpers = (() => {
		const exports = {
			...
			*/
			...
			checkForChromeErrors(log: boolean) {
		...
		const exports = {
			LogExecution: (() => {
				return {
					...
					displayHints(message: CRMAPIMessage<'displayHints', {
						hints: Array<string>;
						id: number;
	...
	const CRMFunctions = {
		...
		getScriptValue(__this: CRMFunction) {
			__this.checkPermissions(['crmGet'], () => {
				__this.getNodeFromId(__this.message.nodeId, true).run((node) => {
					if (node.type === 'script') {
						...
						if (node['scriptVal']) {
							this.respondSuccess(node['scriptVal'].script);
						} else {
	...
	const CRM = (() => {
		...
		const exports = {
			...
			NodeCreation: (() => {
				...
				function addRightClickItemClick(node: CRMNode, launchMode: CRMLaunchModes,
