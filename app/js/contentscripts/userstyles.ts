...
(() => {
	...
	*/
	function getStyleURL() {
		...
		const jsonUrl =
			getMeta('stylish-code-chrome') ||
			textUrl.replace(/styles\/(\d+)\/[^?]*/, 'styles/chrome/$1.json');
		const paramsMissing =
			jsonUrl.indexOf('?') === -1 && textUrl.indexOf('?') !== -1;
	...
	function checkUpdatability([installedStyle]: {
		...
		document.dispatchEvent(
			new CustomEvent('stylusFixBuggyUSOsettings', {
				detail: updateURL,
			})
		);
		if (!installedStyle) {
		...
		sendEvent({
			type:
				installedStyle.state === 'updatable'
	...
	function sendEvent(event: { type: string; detail?: any }) {
		lastEvent = event;
	...
	function onClick(
		...
		processing = true;
		(event.type.indexOf('Update') !== -1 ? onUpdate() : onInstall()).then(
			done,
			done
		);
		function done() {
			...
			await browserAPI.runtime.sendMessage({
				...
				data: {
					...
					author: document.querySelector('#style_author a')
						? document.querySelector('#style_author a').innerText
						: 'anonymous',
				},
			});
			sendEvent({
		...
		function enableUpdateButton(state: boolean) {
			...
			if (button) {
				button.style.cssText = state
					? ''
					: important('pointer-events: none; opacity: .35;');
				const icon = button.querySelector('img[src*=".svg"]');
				if (icon) {
					icon.style.cssText = state
						? ''
						: important(
								'transition: transform 5s; transform: rotate(0);'
						  );
					if (state) {
						setTimeout(
							() =>
								(icon.style.cssText += important(
...
document.documentElement.appendChild(
...
).text = `(${(EXTENSION_ORIGIN: string, isChrome: boolean) => {
			...
			constructor(width: number, height: number) {
				...
				window.setInterval(() => {
					if (
						img.src &&
						!loaded[img.src] &&
						/^chrome-extension:/i.test(img.src)
					) {
						loaded[img.src] = true;
						setTimeout(
							() =>
	...
	document.addEventListener('stylusFixBuggyUSOsettings', function _({
		detail,
	}: {
		detail: string;
	}) {
	...
	Response.prototype.json = function (...args: any[]) {
		return originalResponseJson
			.call(this, ...args)
			.then((json: { style_settings?: any }) => {
				if (
					!settings ||
					typeof ((json || {}).style_settings || {}).every !==
						'function'
				) {
					return json;
				}
				...
				for (const jsonSetting of json.style_settings) {
					...
					if (value.startsWith('ik-')) {
						...
						for (
							let i = 0;
							i < jsonSetting.style_setting_options.length;
							i++
						) {
						...
						if (!defaultItem || defaultItem.install_key !== value) {
							...
							jsonSetting.style_setting_options.filter(
								(item: any) => {
					...
					new MutationObserver((_, observer) => {
						...
						for (const [name, url] of images) {
							const elRadio = document.querySelector(
								`input[name="ik-${name}"][value="user-url"]`
							);
							const elUrl =
								elRadio &&
								(document.getElementById(
									elRadio.id.replace('url-choice', 'user-url')
								) as HTMLInputElement);
							if (elUrl) {
								elUrl.value = url;
							}
	...
	document.addEventListener('DOMContentLoaded', function _() {
		...
		new MutationObserver((_, observer) => {
			...
			const links = (document.querySelectorAll(
				'#pagination a[href*="page="]:not([href*="category="])'
