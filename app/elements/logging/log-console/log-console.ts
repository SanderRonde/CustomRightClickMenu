...
namespace LogConsoleElement {
	...
	export class LC {
		...
		static observers = ['_updateLog(selectedId, selectedTab, textfilter)'];
		...
		static async _takeToTab(this: LogConsole, event: Polymer.ClickEvent) {
			...
			const tab = await browserAPI.tabs.get(~~tabId).catch(async () => {
				window.logConsole.$.genericToast.text = await this.__async(
					I18NKeys.logging.tabClosed
				);
				...
				await chromeTabs.highlight(
					...
					async () => {
						if ((window as any).chrome.runtime.lastError) {
							...
							console.log(
								await this.__async(
									I18NKeys.logging.somethingWentWrong
								)
							);
						}
					}
		...
		private static _fixTextareaLines(this: LogConsole) {
			this.$.consoleInput.setAttribute(
				'rows',
				(this.$.consoleInput.value.split('\n').length || 1) + ''
			);
		...
		private static _getSelectedItems(
			this: LogConsole
		): {
			id: {
				id: string | CRM.GenericNodeId;
		...
		static _updateLog(
			this: LogConsole,
			_selectedId: CRM.GenericNodeId,
			_selectedTab: number,
			textfilter: string
		) {
			const selected = this._getSelectedValues();
			const lines: LogListenerLine[] =
				(this._logListener &&
					this._logListener.update(
						selected.id,
						selected.tab,
						selected.tabIndex,
						textfilter
		...
		private static _transitionSelected(
			this: LogConsole,
			prev: {
				id: string | number;
				title: string;
		...
		static _getIdTabs(
			this: LogConsole,
			selectedId: string | CRM.GenericNodeId
		): TabData[] {
			this.async(() => {
			...
			if (this._bgPage) {
				const id =
					selectedId === 0
						? (0 as CRM.GenericNodeId)
						: (~~this.ids[~~selectedId - 1]
								.id as CRM.GenericNodeId);
		...
		static _getTabsIds(
			...
			if (this._bgPage) {
				const tab: TabId | 'background' =
					selectedTab === 0 ? -1 : this.tabs[selectedTab - 1].id;
				this._bgPage._getIdsAndTabs(
					0 as CRM.GenericNodeId,
					tab,
					({ ids }) => {
		...
		static _getTabIndexes(
			...
			if (this._bgPage) {
				...
				const tab =
					selectedTab === 0 ? -1 : this.tabs[selectedTab - 1].id;
				this._bgPage._getCurrentTabIndex(
					id,
					tab,
					(newTabIndexes: TabIndex[]) => {
		...
		private static _processEvalLine(
			this: LogConsole,
			line: LogListenerLine
		) {
		...
		private static _refreshMenu(
			this: LogConsole,
			menu: PaperDropdownMenu,
			template: HTMLDomRepeatElement
		) {
			template.render();
			this.async(() => {
				menu.init();
		...
		static _contextStoreAsLocal(this: LogConsole) {
			...
			const index = (sourceLine.props.value as LogLineData[]).indexOf(
				sourceVal as LogLineData
			);
		...
		private static _contextCopyPath(
			this: LogConsole,
			...
			while (source.props.parent && !source.props.parent.isLine()) {
				...
				if (Array.isArray(source.props.value)) {
					path.push(
						'[' +
							source.props.value.indexOf(
								childValue as LogLineData
							) +
							']'
					);
				} else {
					...
					for (let i = 0; i < keys.length; i++) {
						if (
							(source.props.value as any)[keys[i]] === childValue
						) {
		...
		private static _setPossibleOptions(
			this: LogConsole,
			...
			this.$.copyAsJSON.classList[enableCopyAsJSON ? 'remove' : 'add'](
				'disabled'
			);
			this.$.storeAsLocal.classList[
				enableCreateLocalVar ? 'remove' : 'add'
			]('disabled');
		}
		...
		static ready(this: LogConsole) {
			...
			this._logLines = (ReactDOM.render(
				...
				this.$.lines
			) as any) as LogLineContainerInterface;
...
export type LogConsole = Polymer.El<
	'log-console',
