...
import {
	PaperDropdownMenu,
	PaperDropdownMenuBase,
} from '../paper-dropdown-menu/paper-dropdown-menu';
...
namespace PaperDropdownBehaviorNamespace {
	...
	export class PDB {
		...
		static onValueChange(
			this: PaperDropdownInstance,
			_oldState: number | number[],
			_newState: number | number[]
		) {}

		/**
		 * Fires all added listeners, triggers when a new value is selected
		 */
		static _fireListeners(
			this: PaperDropdownInstance,
			...
			this._listeners.forEach((listener) => {
				if (listener.id === this.id) {
					listener.listener.apply(listener.thisArg, [
						oldState,
						newState,
					]);
				}
		...
		static querySlot<K extends keyof Polymer.ElementTagNameMap>(
			parent: HTMLElement | Polymer.RootElement,
			selector?: string,
			slotSelector?: string
		): (HTMLElement | Polymer.PolymerElement)[] | null;
		static querySlot<K extends keyof Polymer.ElementTagNameMap>(
			parent: HTMLElement | Polymer.RootElement,
			selector?: K,
			slotSelector?: string
		): Polymer.ElementTagNameMap[K][] | null;
		static querySlot<K extends keyof Polymer.ElementTagNameMap>(
			parent: HTMLElement | Polymer.RootElement,
			selector?: string,
			slotSelector?: string
		...
		static querySlot<K extends keyof Polymer.ElementTagNameMap>(
			parent: HTMLElement | Polymer.RootElement,
			selector: K | string = null,
			slotSelector: string = 'slot'
		): (Polymer.ElementTagNameMap[K] | HTMLElement)[] | null {
			const selectFn =
				'$$' in parent ? (parent as any).$$ : parent.querySelector;
			const slotChildren = (selectFn.bind(parent)(
				slotSelector
			) as HTMLSlotElement)
				.assignedNodes()
				.filter((node) => {
		...
		static doHighlight(this: PaperDropdownInstance) {
			...
			const paperItems = Array.prototype.slice.apply(
				content.querySelectorAll('paper-item')
			);
			paperItems.forEach(
				(paperItem: HTMLPaperItemElement, index: number) => {
					const checkMark = this.querySlot(
						paperItem
					)[0] as HTMLElement;
					if (!checkMark) {
						return;
			...
			const interval = window.setInterval(() => {
				if (this.getMenu() && this._getMenuContent()) {
					...
					const innerInterval = window.setInterval(() => {
						if (
							this._getMenuContent().querySelectorAll(
								'paper-item'
							)[0] &&
							this.querySlot(
								this._getMenuContent().querySelectorAll(
		 ...
		 */
		static _animateBoxShadowIn(
			timestamp: number,
			__this: PaperDropdownInstance
		) {
			...
			if (timestamp - 100 < __this._startTime) {
				...
				__this._getMenuContent().style.boxShadow =
					'0 ' +
					doubleScale +
					'px ' +
					doubleScale +
					'px 0 rgba(0,0,0,0.14),' +
					' 0 ' +
					scale +
					'px ' +
					5 * scale +
					'px 0 rgba(0,0,0,0.12),' +
					' 0 ' +
					scale * 3 +
					'px ' +
					scale +
					...
					'px rgba(0,0,0,0.2)';
				if (!__this.indent) {
					__this._dropdownSelectedCont.style.marginLeft =
						scale * 15 + 'px';
				}
				window.requestAnimationFrame(function (time) {
					__this._animateBoxShadowIn(time, __this);
		...
		static _animateBoxShadowOut(
			...
			if (timestamp - 100 < __this._startTime) {
				...
				let doubleScale = scale * 2;
				__this.getMenu().style.boxShadow =
					'0 ' +
					doubleScale +
					'px ' +
					doubleScale +
...
export type PaperDropdownBehaviorBase = Polymer.El<
	'paper-dropdown-behavior',
	typeof PaperDropdownBehaviorNamespace.PDB &
		typeof PaperDropdownBehaviorNamespace.paperDropdownBehaviorProperties
>;
