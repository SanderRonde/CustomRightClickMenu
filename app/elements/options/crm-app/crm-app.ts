...
import {
	Polymer,
	ElementTagNameMaps,
} from '../../../../tools/definitions/polymer';
...
namespace CRMAppElement {
	...
	window.runOrAddAsCallback = function (
		toRun: Function,
	...
	(() => {
		...
		const animatePolyFill = function (
			this: HTMLElement,
			properties: {
				[key: string]: any;
			}[],
			...
			var returnVal: Animation = {
				play() {
					...
					state.finishedPromise = new Promise<Animation>(
						(resolve) => {
					...
					$(element)
						.stop()
						.animate(properties[~~(direction === 'forwards')], {
							duration: duration,
							complete() {
								state.playState = 'finished';
								state.isPaused = false;
								state.finishPromise &&
									state.finishPromise(returnVal);
				...
				cancel() {
					...
					returnVal.oncancel &&
						returnVal.oncancel.apply(returnVal, {
							currentTime: Date.now(),
							timelineTime: null,
						});
				},
	...
	type TransferOnError = (
		position: TransferOnErrorError,
		passes: number
	) => void;

	type ScriptUpgradeErrorHandler = (
	...
	class CA {
		...
		private static _getRegisteredListener(
			...
			return (element as Polymer.PolymerElement).getAttribute(
				`data-on-${eventType}`
			) as keyof typeof listeners;
		}

		static domListener(this: CrmApp, event: Polymer.CustomEvent) {
			const listeners = this.listeners;

			const fnName: keyof typeof listeners = window.app.util.iteratePath(
				event,
				(element) => {
			...
			if (fnName) {
				if (
					...
					(listener as (
						...
						eDetail: Polymer.CustomEvent['detail']
					) => void).bind(listeners)(event, event.detail);
				} else {
					console.warn.apply(
						console,
						this._logf(
							`_createEventHandler`,
							`listener method ${fnName as any} not defined`
						)
					);
				}
			} else {
				console.warn.apply(
					...
					this._logf(
						`_createEventHandler`,
						`property data-on${event.type} not defined`
					)
				);
			}
		}
		...
		static generateCodeOptionsArray<T extends CRM.Options>(
			this: CrmApp,
			settings: T | string
		): {
			key: keyof T;
			...
			return Object.getOwnPropertyNames(settings)
				.map((key: keyof T) => {
					if (key === '$schema') {
						return null;
					}
		...
		private static _getUpdatedScriptString(
			this: CrmApp,
			updatedScript: {
				name: string;
				oldVersion: string;
			...
			return this.___(
				I18NKeys.crmApp.code.nodeUpdated,
				updatedScript.name,
		...
		static _getPermissionDescription(
			this: CrmApp
		...
		static _getNodeVersion(this: CrmApp, nodeId: CRM.GenericNodeId) {
			return (
				(window.app.nodesById.get(nodeId).nodeInfo &&
		...
		static _placeCommas(this: CrmApp, num: number): string {
			const split = this._reverseString(num.toString()).match(
				/[0-9]{1,3}/g
			);
		...
		static _getStorageSyncDisabledReason(this: CrmApp) {
			if (!this._supportsStorageSync()) {
				return this.___(
					I18NKeys.crmApp.options.useStorageSyncDisabledUnavailable
				);
			} else {
				return this.___(
					I18NKeys.crmApp.options.useStorageSyncDisabledTooBig
				);
			}
		}
		...
		private static _findScriptsInSubtree(
			this: CrmApp,
			toFind: CRM.Node,
			container: CRM.Node[]
		) {
		...
		private static async _runDialogsForImportedScripts(
			this: CrmApp,
			nodesToAdd: CRM.Node[],
			dialogs: CRM.ScriptNode[]
		) {
			if (dialogs[0]) {
		...
		private static _addImportedNodes(
			this: CrmApp,
			nodesToAdd: CRM.Node[]
		): boolean {
			if (!nodesToAdd[0]) {
		...
		private static _genRequestPermissionsHandler(
			this: CrmApp,
			overlayContainer: {
			...
			const fn = () => {
				...
				$(
					...
					.on('click', function (this: HTMLElement) {
						...
						const otherPermissions = $(this)
							.parent()
							.parent()
							.parent()
							.children('#requestPermissionsOther')[0];
						if (
							!otherPermissions.style.height ||
							otherPermissions.style.height === '0px'
						) {
							$(otherPermissions).animate(
								{
									height:
										otherPermissions.scrollHeight + 'px',
								},
								350,
								function () {
									((<unknown>(
										showHideSvg.children[0]
									)) as HTMLElement).style.display = 'none';
									((<unknown>(
										showHideSvg.children[1]
				...
				$(this.shadowRoot.querySelectorAll('.requestPermissionButton'))
					.off('click')
					.on('click', function (this: HTMLPaperCheckboxElement) {
						permission = this.previousElementSibling
						...
						browserAPI.permissions
							...
							.then((accepted) => {
								if (accepted) {
									...
									$('.requestPermissionButton.required').each(
										function (
											this: HTMLPaperCheckboxElement
										) {
											this.checked = true;
										}
									);
		 ...
		 */
		private static async _requestPermissions(
			this: CrmApp,
			toRequest: CRM.Permission[],
			force: boolean = false
		) {
			...
			if (toRequest.length > 0 || force) {
				const allowed = browserAPI.permissions
				...
				for (i = 0; i < toRequest.length; i++) {
					requested.push({
						name: toRequest[i],
						description: this.templates.getPermissionDescription(
							toRequest[i]
						),
						toggled: false,
					});
				...
				for (i = 0; i < allPermissions.length; i++) {
					other.push({
						...
						toggled:
							allowed.permissions.indexOf(
								(allPermissions as _browser.permissions.Permission[])[
									i
								]
							) > -1,
					});
				}
				const requestPermissionsOther = this.$$(
				...
				const handler = this._genRequestPermissionsHandler(
					overlayContainer,
					toRequest
				);
				...
				const interval = window.setInterval(() => {
					try {
						const centerer = window.doc
							.requestPermissionsCenterer as CenterElement;
						const overlay = (overlayContainer.overlay = window.app.util.getQuerySlot()(
							centerer
						)[0] as HTMLPaperDialogElement);
						if (overlay.open) {
							window.clearInterval(interval);
							const innerOverlay = window.app.util.getQuerySlot()(
								overlay
							)[0] as HTMLElement;
							...
							setTimeout(function () {
								const requestedPermissionsCont = innerOverlay.querySelector(
									'#requestedPermissionsCont'
								);
								const requestedPermissionsAcceptAll = innerOverlay.querySelector(
									'#requestPermissionsAcceptAll'
								);
								const requestedPermissionsType = innerOverlay.querySelector(
									'.requestPermissionsType'
								);
								if (requested.length === 0) {
									requestedPermissionsCont.style.display =
										'none';
									requestPermissionsOther.style.height =
										31 * other.length + 'px';
									requestedPermissionsAcceptAll.style.display =
										'none';
									requestedPermissionsType.style.display =
										'none';
								} else {
									requestedPermissionsCont.style.display =
										'block';
		...
		private static async _transferCRMFromOld(
			this: CrmApp,
			openInNewTab: boolean,
			storageSource: {
				getItem(index: string | number): any;
			} = localStorage,
			method: SCRIPT_CONVERSION_TYPE = SCRIPT_CONVERSION_TYPE.BOTH
		): Promise<CRM.Tree> {
			return await this._transferFromOld.transferCRMFromOld(
				openInNewTab,
				storageSource,
				method
			);
		...
		static initCodeOptions(
			...
			this.$.codeSettingsRepeat.items = this.generateCodeOptionsArray(
				node.value.options
			);
			this.$.codeSettingsNoItems.if =
				this.$.codeSettingsRepeat.items.length === 0;
			this.$.codeSettingsRepeat.render();
			this.async(() => {
				this.$.codeSettingsDialog.fit();
				Array.prototype.slice
					.apply(
						this.$.codeSettingsDialog.querySelectorAll(
							'paper-dropdown-menu'
						)
					)
		...
		static async versionUpdateChanged(this: CrmApp) {
			if (this._isVersionUpdateTabX(this.versionUpdateTab, 1)) {
				...
				if (!versionUpdateDialog.editorManager) {
					versionUpdateDialog.editorManager = await this.$.tryOutEditor.create(
						this.$.tryOutEditor.EditorMode.JS,
						{
							value:
							...
							wordWrap: 'off',
							fontSize:
								(~~window.app.settings.editor.zoom / 100) * 14,
							folding: true,
						}
					);
				}
		...
		static toggleShrinkTitleRibbon(this: CrmApp) {
			...
			if (window.app.storageLocal.shrinkTitleRibbon) {
				...
				window.setTransform(
					window.doc.shrinkTitleRibbonButton,
					'rotate(270deg)'
				);
			...
			window.app.storageLocal.shrinkTitleRibbon = !window.app.storageLocal
				.shrinkTitleRibbon;
			browserAPI.storage.local.set({
		...
		static addSettingsReadyCallback(
			this: CrmApp,
			callback: Function,
			thisElement: HTMLElement,
			params: any[]
		...
		private static _assertCRMNodeShape(
			this: CrmApp,
			node: CRM.Node
		): boolean {
			let changed = false;
		...
		static setLocal<K extends keyof CRM.StorageLocal>(
			this: CrmApp,
			key: K,
			value: CRM.StorageLocal[K]
		) {
			const obj = {
				[key]: value,
			};
			...
			browserAPI.storage.local
				...
				.then((storageLocal) => {
					...
					if (key === 'CRMOnPage' || key === 'editCRMInRM') {
						(window.doc.editCRMInRM as PaperToggleOption)
							.setCheckboxDisabledValue &&
							(window.doc
								.editCRMInRM as PaperToggleOption).setCheckboxDisabledValue(
								!storageLocal.CRMOnPage
							);
		...
		static async refreshPage(this: CrmApp) {
			...
			Array.prototype.slice
				.apply(this.shadowRoot.querySelectorAll('default-link'))
				.forEach(function (link: DefaultLink) {
					link.reset();
				});

			//Reset regedit part
			window.doc.URISchemeFilePath.value = 'C:\\files\\my_file.exe';
			window.doc.URISchemeSchemeName.value = await this.__async(
				I18NKeys.crmApp.uriScheme.example
			);

			//Hide all open dialogs
			Array.prototype.slice
				.apply(this.shadowRoot.querySelectorAll('paper-dialog'))
				.forEach((dialog: HTMLPaperDialogElement) => {
		...
		private static _getDotValue<
			T extends {
				[key: string]: T | U;
			},
			U
		>(this: CrmApp, source: T, index: string): U {
		...
		private static _typesMatch(
			...
			errors.push({
				err: `Value for ${data.val} is not of type ${types.join(
					' or '
				)}`,
			});
			return null;
		...
		private static _checkNumberConstraints(
			this: CrmApp,
			data: TypeCheckConfig,
			value: number,
			errors: TypeCheckErrors
		...
		private static _checkArrayChildType(
			this: CrmApp,
			data: TypeCheckConfig,
			value: any,
			forChild: {
			...
			const types = Array.isArray(forChild.type)
				? forChild.type
				: [forChild.type];
			for (let i = 0; i < types.length; i++) {
				const type = types[i];
			...
			errors.push({
				err: `For not all values in the array ${
					data.val
				} is the property ${forChild.val} of type ${types.join(
					' or '
				)}`,
			});
			return false;
		...
		private static _checkArrayChildrenConstraints<
			T extends {
				[key: string]: any;
			}
		>(
		...
		private static _checkConstraints(
			this: CrmApp,
			data: TypeCheckConfig,
			value: any,
			errors: TypeCheckErrors
		...
		private static typeCheck(
			...
			for (let i = 0; i < toCheck.length; i++) {
				...
				//Check if it's defined
				const isDefined = this._isDefined(
					config,
					value,
					optionals,
					errors
		...
		private static _setupConsoleInterface(this: CrmApp) {
			window.consoleInfo = () => {
				this._logCode(
					'Edit local (not synchronized with your google account) settings as follows:'
				);
				this._logCode(
					'	',
					this._codeStr(
						'window.app.storageLocal.<setting> = <value>;'
					)
				);
				this._logCode(
					'	For example: ',
				...
				this._logCode(
					'	To get the type formatting of local settings call ',
					this._codeStr('window.getLocalFormat();')
				);
				this._logCode(
					'	To read the current settings just call ',
					this._codeStr('window.app.storageLocal;')
				);
				this._logCode('');
				this._logCode('Edit synchronized settings as follows:');
				this._logCode(
					'	',
					this._codeStr('window.app.settings.<setting> = <value>')
				);
				this._logCode(
					'	For example: ',
					this._codeStr('window.app.settings.rootName = "ROOT";')
				);
				this._logCode(
					'	Or: ',
					this._codeStr('window.app.settings.editor.theme = "white";')
				);
				this._logCode(
					'	To get the type formatting of local settings call ',
					this._codeStr('window.getSyncFormat();')
				);
				this._logCode(
					'	To read the current settings just call ',
					this._codeStr('window.app.settings;')
				);
				this._logCode('');
				...
				this._logCode(
					'	',
					this._codeStr(
						'window.app.settings.crm[<index>].<property> = <value>'
					)
				);
				this._logCode(
					'	For example: ',
					this._codeStr('window.app.settings.crm[0].name = "MyName";')
				);
				this._logCode(
					'	To find the index either call ',
					this._codeStr('window.app.settings.crm;'),
					' or ',
					this._codeStr('window.getIndexByName("<name>");')
				);
				this._logCode(
					'	To get the type formatting of a CRM node call ',
					this._codeStr('window.getCRMFormat();')
				);
				this._logCode('');
				this._logCode(
					'To force upload any changes you made call ',
					this._codeStr('window.upload();')
				);
				this._logCode(
					'To look at the changes that were made call ',
					this._codeStr('window.getChanges();')
				);
				this._logCode(
					'To check the format of your changes call ',
					this._codeStr('window.checkFormat();')
				);
				this._logCode(
					'To upload changes you made if the format is correct call ',
					this._codeStr('window.uploadIfCorrect();')
				);
			};
			window.getLocalFormat = () => {
				this._logCode(
					'Format can be found here https://github.com/SanderRonde/CustomRightClickMenu/blob/polymer-2/tools/definitions/crm.d.ts#L1148'
				);
			};
			window.getSyncFormat = () => {
				this._logCode(
					'Format can be found here https://github.com/SanderRonde/CustomRightClickMenu/blob/polymer-2/tools/definitions/crm.d.ts#L1091'
				);
			};
			window.getCRMFormat = () => {
				this._logCode(
					'Format can be found here https://github.com/SanderRonde/CustomRightClickMenu/blob/polymer-2/tools/definitions/crm.d.ts#L1103'
				);
			};
			window.upload = window.app.upload;
			window.getChanges = () => {
				this._logCode(
					'Here are the changes that have been made. Keep in mind that this includes unuploaded changes the extension made.'
				);
				this._logCode('');
				const {
					hasLocalChanged,
				...
				if (!hasLocalChanged) {
					...
					this._logCode(
						'The following changes to local storage were made'
					);
					for (const change of localChanges) {
						this._logCode(
							'Key ',
							this._codeStr(change.key),
							' had value ',
							this._codeStr(change.oldValue),
							' and was changed to ',
				...
				if (!haveSettingsChanged) {
					...
					this._logCode(
						'The following changes to synced storage were made'
					);
		...
		static ready(this: CrmApp) {
			...
			browserAPI.runtime.onInstalled.addListener(async (details) => {
				if (details.reason === 'update') {
					...
					this.$.messageToast.text = this.___(
						I18NKeys.crmApp.code.extensionUpdated,
			...
			if (this._onIsTest()) {
				var dummyContainer = (window.dummyContainer = document.createElement(
					'div'
				));
		...
		private static _transferFromOld = class CRMAppTransferFromOld {
			private static _backupLocalStorage() {
				if (
					...
					(typeof window.indexedDB === 'undefined' &&
						typeof (window as any).webkitIndexedDB === 'undefined')
				) {
					return;
				}
				const data = JSON.stringify(localStorage);
				const idb: IDBFactory =
					window.indexedDB || (window as any).webkitIndexedDB;
				const req = idb.open('localStorageBackup', 1);
				req.onerror = () => {
					console.log('Error backing up localStorage data');
				};
			...
			private static _parseOldCRMNode(
				string: string,
				openInNewTab: boolean,
				...
				switch (type) {
					...
					case 'link':
						...
						node = this.parent().templates.getDefaultLinkNode({
							name: name,
							id: this.parent().generateItemId() as CRM.NodeId<
								CRM.LinkNode
							>,
					...
					case 'script':
						...
						node = this.parent().templates.getDefaultScriptNode({
							...
							value: {
								...
								script: this.parent().legacyScriptReplace.convertScriptFromLegacy(
									scriptData,
									id,
									method
								),
			...
			private static _chainPromise<T>(
				promiseInitializers: (() => Promise<T>)[],
				index: number = 0
			): Promise<T> {
				return new Promise<T>((resolve, reject) => {
					promiseInitializers[index]().then(
						(value) => {
			...
			private static async _execFile(path: string): Promise<void> {
				...
				el.src = browserAPI.runtime.getURL(
			...
			static async transferCRMFromOld(
				openInNewTab: boolean,
				storageSource: {
					getItem(index: string | number): any;
				},
				...
				let i;
				const amount =
					parseInt(storageSource.getItem('numberofrows'), 10) + 1;

				const nodes = [];
				for (i = 1; i < amount; i++) {
					nodes.push(
						this._parseOldCRMNode(
							storageSource.getItem(i),
							openInNewTab,
							method
						)
					);
		...
		private static _setup = class CRMAppSetup {
			private static async _restoreUnsavedInstances(editingObj: {
				...
				const crmItem = this.parent().nodesById.get(editingObj.id) as
					| CRM.ScriptNode
					| CRM.StylesheetNode;
				const code =
					crmItem.type === 'script'
						? editingObj.mode === 'main'
							? crmItem.value.script
							: crmItem.value.backgroundScript
						: crmItem.value.stylesheet;
				this.parent().listeners.iconSwitch(null, editingObj.crmType);
				this.parent().$.keepChangesButton.addEventListener(
					'click',
					() => {
				...
				const stopHighlighting = (element: HTMLEditCrmItemElement) => {
					...
					item.animate(
						...
						Array.prototype.slice
							.apply(
								this.parent().shadowRoot.querySelectorAll(
									'.crmType'
								)
							)
							.forEach((crmType: HTMLElement) => {
						...
						window.setTimeout(() => {
							window.doc.restoreChangesDialog.style.display =
								'block';
						}, 200);
					};
				};
				...
				const highlightItem = () => {
					...
					const columnConts = this.parent().editCRM.$
						.CRMEditColumnsContainer.children;
					const columnCont = columnConts[path.length - 1];
					const paperMaterial = columnCont.querySelector(
						'.paper-material'
					);
					const crmEditColumn = paperMaterial.querySelector(
						'.CRMEditColumn'
					);
					const editCRMItems = crmEditColumn.querySelectorAll(
						'edit-crm-item'
					);
					...
					if (crmElement.$$('.item')) {
						...
						const crmeditItemItems = window.app.editCRM
							.getItems()
							.map((element) => {
								return element.$$('.item');
							});
				...
				window.setTimeout(async () => {
					...
					const type =
						crmItem.type === 'script'
							? isTs
								? me.EditorMode.TS_META
								: me.EditorMode.JS_META
							: me.EditorMode.CSS_META;
					editor = await window.doc.restoreChangesEditor.createDiff(
						[code, editingObj.val],
						type,
						{
							wordWrap: 'off',
			...
			private static _bindListeners() {
				...
				window.doc.addLibraryUrlOption.addEventListener(
					'change',
					function () {
				...
				window.doc.addLibraryManualOption.addEventListener(
					'change',
					function () {
				...
				$('#addLibraryDialog').on('iron-overlay-closed', function (
					...
					$(this)
						.find(
							'#addLibraryButton, #addLibraryConfirmAddition, #addLibraryDenyConfirmation'
						)
						.off('click');
				});
			}
			...
			static async setupStorages() {
				...
				const storageLocal = await browserAPI.storage.local.get<
					CRM.StorageLocal & {
						nodeStorage: any;
						settings?: CRM.SettingsStorage;
					}
				...
				function callback(items: CRM.SettingsStorage) {
					...
					for (
						let i = 0;
						i < parent.onSettingsReadyCallbacks.length;
						i++
					) {
				...
				Array.prototype.slice
					.apply(
						parent.shadowRoot.querySelectorAll(
							'paper-toggle-option'
						)
					)
					.forEach(function (setting: PaperToggleOption) {
				...
				if (
					storageLocal.requestPermissions &&
					storageLocal.requestPermissions.length > 0
				) {
					if (browserAPI.permissions) {
						await parent._requestPermissions(
							storageLocal.requestPermissions as CRM.Permission[]
						);
					}
				}
				if (storageLocal.editing) {
					...
					setTimeout(function () {
						...
						const node = parent.nodesById.get(editing.id) as
							| CRM.ScriptNode
							| CRM.StylesheetNode;
						const nodeCurrentCode =
							node.type === 'script'
								? node.value.script
				...
				if (
					storageLocal.globalExcludes &&
					storageLocal.globalExcludes.length > 1
				) {
				...
				if (
					...
					storageLocal.addedPermissions.length > 0
				) {
					window.setTimeout(function () {
						(window.doc
							.addedPermissionsTabContainer as AddedPermissionsTabContainer).tab = 0;
						(window.doc
							.addedPermissionsTabContainer as AddedPermissionsTabContainer).maxTabs =
						...
						if (storageLocal.addedPermissions.length === 1) {
							(window.doc.addedPermissionNextButton.querySelector(
								'.next'
							) as HTMLElement).style.display = 'none';
						} else {
							(window.doc.addedPermissionNextButton.querySelector(
						...
						window.doc.addedPermissionPrevButton.style.display =
							'none';
				...
				if (
					storageLocal.settingsVersionData &&
					storageLocal.settingsVersionData.wasUpdated
				) {
					...
					toast.text = this.parent().___(
						I18NKeys.crmApp.code.settingsUpdated,
				...
				if (storageLocal.isTransfer) {
					...
					//Lazyload the image
					window.app.$.stylesheetGif.src = window.app.$.stylesheetGif.getAttribute(
						'data-src'
					);
				...
				parent._storageLocalCopy = JSON.parse(
					JSON.stringify(storageLocal)
				);
				if (
					storageLocal.useStorageSync &&
					parent._supportsStorageSync()
				) {
						...
						if (
							...
							const indexesLength =
								typeof indexes === 'number'
									...
									: 0;
							window.app.util
								.createArray(indexesLength)
								.forEach((_, index) => {
									settingsJsonArray.push(
										sync[`section${index}`]
									);
								});
							...
							if (parent.settingsJsonLength >= 102400) {
								window.app.$.useStorageSync.setCheckboxDisabledValue(
									true
								);
							}
							callback(settings);
					...
					parent.settingsJsonLength = JSON.stringify(
					...
					if (
						!parent._supportsStorageSync() ||
						parent.settingsJsonLength >= 102400
					) {
						window.app.$.useStorageSync.setCheckboxDisabledValue(
							true
						);
						...
						window.setTimeout(() => {
							...
							window.setTimeout(() => {
								...
								if (BrowserAPI.getBrowser() === 'edge') {
									console.log(
										this.parent().___(
											I18NKeys.crmApp.code.hiMessage
										)
									);
								} else {
									console.log(
								...
								console.log(
									this.parent().___(
										I18NKeys.crmApp.code.consoleInfo
									)
								);
							}, 200);
			...
			static switchToIcons(indexes: boolean[]) {
				...
				let i;
				let element;
				const crmTypes = this.parent().shadowRoot.querySelectorAll(
					'.crmType'
				);
				for (i = 0; i < 6; i++) {
					if (indexes[i]) {
		...
		static uploading = class CRMAppUploading {
			private static _areValuesDifferent(
				val1: any[] | Object,
				val2: any[] | Object
			): boolean {
				//Array or object
					...
					if (!obj2ValIsObjOrArray) {
						...
						if (obj1ValIsArray) {
							...
							if (!obj2ValIsArray) {
								...
								if (
									!this._parent().util.compareArray(
										val1 as any[],
										val2 as any[]
									)
								) {
							...
							if (obj2ValIsArray) {
								...
								if (
									!this._parent().util.compareObj(val1, val2)
								) {
									//Changes have been found, also say the container arrays have changed
									return true;
								}
			...
			private static _getObjDifferences<T, S>(
				obj1: {
					[key: string]: T;
					[key: number]: T;
				},
			...
			static getChanges(
				...
				const hasLocalChanged = this._getObjDifferences(
					local,
					force ? {} : localCopy,
					localChanges
				);
				const haveSettingsChanged = this._getObjDifferences(
					sync,
					force ? {} : syncCopy,
					settingsChanges
				);
			...
			private static _updateCopies() {
				this._parent()._storageLocalCopy = JSON.parse(
					JSON.stringify(this._parent().storageLocal)
				);
				this._parent()._settingsCopy = JSON.parse(
					JSON.stringify(this._parent().settings)
				);
			}
				...
				if (hasLocalChanged || haveSettingsChanged) {
					...
					browserAPI.runtime.sendMessage({
						...
						data: {
							...
							settingsChanges:
								haveSettingsChanged && settingsChanges,
						},
					});
				}
		...
		static legacyScriptReplace = class LegacyScriptReplace {
				...
				static findExpression(
					...
					switch (expression.type) {
						...
						case 'VariableDeclaration':
							for (
								let i = 0;
								i < expression.declarations.length;
								i++
							) {
						...
						case 'MemberExpression':
							...
							return this.findExpression(
								expression.property as Tern.Identifier,
								data,
								strToFind,
								onFind
							);
						...
						case 'FunctionDeclaration':
							for (
								let i = 0;
								i < expression.body.body.length;
								i++
							) {
								if (
									this.findExpression(
										expression.body.body[i],
										data,
										strToFind,
										onFind
									)
						...
						case 'ObjectExpressions':
							for (
								let i = 0;
								i < expression.properties.length;
								i++
							) {
								if (
									this.findExpression(
										expression.properties[i].value,
										data,
										strToFind,
										onFind
									)
			...
			static chromeCallsReplace = class ChromeCallsReplace {
				...
				private static _getFunctionCallExpressions(
					data: ChromePersistentData
				): Tern.Expression {
				...
				private static _getChromeAPI(
					expr: Tern.Expression,
					data: ChromePersistentData
				): {
					call: string;
					args: string;
					...
					const args = data.persistent.script.slice(
						argsStart,
						argsEnd
					);
				...
				private static _getLineIndexFromTotalIndex(
					lines: string[],
					line: number,
					index: number
				): number {
				...
				private static _replaceChromeFunction(
					data: ChromePersistentData,
					expr: Tern.Expression,
					callLine: {
						from: {
					...
					var lineExprStart = this._getLineIndexFromTotalIndex(
						data.persistent.lines,
						callLine.from.line,
						(data.returnExpr && data.returnExpr.start) ||
							expr.callee.start
					);
					var lineExprEnd = this._getLineIndexFromTotalIndex(
						data.persistent.lines,
						callLine.from.line,
						expr.callee.end
					);
					...
					var newLine =
						firstLine.slice(0, lineExprStart) +
					...
					if (data.isReturn) {
						var lineRest = firstLine.slice(
							lineExprEnd + chromeAPI.args.split('\n')[0].length
						);
						while (lineRest.indexOf(';') === 0) {
							lineRest = lineRest.slice(1);
						}
						newLine +=
							`.return(function(${data.returnName}) {` + lineRest;
						var usesTabs = true;
						var spacesAmount = 0;
						...
						for (let i = 0; i < data.persistent.lines.length; i++) {
							if (data.persistent.lines[i].indexOf('	') === 0) {
								...
								data.persistent.lines[i].indexOf('  ') === 0
							) {
						...
						var idx = null;
						for (
							i = data.parentExpressions.length - 1;
							scopeLength === null && i !== 0;
							i--
						) {
							if (
								data.parentExpressions[i].type ===
									'BlockStatement' ||
								(data.parentExpressions[i].type ===
									'FunctionExpression' &&
									(data.parentExpressions[i]
										.body as Tern.BlockStatement).type ===
										'BlockStatement')
							) {
								scopeLength = this._getLineIndexFromTotalIndex(
									data.persistent.lines,
									callLine.from.line,
									data.parentExpressions[i].end
								);
								idx = 0;
								...
								while (scopeLength > 0) {
									scopeLength = this._getLineIndexFromTotalIndex(
										data.persistent.lines,
										callLine.from.line + ++idx,
										data.parentExpressions[i].end
									);
								}
								scopeLength = this._getLineIndexFromTotalIndex(
									data.persistent.lines,
									callLine.from.line + (idx - 1),
									data.parentExpressions[i].end
								);
							}
						}
						...
						for (
							i = callLine.from.line;
							i < callLine.from.line + (idx - 1);
							i++
						) {
						...
						lines[callLine.from.line + (i - 1)] =
							lines[callLine.from.line + (i - 1)] + '.send();';
						if (i === 1) {
							newLine += '.send();';
						}
					}
				...
				private static _callsChromeFunction(
					callee: Tern.FunctionCallExpression,
					data: ChromePersistentData,
					onError: TransferOnError
				): boolean {
					...
					if (callee.arguments && callee.arguments.length > 0) {
						for (let i = 0; i < callee.arguments.length; i++) {
							if (
								this._findChromeExpression(
									callee.arguments[i],
									this._removeObjLink(data),
									onError
								)
							) {
								return true;
					...
					if (callee.type !== 'MemberExpression') {
						...
						return this._findChromeExpression(
							callee,
							this._removeObjLink(data),
							onError
						);
					...
					if (callee.property) {
						...
						data.functionCall.push(
							callee.property.name || (callee.property as any).raw
						);
					}
					if (callee.object && callee.object.name) {
						//First object
						const isWindowCall =
							this._isProperty(callee.object.name, 'window') &&
							this._isProperty(
								callee.property.name ||
									(callee.property as any).raw,
								'chrome'
							);
						if (
							isWindowCall ||
							this._isProperty(callee.object.name, 'chrome')
						) {
							data.expression = callee;
							const expr = this._getFunctionCallExpressions(data);
							const callLines = this._getCallLines(
								data.persistent.lineSeperators,
								expr.start,
								expr.end
							);
							if (data.isReturn && !data.isValidReturn) {
								callLines.from.index = this._getLineIndexFromTotalIndex(
									data.persistent.lines,
									callLines.from.line,
									callLines.from.index
								);
								callLines.to.index = this._getLineIndexFromTotalIndex(
									data.persistent.lines,
									callLines.to.line,
									callLines.to.index
								);
						...
						return this._callsChromeFunction(
							callee.object as any,
							data,
							onError
						);
					}
				...
				private static _removeObjLink(
					data: ChromePersistentData
				): ChromePersistentData {
				...
				private static _findChromeExpression(
					...
					switch (expression.type) {
						case 'VariableDeclaration':
							data.isValidReturn =
								expression.declarations.length === 1;
							for (
								let i = 0;
								i < expression.declarations.length;
								i++
				...
				private static _generateOnError(
					container: TransferOnErrorError[][]
				...
				private static _replaceChromeCalls(
					lines: string[],
					passes: number,
					onError: TransferOnError
				): string {
					...
					for (let i = 0; i < scriptExpressions.length; i++) {
						...
						if (
							...
							script = this._replaceChromeCalls(
				...
				private static _removePositionDuplicates(
					arr: TransferOnErrorError[]
				...
				static replace(
					...
					if (lineIndex !== -1) {
						...
						if (
							lineIndex === script.indexOf('/*execute locally*/')
						) {
							script = script.replace('/*execute locally*/', '');
						}
					}
					...
					if (finalPassErrors) {
						onError(
							this._removePositionDuplicates(firstPassErrors),
			...
			static generateScriptUpgradeErrorHandler(
				id: CRM.GenericNodeId
			): ScriptUpgradeErrorHandler {
				return function (oldScriptErrors, newScriptErrors, parseError) {
					browserAPI.storage.local
						...
						.then((keys) => {
							...
							keys.upgradeErrors[
								id
							] = window.app.storageLocal.upgradeErrors[id] = {
								oldScript: oldScriptErrors,
								newScript: newScriptErrors,
								generalError: parseError,
							};
							browserAPI.storage.local.set({
								upgradeErrors: keys.upgradeErrors,
							} as any);
						});
				};
			}
			static convertScriptFromLegacy(
				script: string,
				id: CRM.GenericNodeId,
				method: SCRIPT_CONVERSION_TYPE
			): string {
				...
				try {
					switch (method) {
						case SCRIPT_CONVERSION_TYPE.CHROME:
							script = this.chromeCallsReplace.replace(
								script,
						...
						case SCRIPT_CONVERSION_TYPE.LOCAL_STORAGE:
							script = usedExecuteLocally
								? this.localStorageReplace.replaceCalls(
										script.split('\n')
								  )
								: script;
							break;
		...
		static listeners = class CRMAppListeners {
			...
			static toggleToolsRibbon() {
				const horizontalCenterer =
					window.crmEditPage.$.horizontalCenterer;
				const bcr = horizontalCenterer.getBoundingClientRect();
				const viewportWidth = bcr.width + 20;

				$(window.doc.editorToolsRibbonContainer).animate(
					{
						marginLeft: window.app.storageLocal.hideToolsRibbon
							? '0'
							: '-200px',
					},
					{
						duration: 250,
						easing: ($ as JQueryContextMenu).bez([
							0.215,
							0.61,
							0.355,
							1.0,
						...
						step: (now: number) => {
							window.doc.fullscreenEditorEditor.style.width = `${
								viewportWidth - 200 - now
							}px`;
							window.doc.fullscreenEditorEditor.style.marginLeft = `${
								now + 200
							}px`;
							(window.scriptEdit || window.scriptEdit)
								.getEditorInstance()
								.editor.layout();
						},
					}
				);
				window.app.storageLocal.hideToolsRibbon = !window.app
					.storageLocal.hideToolsRibbon;
				window.app.upload();
			}
			...
			static launchSearchWebsiteToolScript() {
				if (
					this.parent().item &&
					this.parent().item.type === 'script' &&
					window.scriptEdit
				) {
					const paperSearchWebsiteDialog = this.parent().$
			...
			static launchExternalEditorDialog() {
				if (
					!(window.doc.externalEditorDialogTrigger as HTMLElement & {
						disabled: boolean;
					}).disabled
				) {
					window.externalEditor.init();
					window.externalEditor.editingCRMItem = window.codeEditBehavior.getActive()
						.item as any;
			...
			static async showManagePermissions() {
				if (browserAPI.permissions) {
					...
					window.app.util.showToast(
						this.parent().___(
							I18NKeys.crmApp.code.permissionsNotSupported
						)
					);
				}
			}
			...
			static iconSwitch(
				...
				if (Array.isArray(types)) {
					...
					const element = this.parent().util.findElementWithClassName(
						e,
						'crmType'
					);
					const crmTypes = this.parent().shadowRoot.querySelectorAll(
			...
			static async _generateRegexFile() {
				const filePath = this.parent()
					.$.URISchemeFilePath.$$('input')
					.value.replace(/\\/g, '\\\\');
				const schemeName = this.parent().$.URISchemeSchemeName.$$(
					'input'
				).value;
			...
			static globalExcludeChange(e: Polymer.ClickEvent) {
				const input = this.parent().util.findElementWithTagname(
					e,
					'paper-input'
				);

				let excludeIndex = null;
				const allExcludes = document.getElementsByClassName(
			...
			static removeGlobalExclude(e: Polymer.ClickEvent) {
				const node = this.parent().util.findElementWithTagname(
					e,
					'paper-icon-button'
				);
			...
			static async importData() {
				const dataString = this.parent().$.importSettingsInput
					.value as EncodedString<{
					local?: CRM.StorageLocal;
				...
				if (!this.parent().$.oldCRMImport.checked) {
					...
					try {
						...
						this.parent().$.importSettingsError.style.display =
							'none';
					} catch (e) {
						this.parent().$.importSettingsError.style.display =
							'block';
					...
					if (
						overWriteImport.checked &&
						(data.local || data.storageLocal)
					) {
						this.parent().settings =
							data.nonLocal || this.parent().settings;
						this.parent().storageLocal =
							data.local || this.parent().storageLocal;
					}
					if (data.crm) {
						if (overWriteImport.checked) {
							this.parent().settings.crm = this.parent().util.crmForEach(
								data.crm,
								(node) => {
					...
					//Apply settings
					this.parent()._setup.initCheckboxes(
						this.parent().storageLocal
					);
					this.parent().upload();
				} else {
					try {
						...
						if (settingsArr[0] === 'all') {
							this.parent().storageLocal.showOptions =
								settingsArr[2];

							const rows = settingsArr.slice(6);
							class LocalStorageWrapper {
								getItem(
									index: 'numberofrows' | number
								): string {
							...
							const crm = await this.parent()._transferCRMFromOld(
								settingsArr[4],
							...
							alert(
								'This method of importing no longer works, please export all your settings instead'
							);
						}
					} catch (e) {
						this.parent().$.importSettingsError.style.display =
				...
				this.parent().util.showToast(
					this.parent().___(I18NKeys.crmApp.code.importSuccess)
				);
			}
			...
			static exportData() {
				...
				if (this.parent().$.exportCRM.checked) {
					toExport.crm = JSON.parse(
						JSON.stringify(this.parent().settings.crm)
					);
					for (let i = 0; i < toExport.crm.length; i++) {
						toExport.crm[i] = this.parent().editCRM.makeNodeSafe(
							toExport.crm[i] as CRM.Node
						);
					}
				}
				if (this.parent().$.exportSettings.checked) {
					toExport.local = this.parent().storageLocal;
					toExport.nonLocal = JSON.parse(
						JSON.stringify(this.parent().settings)
					);
					delete toExport.nonLocal.crm;
				}
				window.doc.exportSettingsSpinner.hide = false;
				window.setTimeout(() => {
					this.parent().$.exportSettingsOutput.value = JSON.stringify(
						toExport
					);
			...
			static nextUpdatedScript() {
				...
				if (
					this.parent().$.scriptUpdatesToast.scripts.length - index >
					1
				) {
					this.parent().$.nextScriptUpdateButton.style.display =
						'inline';
				} else {
					this.parent().$.nextScriptUpdateButton.style.display =
						'none';
			...
			private static _copyFromElement(
				target: HTMLTextAreaElement,
				button: HTMLPaperIconButtonElement
			) {
			...
			static copyExportDialogToClipboard() {
				this._copyFromElement(
					this.parent().$.exportJSONData,
			...
			static copyExportToClipboard() {
				this._copyFromElement(
					this.parent().$.exportSettingsOutput,
			...
			static goNextVersionUpdateTab() {
				if (this.parent().versionUpdateTab === 4) {
					...
					const currentHeight = tabCont.getBoundingClientRect()
						.height;
			...
			private static _applyAddedPermissions() {
				const panels = Array.prototype.slice.apply(
					window.doc.addedPermissionsTabContainer.querySelectorAll(
						'.nodeAddedPermissionsCont'
					)
				);
				panels.forEach((panel: HTMLElement) => {
					const node = this.parent().nodesById.get(
						~~(panel.getAttribute(
							'data-id'
						) as any) as CRM.GenericNodeId
					) as CRM.ScriptNode;
					const permissions = Array.prototype.slice
						.apply(panel.querySelectorAll('paper-checkbox'))
					...
					permissions.forEach(function (
						addedPermission: CRM.Permission
					) {
			...
			static addedPermissionNext() {
				const cont = window.doc
					.addedPermissionsTabContainer as AddedPermissionsTabContainer;
				...
				Array.prototype.slice
					...
					.forEach((element: HTMLElement) => {
						...
						const type = element.getAttribute(
							'data-type'
						) as CRM.OptionsValue['type'];
						const currentVal = (this.parent().$.codeSettingsDialog
							.item.value.options as CRM.Options)[key];
						switch (type) {
							case 'number':
							...
							case 'color':
								value = this.parent().templates.mergeObjects(
									currentVal,
									{
										value: element.querySelector('input')
											.value,
									}
								);
								break;
							...
							case 'choice':
								...
								break;
							case 'array':
								const arrayInput = element.querySelector(
									'paper-array-input'
								);
								arrayInput.saveSettings();
								let values = arrayInput.values;
								if (
									(currentVal as CRM.OptionArray).items ===
									'string'
								) {
									//Strings
		...
		static templates = class CRMAppTemplates {
				...
				for (let i = 0; i < additionArray.length; i++) {
					if (
						...
						if (Array.isArray(additionArray[i])) {
							mainArray[i] = this.mergeArrays<T, U>(
								mainArray[i] as T,
				...
				for (let key in additions) {
					if (additions.hasOwnProperty(key)) {
						if (
							...
							(mainObject as any)[key] = (additions[
								key
							] as any) as T[keyof T];
						}
					}
			...
			static mergeArraysWithoutAssignment<T extends T[] | U[], U>(
				mainArray: T,
				additionArray: T
			) {
				for (let i = 0; i < additionArray.length; i++) {
					if (
						...
						if (Array.isArray(additionArray[i])) {
							this.mergeArraysWithoutAssignment<T, U>(
								mainArray[i] as T,
								additionArray[i] as T
							);
						} else {
							this.mergeObjectsWithoutAssignment(
								mainArray[i],
								additionArray[i]
							);
						}
			...
			static mergeObjectsWithoutAssignment<
				T extends {
					[key: string]: any;
					[key: number]: any;
				},
				Y extends Partial<T>
			>(mainObject: T, additions: Y) {
				for (let key in additions) {
					if (additions.hasOwnProperty(key)) {
						if (
							typeof additions[key] === 'object' &&
							...
							if (Array.isArray(additions[key])) {
								this.mergeArraysWithoutAssignment(
									mainObject[key],
									(additions as any)[key]
								);
							} else {
			 ...
			 */
			static getDefaultStylesheetValue(
			 ...
			 */
			static getDefaultStylesheetNode(
			 ...
			 */
			static getDefaultDividerOrMenuNode(
			...
			static getDefaultDividerOrMenuNode(
				...
				const defaultNode: Partial<CRM.PassiveNode> = {
					name:
						type === 'menu'
							...
							: this.parent().___(
									I18NKeys.crm.exampleDividerName
							  ),
					type: type,
					nodeInfo: this.getDefaultNodeInfo(options.nodeInfo),
					onContentTypes: [true, true, true, false, false, false],
				...
				return this.mergeObjects(defaultNode, options) as
					| CRM.DividerNode
					| CRM.MenuNode;
			}
			 ...
			 */
			static getDefaultNodeOfType(
				type: CRM.NodeType,
				options: Partial<CRM.Node> = {}
			): CRM.Node {
				switch (type) {
					case 'link':
						return this.getDefaultLinkNode(
							options as Partial<CRM.LinkNode>
						);
					case 'script':
						return this.getDefaultScriptNode(
							options as Partial<CRM.ScriptNode>
						);
					case 'divider':
						return this.getDefaultDividerNode(
							options as Partial<CRM.DividerNode>
						);
					case 'menu':
						return this.getDefaultMenuNode(
							options as Partial<CRM.MenuNode>
						);
					case 'stylesheet':
						return this.getDefaultStylesheetNode(
							options as Partial<CRM.StylesheetNode>
						);
				}
			}
			...
			static getPermissionDescription(
				permission: CRM.Permission
			): string {
				const descriptions = {
					alarms: this.parent().___(I18NKeys.permissions.alarms),
					activeTab: this.parent().___(
						I18NKeys.permissions.activeTab
					),
					background: this.parent().___(
						I18NKeys.permissions.background
					),
					bookmarks: this.parent().___(
						I18NKeys.permissions.bookmarks
					),
					browsingData: this.parent().___(
						I18NKeys.permissions.browsingData
					),
					clipboardRead: this.parent().___(
						I18NKeys.permissions.clipboardRead
					),
					clipboardWrite: this.parent().___(
						I18NKeys.permissions.clipboardWrite
					),
					cookies: this.parent().___(I18NKeys.permissions.cookies),
					contentSettings: this.parent().___(
						I18NKeys.permissions.contentSettings
					),
					contextMenus: this.parent().___(
						I18NKeys.permissions.contextMenus
					),
					declarativeContent: this.parent().___(
						I18NKeys.permissions.declarativeContent
					),
					desktopCapture: this.parent().___(
						I18NKeys.permissions.desktopCapture
					),
					downloads: this.parent().___(
						I18NKeys.permissions.downloads
					),
					...
					management: this.parent().___(
						I18NKeys.permissions.management
					),
					notifications: this.parent().___(
						I18NKeys.permissions.notifications
					),
					pageCapture: this.parent().___(
						I18NKeys.permissions.pageCapture
					),
					power: this.parent().___(I18NKeys.permissions.power),
					privacy: this.parent().___(I18NKeys.permissions.privacy),
					printerProvider: this.parent().___(
						I18NKeys.permissions.printerProvider
					),
					sessions: this.parent().___(I18NKeys.permissions.sessions),
					'system.cpu': this.parent().___(
						I18NKeys.permissions.systemcpu
					),
					'system.memory': this.parent().___(
						I18NKeys.permissions.systemmemory
					),
					'system.storage': this.parent().___(
						I18NKeys.permissions.systemstorage
					),
					topSites: this.parent().___(I18NKeys.permissions.topSites),
					tabCapture: this.parent().___(
						I18NKeys.permissions.tabCapture
					),
					tabs: this.parent().___(I18NKeys.permissions.tabs),
					tts: this.parent().___(I18NKeys.permissions.tts),
					webNavigation:
						this.parent().___(I18NKeys.permissions.webNavigation) +
						' (https://developer.chrome.com/extensions/webNavigation)',
					webRequest: this.parent().___(
						I18NKeys.permissions.webRequest
					),
					webRequestBlocking: this.parent().___(
						I18NKeys.permissions.webRequestBlocking
					),
					...
					crmContextmenu: this.parent().___(
						I18NKeys.permissions.crmContextmenu
					),
					chrome: this.parent().___(I18NKeys.permissions.chrome),
					browser: this.parent().___(I18NKeys.permissions.browser),

					//Tampermonkey APIs
					GM_addStyle: this.parent().___(
						I18NKeys.permissions.GMAddStyle
					),
					GM_deleteValue: this.parent().___(
						I18NKeys.permissions.GMDeleteValue
					),
					GM_listValues: this.parent().___(
						I18NKeys.permissions.GMListValues
					),
					GM_addValueChangeListener: this.parent().___(
						I18NKeys.permissions.GMAddValueChangeListener
					),
					GM_removeValueChangeListener: this.parent().___(
						I18NKeys.permissions.GMRemoveValueChangeListener
					),
					GM_setValue: this.parent().___(
						I18NKeys.permissions.GMSetValue
					),
					GM_getValue: this.parent().___(
						I18NKeys.permissions.GMGetValue
					),
					GM_log: this.parent().___(I18NKeys.permissions.GMLog),
					GM_getResourceText: this.parent().___(
						I18NKeys.permissions.GMGetResourceText
					),
					GM_getResourceURL: this.parent().___(
						I18NKeys.permissions.GMGetResourceURL
					),
					GM_registerMenuCommand: this.parent().___(
						I18NKeys.permissions.GMRegisterMenuCommand
					),
					GM_unregisterMenuCommand: this.parent().___(
						I18NKeys.permissions.GMUnregisterMenuCommand
					),
					GM_openInTab: this.parent().___(
						I18NKeys.permissions.GMOpenInTab
					),
					GM_xmlhttpRequest: this.parent().___(
						I18NKeys.permissions.GMXmlhttpRequest
					),
					GM_download: this.parent().___(
						I18NKeys.permissions.GMDownload
					),
					GM_getTab: this.parent().___(I18NKeys.permissions.GMGetTab),
					GM_saveTab: this.parent().___(
						I18NKeys.permissions.GMSaveTab
					),
					GM_getTabs: this.parent().___(
						I18NKeys.permissions.GMGetTabs
					),
					GM_notification: this.parent().___(
						I18NKeys.permissions.GMNotification
					),
					GM_setClipboard: this.parent().___(
						I18NKeys.permissions.GMSetClipboard
					),
					GM_info: this.parent().___(I18NKeys.permissions.GMInfo),
					unsafeWindow: this.parent().___(
						I18NKeys.permissions.unsafeWindow
					),
				};
		...
		static crm = class CRMAppCRMFunctions {
			...
			static lookup(
				...
				if (path.length === 1) {
					return returnArray
						...
						: window.app.settings.crm[path[0]];
				}
				...
				for (let i = 0; i < path.length; i++) {
					parent = currentTree;
					if (i !== path.length - 1) {
						currentTree = currentTree[path[i]]
							.children as CRM.Node[];
					} else {
			 ...
			 */
			static add<T extends CRM.Node>(
				value: T,
				position: string = 'last'
			) {
				if (position === 'first') {
					this._parent().settings.crm = this._parent().util.insertInto(
						value,
						this._parent().settings.crm,
						0
					);
				} else if (position === 'last' || position === undefined) {
					this._parent().settings.crm[
						this._parent().settings.crm.length
					] = value;
				} else {
					this._parent().settings.crm = this._parent().util.insertInto(
						value,
			...
			static move(toMove: number[], target: number[]) {
				...
				for (
					let i = 1;
					i <= window.app.editCRM.setMenus.length - 1;
					i++
				) {
					const lookedup = this.lookup(
						window.app.editCRM.setMenus.slice(0, i),
						false
					);
		...
		static util = class CRMAppUtil {
			...
			static xhr(path: string, local: boolean): Promise<string> {
				return new Promise<string>((resolve, reject) => {
					...
					xhr.open(
						'GET',
						local ? browserAPI.runtime.getURL(path) : path
					);
			...
			static createElement<
				K extends keyof ElementTagNameMaps,
				T extends ElementTagNameMaps[K]
			>(
				tagName: K,
				options: {
					id?: string;
			...
			static createSVG<
				...
				T extends SVGElementTagNameMap[K]
			>(
				tag: K,
				options: {
					id?: string;
			...
			private static _generatePathFrom(
				element: HTMLElement
			): HTMLElement[] {
				const path = [];
				while (element) {
			...
			static findElementWithTagname<T extends keyof ElementTagNameMaps>(
				event:
					| {
							path: HTMLElement[];
					  }
					...
					| Polymer.CustomEvent,
				tagName: T
			): ElementTagNameMaps[T] {
				return this.iteratePath(event, (node) => {
					if (
						node &&
						'tagName' in node &&
						(node as Polymer.PolymerElement).tagName.toLowerCase() ===
							tagName
					) {
						return node;
					}
			...
			static findElementWithClassName(
				event:
					| {
							path: HTMLElement[];
					  }
				...
				return this.iteratePath(event, (node) => {
					if (
						node &&
						'classList' in node &&
						(node as Polymer.PolymerElement).classList.contains(
							className
						)
					) {
						return node;
				...
				for (i = 0; i < firstLength; i++) {
					if (typeof firstArray[i] === 'object') {
						...
						if (Array.isArray(firstArray[i])) {
							...
							if (
								!this.compareObj(firstArray[i], secondArray[i])
							) {
								return false;
							}
						}
		...
		static pageDemo = class CRMAppPageDemo {
			...
			private static _setContentTypeClasses(
				el: HTMLElement,
				node: CRM.Node
			) {
			...
			private static _editNodeFromClick(node: CRM.Node) {
				if (window.app.item) {
					window.app.$.messageToast.text = this.parent().___(
						I18NKeys.crmApp.code.alreadyEditingNode
					);
					window.app.$.messageToast.show();
				} else {
					const elements = window.app.editCRM.shadowRoot.querySelectorAll(
						'edit-crm-item'
					);
			...
			private static _genLinkNode(node: CRM.LinkNode) {
				return this.parent().util.createElement(
					'div',
					{
					...
					[
						this.parent().util.createElement(
							'div',
							{
					...
					'div',
					{
						...
						onclick: () => {
							if (window.app.storageLocal.editCRMInRM) {
								...
								window.app.$.messageToast.text = this.parent().___(
									I18NKeys.crmApp.code.wouldExecuteScript
								);
								window.app.$.messageToast.show();
							}
						},
				...
				return this.parent().util.createElement(
					'div',
					{
						...
						onclick: () => {
							if (window.app.storageLocal.editCRMInRM) {
								...
								thisEl.parentElement.classList.add(
									'forcedVisible'
								);

								timer && window.clearTimeout(timer);
								timer = window.setTimeout(() => {
									thisEl.parentElement.classList.remove(
										'forcedVisible'
									);
						...
						onhover(_el, e) {
							if (
								!thisEl.parentElement.classList.contains(
									'forcedVisible'
								)
							) {
					...
					[
						this.parent().util.createElement(
							...
							[
								...
								this.parent().util.createElement(
									...
									[
										this.parent().util.createSVG(
											'svg',
											{
												classes: [
													'contextMenuMenuArrowImage',
												],
												props: {
													width: '48',
													height: '48',
						...
						this.parent().util.createElement(
							...
							{
								classes: [
									'contextMenuMenuSubmenu',
									'contextMenuMenuChildren',
									'hidden',
								],
								ref(el) {
			...
			private static _setAllContentTypeClasses(
				el: HTMLElement,
				op: 'add' | 'remove'
			) {
				const arr: [
				...
				el.classList[op](
					...arr.map((_item: any, i: number) => `hidden${i}`)
				);
				el.classList[op]('rootHidden');
			}

			private static _setMenuPosition(
				menu: HTMLElement,
				e: Polymer.ClickEvent
			) {
	...
	export type CrmApp = Polymer.El<
		'crm-app',
		typeof CA &
			typeof crmAppProperties & {
				editCRM: EditCrm;
			}
	>;
