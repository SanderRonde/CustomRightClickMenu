...
namespace StylesheetEditElement {
	...
	export class STE implements I18NClass {
		...
		private static _getExportData(
			this: NodeEditBehaviorStylesheetInstance
		): CRM.StylesheetNode {
			const settings = {};
		...
		static exportStylesheetAsUserscript(
			this: NodeEditBehaviorStylesheetInstance
		) {
		...
		static exportStylesheetAsUserstyle(
			this: NodeEditBehaviorStylesheetInstance
		) {
			window.app.editCRM.exportSingleNode(
				this._getExportData(),
		...
		static saveChanges(
			this: NodeEditBehaviorStylesheetInstance,
			resultStorage: Partial<CRM.StylesheetNode>
		) {
			resultStorage.value.stylesheet =
				(this.editorManager &&
					this.editorManager.editor &&
					this.editorManager.getValue()) ||
				this.item.value.stylesheet;
		...
		static editorLoaded(this: NodeEditBehaviorStylesheetInstance) {
			...
			(editorManager.getTypeHandler() as any)[0].listen(
				'metaChange',
				(
					{ content: oldTags }: MetaBlock,
					{ content: newTags }: MetaBlock
				) => {
					if (this.editorMode === 'main') {
						const oldPreprocessor =
							oldTags['preprocessor'] &&
							oldTags['preprocessor'][0];
						const newPreprocessor =
							newTags['preprocessor'] &&
							newTags['preprocessor'][0];
						if (
							(oldPreprocessor !== newPreprocessor &&
								(oldPreprocessor === 'less' ||
									oldPreprocessor === 'stylus') &&
								newPreprocessor !== 'less' &&
								newPreprocessor !== 'stylus') ||
							((newPreprocessor === 'less' ||
								newPreprocessor === 'stylus') &&
								oldPreprocessor !== 'less' &&
								oldPreprocessor !== 'stylus')
		...
		private static _getPreprocessor(
			this: NodeEditBehaviorStylesheetInstance,
			stylesheet: string
		) {
			const tags = window.app.editCRM.getMetaTags(stylesheet);
			return (
				(tags['preprocessor'] && tags['preprocessor'][0]) || 'default'
			);
		}
		 ...
		 */
		private static async _loadEditor(
			this: NodeEditBehaviorStylesheetInstance,
			content: string = this.item.value.stylesheet
		) {
			...
			const editorType =
				preprocessor === 'stylus' || preprocessor === 'less'
					? this.$.editor.EditorMode.LESS_META
					: this.$.editor.EditorMode.CSS_META;
			this.$.editorStylusInfo.classList[
				preprocessor === 'stylus' ? 'remove' : 'add'
			]('hidden');
		...
		private static _showMainTab(this: NodeEditBehaviorStylesheetInstance) {
			this.editorManager.switchToModel(
				...
				this.editorManager.EditorMode.CSS_META
			);
		}

		private static _parseVar(
			this: NodeEditBehaviorStylesheetInstance,
			value: string
		) {
			...
			lastLabelChar =
				type.length + 1 + name.length + 1 + label.length + 2;
		...
		private static _codeOptionTypeToMetaTagVarType(
			type: CRM.OptionsValue['type']
		) {
			switch (type) {
				case 'number':
				case 'string':
		...
		private static _metaTagVarsToCodeOptions(
			this: NodeEditBehaviorStylesheetInstance,
			stylesheet: string,
			...
			if (vars.length === 0) {
				...
				vars.forEach((value: string) => {
					const { type, name, label, defaultValue } = this._parseVar(
						value
					);
					const descriptor = window.app.templates.mergeObjects(
						options[name] || {},
						{
						...
						case 'select':
							try {
								...
								if (Array.isArray(defaultValue)) {
									obj[
										name
									] = window.app.templates.mergeObjects(
										descriptor,
										{
											values: defaultValue.map(
												(value) => {
													if (
														value.indexOf(':') > -1
													) {
								...
								obj[name] = window.app.templates.mergeObjects(
									descriptor,
									{
										values: [],
										selected: 0,
		...
		private static _codeOptionsToMetaTagVars(
			this: NodeEditBehaviorStylesheetInstance,
			options: CRM.Options | string
		) {
			if (typeof options === 'string') {
			...
			return Object.getOwnPropertyNames(options)
				.map((key) => {
					...
					const type = this._codeOptionTypeToMetaTagVarType(
						option.type
					);
					if (!type) {
						return null;
		...
		static changeTabEvent(
			...
			if (mainClicked && this.editorMode !== 'main') {
				this.$.editorStylusInfo.classList[
					this._getPreprocessor(this.newSettings.value.stylesheet) ===
					'stylus'
						? 'remove'
						: 'add'
				]('hidden');
				...
				if (window.app.editCRM.getMetaLines(stylesheet).length > 0) {
					const metaIndexes = window.app.editCRM.getMetaIndexes(
						stylesheet
					);
					...
					const metaLines = [
						...window.app.editCRM
							.getMetaLinesForIndex(stylesheet, lastIndex)
							.filter((line) => {
								return (
									line.indexOf('@var') === -1 &&
			...
			Array.prototype.slice
				.apply(
					window.stylesheetEdit.shadowRoot.querySelectorAll(
						'.editorTab'
					)
				)
...
export type StylesheetEdit = Polymer.El<
	'stylesheet-edit',
	typeof StylesheetEditElement.STE &
