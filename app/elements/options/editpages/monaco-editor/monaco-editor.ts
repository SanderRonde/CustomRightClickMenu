...
export type MonacoEditorScriptMetaMods = MonacoEditorElement.MonacoEditorScriptMetaMods;
export type MonacoEditorHookManager = typeof MonacoEditorElement.MonacoEditorHookManager;
export type MonacoEditorTSLibrariesMetaMods = MonacoEditorElement.MonacoEditorTSLibrariesMetaMods;
...
namespace MonacoEditorElement {
	function getMetaDescriptions() {
		return {
			...
			namespace: window.__.sync(
				I18NKeys.options.editPages.metadata.namespace
			),
			version: window.__.sync(
				I18NKeys.options.editPages.metadata.version
			),
			author: window.__.sync(I18NKeys.options.editPages.metadata.author),
			description: window.__.sync(
				I18NKeys.options.editPages.metadata.description
			),
			homepage: window.__.sync(
				I18NKeys.options.editPages.metadata.homepage
			),
			homepageURL: window.__.sync(
				I18NKeys.options.editPages.metadata.homepageURL
			),
			website: window.__.sync(
				I18NKeys.options.editPages.metadata.website
			),
			...
			iconURL: window.__.sync(
				I18NKeys.options.editPages.metadata.iconURL
			),
			defaulticon: window.__.sync(
				I18NKeys.options.editPages.metadata.defaulticon
			),
			icon64: window.__.sync(I18NKeys.options.editPages.metadata.icon64),
			icon64URL: window.__.sync(
				I18NKeys.options.editPages.metadata.icon64URL
			),
			updateURL: window.__.sync(
				I18NKeys.options.editPages.metadata.updateURL
			),
			downloadURL: window.__.sync(
				I18NKeys.options.editPages.metadata.downloadURL
			),
			supportURL: window.__.sync(
				I18NKeys.options.editPages.metadata.supportURL
			),
			include: window.__.sync(
				I18NKeys.options.editPages.metadata.include
			),
			match: window.__.sync(I18NKeys.options.editPages.metadata.match),
			exclude: window.__.sync(
				I18NKeys.options.editPages.metadata.exclude
			),
			require: window.__.sync(
				I18NKeys.options.editPages.metadata.require
			),
			resource: window.__.sync(
				I18NKeys.options.editPages.metadata.resource
			),
			connect: window.__.sync(
				I18NKeys.options.editPages.metadata.connect
			),
			...
			noframes: window.__.sync(
				I18NKeys.options.editPages.metadata.noframes
			),
			CRM_contentTypes: window.__.sync(
				I18NKeys.options.editPages.metadata.CRMContentTypes
			),
			CRM_launchMode: window.__.sync(
				I18NKeys.options.editPages.metadata.CRMLaunchMode
			),
			CRM_stylesheet: window.__.sync(
				I18NKeys.options.editPages.metadata.CRMStylesheet
			),
			CRM_toggle: window.__.sync(
				I18NKeys.options.editPages.metadata.CRMToggle
			),
			CRM_defaultOn: window.__.sync(
				I18NKeys.options.editPages.metadata.CRMDefaultOn
			),
			CRM_libraries: window.__.sync(
				I18NKeys.options.editPages.metadata.CRMLibraries
			),
			license: window.__.sync(
				I18NKeys.options.editPages.metadata.license
			),
			preprocessor: window.__.sync(
				I18NKeys.options.editPages.metadata.preprocessor
			),
	...
	abstract class EventEmitter<PubL extends string, PriL extends string> {
		...
		private _insertOnce<T extends (...args: any[]) => any>(
			arr: T[],
			value: T
		) {
			const self = ((...args: any[]) => {
		...
		protected _listen<E extends PriL>(
			event: E,
		...
		protected _firePrivate<R, E extends PriL = PriL>(
			event: E,
			params: any[]
		): R[] {
			return !(event in this._privateListenerMap)
				? []
				: this._privateListenerMap[event].map((listener) => {
						return listener(...params);
				  });
		}

		protected _firePublic<R, E extends PubL>(event: E, params: any[]): R[] {
			return !(event in this._publicListenerMap)
				? []
				: this._publicListenerMap[event].map((listener) => {
						return listener(...params);
				  });
		}
	}
	...
	abstract class MonacoTypeHandler<
		PubL extends string = '_',
		PriL extends string = '_'
	> extends EventEmitter<PubL, PriL> {
		/**
	...
	abstract class MonacoEditorEventHandler<
		PubL extends string = '_',
		PriL extends string = '_'
	> extends MonacoTypeHandler<
		PubL,
		PriL | 'onLoad' | 'onModelContentChange'
	> {
		/**
		...
		protected static _genDisposable<T>(
			fn: () => T,
			onDispose: (args: T) => void
		) {
			const res = fn();
			return {
				dispose: () => {
	...
	abstract class MonacoEditorMetaBlockMods<
		PubL extends string = '_',
		PriL extends string = '_'
	> extends MonacoEditorEventHandler<
		PubL | 'metaChange',
		PriL | 'decorate' | 'shouldDecorate'
	> {
		/**
					...
					suggestions: [
						{
							...
							documentation: window.__.sync(
								I18NKeys.options.editPages.monaco
									.startTagUserscript
							),
							range,
						},
		...
		private static _containsPosition(
			metablock: MetaBlock,
		...
		protected static readonly _metaKeyProvider: monaco.languages.CompletionItemProvider = {
			provideCompletionItems: (model, position) => {
				...
				if (
					!metaBlock ||
					MonacoEditorMetaBlockMods._containsPosition(
						metaBlock,
						position
					)
				) {
					...
					return {
						incomplete: true,
						suggestions: Object.getOwnPropertyNames(
							descriptions
						).map((key: keyof typeof descriptions) => {
		...
		private _attachListeners() {
			...
			this._listen(
				...
				(changeEvent: monaco.editor.IModelContentChangedEvent) => {
					...
					if (
						!force &&
						(!changeEvent || !changeEvent.isRedoing) &&
						(!changeEvent ||
							changeEvent.changes.filter((change) => {
			...
			if (window.app) {
				this._isMetaDataHighlightDisabled =
					window.app.settings.editor.disabledMetaDataHighlight;
			} else {
				this._isMetaDataHighlightDisabled =
					window.installPage.settings.editor.disabledMetaDataHighlight;
			}
				...
				this._disposables.push(
					this._editor.addAction({
						...
						label: window.__.sync(
							I18NKeys.options.editPages.monaco.disableMeta
						),
						run: () => {
			...
			if (!this._isDiff(this._editor)) {
				this._disposables.push(
					this._editor.addAction({
						...
						label: window.__.sync(
							I18NKeys.options.editPages.monaco.enableMeta
						),
						run: () => {
		...
		private _getMetaOutlines(): {
			...
			if (
				(editorContent.indexOf(
					MonacoEditorMetaBlockMods._userScriptStart
				) === -1 ||
					editorContent.indexOf(
						MonacoEditorMetaBlockMods._userScriptEnd
					) === -1) &&
				(editorContent.indexOf(
					MonacoEditorMetaBlockMods._userStyleStart
				) === -1 ||
					editorContent.indexOf(
						MonacoEditorMetaBlockMods._userStyleEnd
					) === -1)
			) {
			...
			for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
				...
				if (
					line.indexOf(MonacoEditorMetaBlockMods._userScriptStart) !==
						-1 ||
					line.indexOf(MonacoEditorMetaBlockMods._userStyleStart) !==
						-1
				) {
				...
				if (
					line.indexOf(MonacoEditorMetaBlockMods._userScriptEnd) !==
						-1 ||
					...
					if (!state.end) {
						state.end = new monaco.Position(
							lineIndex + 1,
							index +
								MonacoEditorMetaBlockMods._userScriptEnd.length
						);
					}
					indexes.push({
						start: state.start,
		...
		private _isDifferent(prev: MetaBlock, current: MetaBlock): boolean {
			...
			for (let i = 0; i < prev.indexes.length; i++) {
				...
				if (
					!prevIndex.start.equals(currentIndex.start) ||
					!prevIndex.end.equals(currentIndex.end)
				) {
					return false;
				}
			}
		...
		private _getKeyDescription(metaKey: string) {
			...
			if (metaKey in descriptions) {
				return window.__.sync(
					I18NKeys.options.editPages.monaco.keyDescription,
					metaKey,
					descriptions[metaKey as keyof typeof descriptions]
				);
			}
			return window.__.sync(
				I18NKeys.options.editPages.monaco.keyDescriptionUnknown,
				metaKey
			);
		}
				...
				if (match) {
					...
					if (value) {
						const valueStartIndex =
							line.slice(keyEnd).indexOf(value) + keyEnd;
		...
		private _shouldUpdateDecorations(
			...
			const results = this._firePrivate<boolean>('shouldDecorate', [
				changeEvent,
			]);
			if (results.length > 1) {
		...
		private _doModelUpdate() {
			const decorations = this._firePrivate<
				monaco.editor.IModelDeltaDecoration[]
			>('decorate', []).filter((decorationArr) => decorationArr !== null);

			this._doDecorationUpdate(this._formatDecorations(decorations));
		}
	}

	export class MonacoEditorScriptMetaMods<
		PubL extends string = '_',
		PriL extends string = '_'
	> extends MonacoEditorMetaBlockMods<PubL, PriL> {
		...
		constructor(
			...
			MonacoEditorHookManager.Completion.register(
				'javascript',
	...
	> extends MonacoEditorMetaBlockMods<PubL, PriL> {
		...
		constructor(
				...
				(event: monaco.editor.IModelContentChangedEvent) => {
					...
					for (let change of event.changes) {
						...
						for (let line of lines) {
							...
							const lineContent = this._model.getLineContent(
								line
							);
							const cssRuleParts = this._getCssRuleParts(
								lineContent
							);
			...
			this._disposables.push(
				MonacoEditorEventHandler._genDisposable(
					() => {
			...
			if (window.app) {
				this._underlineDisabled =
					window.app.settings.editor.cssUnderlineDisabled;
			} else {
				this._underlineDisabled =
					window.installPage.settings.editor.cssUnderlineDisabled;
			}
			if (!this._isDiff(this._editor)) {
				this._disposables.push(
					this._editor.addAction({
		...
		private _findColor(
			...
			for (let color of MonacoEditorCSSMetaMods._cssColorNames) {
				...
				if ((index = str.toLowerCase().indexOf(color)) > -1) {
					return {
						pos: new monaco.Range(
							lineNumber + 1,
							index + offset + 1,
							lineNumber + 1,
							index + offset + color.length + 1
						),
						color: color,
					};
				}
		...
		private _markUnderlines() {
			...
			if (this._editor.getModel() === this._model) {
				const underlinables = this._editor
					.getDomNode()
					.querySelectorAll('.userScriptColorUnderline');
				Array.prototype.slice
					.apply(underlinables)
					.forEach((underlineable: HTMLElement) => {
						for (
							let i = 0;
							i < underlineable.classList.length;
							i++
						) {
							if (
								underlineable.classList[i].indexOf('color') ===
								0
							) {
	...
	class MonacoEditorJSONOptionsMods<
		PubL extends string = '_',
		PriL extends string = '_'
	> extends MonacoEditorEventHandler<PubL, PriL> {
		constructor(
			editor:
				| monaco.editor.IStandaloneCodeEditor
				| monaco.editor.IDiffEditor,
			...
			const schema: JSONSchema = {
				...
				additionalProperties: {
					title: window.__.sync(
						I18NKeys.options.editPages.monaco.optionName
					),
					type: 'object',
					oneOf: [
						{
	...
	export class MonacoEditorTSLibrariesMetaMods<
		PubL extends string = '_',
		PriL extends string = '_'
	> extends MonacoEditorScriptMetaMods<PubL, PriL> {
		/**
		...
		private async _enable() {
			this._registrationDisposables = await this._registerLibraries(
				this._libraries
			);
		}
		...
		updateLibraries() {
			this._libraries = this._getLibraries(
				this._node,
				this._isBackground
			);
		}
		...
		private async _registerLibrary(
			library: CRM.Library
		...
		private async _registerLibraries(libraries: CRM.Library[]) {
			return await Promise.all(
				libraries.map((library) => this._registerLibrary(library))
			);
		}
	...
	interface MonacoBaseEditor {
		...
		saveViewState():
			| monaco.editor.IEditorViewState
			| MonacoModel
			| {
	...
	interface MonacoModel {
		getValue(
			eol?: monaco.editor.EndOfLinePreference,
			preserveBOM?: boolean
		): string;
		dispose(): void;
		setValue(value: string): void;
	...
	abstract class TextareaEditorBase implements MonacoBaseEditor {
		...
		protected abstract _swapToModel(
			model:
				| TextareaModel
				| {
						original: TextareaModel;
		...
		private _totalCharIndexToPosition(
			content: string,
			total: number
		): {
			line: number;
		...
		dispose() {
			this._textareaElements &&
				this._textareaElements.forEach((el) => {
					el && el.remove();
				});
			this._textareaElements = [];
			this._models.forEach((model) => model.dispose());
			this._models = [];
			this._baseElements.container &&
		...
		getSelected(): {
			...
			for (const textarea of this._textareaElements) {
				...
				return {
					from: {
						...this._totalCharIndexToPosition(
							textarea.value,
							start
						),
						totalChar: start,
	...
	class TextareaStandardEditor
		extends TextareaEditorBase
		implements MonacoStandardEditor {
	...
	class TextareaDiffEditor
		extends TextareaEditorBase
		implements MonacoDiffEditor {
		protected _model: {
			original: TextareaModel;
			modified: TextareaModel;
		};
		...
		setModel(model: { original: TextareaModel; modified: TextareaModel }) {
	...
	class TextareaModel
		extends EventEmitter<'change', 'none'>
		implements MonacoModel {
	...
	class MOE {
		...
		private static _getTypeHandler(
			editorType: EditorConfig,
			editor:
				| monaco.editor.IStandaloneCodeEditor
				| monaco.editor.IDiffEditor,
			...
			if (typeof editorType === 'object') {
				switch (editorType.mode) {
					case CustomEditorModes.TS_LIBRARIES_META:
						return new MonacoEditorTSLibrariesMetaMods(
							editor,
							model,
							editorType.config
						);
		...
		static initTSLibrariesMode(
			this: MonacoEditor,
			node: CRM.ScriptNode,
			isBackground: boolean
		): CustomEditorModeTSLibrariesMeta {
			return {
				custom: true,
				config: {
					node,
		 ...
		 * Merges two arrays
		 */
		private static _mergeArrays<T extends T[] | U[], U>(
			mainArray: T,
			additionArray: T
		): T {
			for (let i = 0; i < additionArray.length; i++) {
				if (
					...
					if (Array.isArray(additionArray[i])) {
						mainArray[i] = this._mergeArrays<T, U>(
							mainArray[i] as T,
							additionArray[i] as T
						);
					} else {
						mainArray[i] = this._mergeObjects(
							mainArray[i],
							additionArray[i]
						);
					}
				...
				if (additions.hasOwnProperty(key)) {
					if (
						...
						if (Array.isArray(additions[key])) {
							mainObject[key] = this._mergeArrays(
								mainObject[key],
								(additions as any)[key]
							);
						} else {
							mainObject[key] = this._mergeObjects(
								mainObject[key],
								additions[key]
							);
						}
		...
		static async setMonacoEditorScopes<T>(
			this: MonacoEditor,
			getEditor: () => T
		): Promise<T> {
		...
		static async create(
			...
			const model = await this.setMonacoEditorScopes(() => {
				if (this._supportsMonaco()) {
					...
					this.editor = window.monaco.editor.create(
						...
						override
					) as MonacoStandardEditor;
					return model;
				} else {
					const model = new TextareaModel(options.value);
					this.editor = new TextareaStandardEditor(
						this.$.editorElement,
						{
							model,
						}
			...
			const typeHandler = this._getTypeHandler(
				editorType,
				this.editor as any,
				model as any
			);
		...
		static async createDiff(
			...
			await this.setMonacoEditorScopes(() => {
				if (this._supportsMonaco()) {
					...
					this.editor = new TextareaDiffEditor(
						this.$.editorElement,
						{}
					);
				}
			...
			let typeHandlers: [MonacoTypeHandler, MonacoTypeHandler] = [
				this._getTypeHandler(
					editorType,
					this.editor as any,
		...
		static async createFrom(this: MonacoEditor, from: MonacoEditor) {
			...
			this.editor.updateOptions(this._getSettings(editorType));
			let typeHandler = this._getTypeHandler(
				editorType,
				this.editor as any,
				this.editor.getModel() as any
			);
			this._models['default'] = {
				models: [this.editor.getModel() as MonacoModel],
				handlers: [typeHandler],
		...
		static isDiff(
			this: MonacoEditor,
			_editor: MonacoStandardEditor | MonacoDiffEditor
		): _editor is MonacoDiffEditor;
		static isDiff(
			this: MonacoEditor,
			_editor: TextareaStandardEditor | TextareaDiffEditor
		): _editor is TextareaDiffEditor;
		static isDiff(
			this: MonacoEditor,
			_editor:
				| monaco.editor.IStandaloneCodeEditor
				| monaco.editor.IDiffEditor
		): _editor is monaco.editor.IDiffEditor;
		static isDiff(
			this: MonacoEditor,
			_editor:
				| MonacoStandardEditor
				| MonacoDiffEditor
				| TextareaStandardEditor
				| TextareaDiffEditor
				| monaco.editor.IStandaloneCodeEditor
				| monaco.editor.IDiffEditor
		): _editor is MonacoDiffEditor | TextareaDiffEditor {
		...
		static isTextarea(
			...
			_editor:
				...
				| monaco.editor.IStandaloneCodeEditor
				| monaco.editor.IDiffEditor
		): _editor is TextareaStandardEditor | TextareaDiffEditor {
			return '__textarea' in this.editor;
		}

		static getEditorAsMonaco(
			this: MonacoEditor
		):
			| TextareaStandardEditor
			| TextareaDiffEditor
		...
		static setLess(this: MonacoEditor, enabled: boolean) {
			...
			currentModel.handlers = newModels.map((newModel) => {
				return this._getTypeHandler(
					currentModel.editorType,
					this.editor as any,
					newModel
				);
			});
			...
			this._children.forEach((child) => {
				...
				const copiedModelName: string = (child._createInfo as any)
					.modelId;
		...
		static addModel(
			...
			const model = monaco.editor.createModel(
				value,
				this._getLanguage(editorType)
			);
			let handler = this._getTypeHandler(
				editorType,
				this.editor as any,
				model
			);
		...
		private static _runJsLint(this: MonacoEditor): LinterWarning[] {
			...
			const { warnings } = window.jslint(code, {}, [
		...
		private static _showLintResults(
			this: MonacoEditor,
			name: string,
			messages: LinterWarning[]
		) {
			if ('__textarea' in this.editor) {
				return;
			}
			monaco.editor.setModelMarkers(
				this.getCurrentModel().models[0] as monaco.editor.IModel,
				name,
		...
		static async runLinter(this: MonacoEditor) {
			...
			if (this._typeIsJS(type)) {
				await MonacoEditorHookManager.Libraries.runFile(
				...
				alert(
					await window.__(
						I18NKeys.options.editPages.monaco.lintingDisabled,
						'typescript'
					)
				);
		...
		static setDefaultHeight(this: MonacoEditor) {
			let previous: number = this.$.editorElement.getBoundingClientRect()
				.height;
		...
		static setTempLayout(this: MonacoEditor) {
			let current: number = this.$.editorElement.getBoundingClientRect()
				.height;
	...
	export class MonacoEditorHookManager {
		...
		private static _scopes: [
			MonacoEditor,
			MonacoStandardEditor | MonacoDiffEditor
		][] = [];
		...
		static Caret = class MonacoEditorCaret {
			 ...
			 */
			private static _charCanvas: HTMLCanvasElement = document.createElement(
				'canvas'
			);
			...
			static caretRangeFromPoint(this: ShadowRoot, x: number, y: number) {
				...
				if (x > rect.left + rect.width) {
					...
					for (var i = 0; i < text.length + 1; i++) {
						...
						step =
							MonacoEditorHookManager.Caret.getCharWidth(
								text.charAt(i),
								font
							) / 2;
		...
		static setScope(scope: MonacoEditor) {
			...
			this._scopeListeners = this._scopeListeners.filter(
				({ scope: listenerScope, listener }) => {
		...
		static registerScope(
			scope: MonacoEditor,
			editor: MonacoStandardEditor | MonacoDiffEditor
		) {
		...
		private static _setupRequire() {
			return new window.Promise<void>(async (resolve) => {
				const require = await window.onExistsChain(
					window,
					'AMDLoader',
					'global',
					'require'
				...
				require.config({
					paths: {
						vs:
							'../elements/options/editpages/monaco-editor/src/min/vs',
					},
				});
		...
		private static _defineProperties() {
			...
			const keyCompletions = {
				isIncomplete: true,
				suggestions: Object.getOwnPropertyNames(descriptions).map(
					(key: keyof typeof descriptions) => {
						const description = descriptions[key];
						return {
							label: `@${key}`,
							kind: monaco.languages.CompletionItemKind.Property,
					...
					get: () => {
						return (context: {
							...
							for (const [scope, editor] of this._scopes) {
								if (
									context.viewDomNode === editor.getDomNode()
								) {
									return this.Caret.caretRangeFromPoint.bind(
										scope.shadowRoot
									);
								}
							}
		...
		static Completion = class MonacoEditorCompletions {
			...
			static register(
				language: 'less' | 'javascript' | 'css' | 'typescript' | 'json',
				item: monaco.languages.CompletionItemProvider
			) {
				this._enabledCompletions[language] =
					this._enabledCompletions[language] || [];
	...
	export type MonacoEditor = Polymer.El<
		'monaco-editor',
		typeof MOE & {
