...
export type CodeEditBehaviorScriptInstance = CodeEditBehavior<
	CodeEditBehaviorScriptInstanceAdditions
>;

type CodeEditBehaviorStylesheetInstanceAdditions = StylesheetEdit & {
	isScript: false;
};
export type CodeEditBehaviorStylesheetInstance = CodeEditBehavior<
	CodeEditBehaviorStylesheetInstanceAdditions
>;

export type CodeEditBehaviorInstance =
	| CodeEditBehavior<NodeEditBehaviorScriptInstance>
...
namespace CodeEditBehaviorNamespace {
	...
	export class CEB {
		 ...
		 */
		static editorMode: 'main' | 'background' | 'options' | 'libraries' =
			'main';
		...
		static fontSizeChange(this: CodeEditBehaviorInstance) {
			this.async(() => {
				window.app.settings.editor.zoom =
					this.$.editorThemeFontSizeInput.$$('input').value + '';
				window.app.upload();
			}, 50);
		}
		...
		static finishEditing(this: CodeEditBehaviorInstance) {
			...
			Array.prototype.slice
				.apply(this.shadowRoot.querySelectorAll('.editorTab'))
		...
		static insertSnippet(
			__this: CodeEditBehaviorInstance,
			snippet: string,
			...
			if (__this.editorManager.isTextarea(editor)) {
				...
				const replacement = noReplace
					? snippet
					: snippet.replace(/%s/g, content);
				const oldValue = editor.getValue();
				const newValue =
					oldValue.slice(0, from.totalChar) +
					replacement +
					oldValue.slice(to.totalChar);
				...
				if (selections.length === 1) {
					if (
						selections[0].toString().length === 0 &&
				...
				const commands = selections.map((selection) => {
					const content = noReplace
						? snippet
						: snippet.replace(/%s/g, selection.toString());
					return window.monacoCommands.createReplaceCommand(
						selection,
						content
					);
				});
		...
		static popOutToolsRibbon(this: CodeEditBehaviorInstance) {
			window.doc.editorToolsRibbonContainer.animate(
				...
				window.doc.editorToolsRibbonContainer.style.marginLeft =
					'-200px';
				window.doc.editorToolsRibbonContainer.classList.remove(
					'visible'
				);
			};
		}
		...
		static initToolsRibbon(this: CodeEditBehaviorInstance) {
			if (this.item.type === 'script') {
				(window.app.$
					.paperLibrariesSelector as PaperLibrariesSelector).init();
				(window.app.$
					.paperGetPageProperties as PaperGetPageProperties).init(
					(snippet: string) => {
			...
			setTimeout(() => {
				...
				if (!window.app.storageLocal.hideToolsRibbon) {
					$(window.doc.editorToolsRibbonContainer).animate(
						...
						{
							duration: 500,
							easing: ($ as CodeEditBehaviorNamespace.JQueryContextMenu).bez(
								[0.215, 0.61, 0.355, 1.0]
							),
							step: (now: number) => {
								window.addCalcFn(
									window.doc.fullscreenEditorEditor,
									'width',
									`100vw - 200px - ${now}px`
								);
					...
					window.doc.editorToolsRibbonContainer.classList.add(
						'visible'
					);
				}
			}, 200);
				...
				$(dummy).animate(
					...
					},
					{
						...
						step: (now: number) => {
							...
							window.addCalcFn(
								window.doc.fullscreenEditorHorizontal,
								'height',
								`100vh - ${now}px`
							);
							this.fullscreenEditorManager.editor.layout();
			...
			setTimeout(() => {
				...
				const hideToolsRibbon =
					(window.app.storageLocal &&
						window.app.storageLocal.hideToolsRibbon) ||
					false;
				if (!hideToolsRibbon) {
					$(window.doc.editorToolsRibbonContainer).animate(
						{
				...
				$(editorCont).animate(
					{
						...
						marginLeft: this.preFullscreenEditorDimensions
							.marginLeft,
					},
					{
						duration: 500,
		...
		static async enterFullScreen(
			...
			return new Promise<void>(async (resolve) => {
				...
				const rect = this.editorManager.editor
					.getDomNode()
					.getBoundingClientRect();
				const editorCont = window.doc.fullscreenEditorEditor;
				const editorContStyle = editorCont.style;
				editorContStyle.marginLeft = this.preFullscreenEditorDimensions.marginLeft =
					rect.left + 'px';
				editorContStyle.marginTop = this.preFullscreenEditorDimensions.marginTop =
					rect.top + 'px';
				window.addCalcFn(editorCont, 'height', '', true);
				window.addCalcFn(editorCont, 'width', '', true);
				editorContStyle.height = this.preFullscreenEditorDimensions.height =
					rect.height + 'px';
				editorContStyle.width = this.preFullscreenEditorDimensions.width =
					rect.width + 'px';
				editorContStyle.position = 'absolute';

				if (this.item.type === 'script') {
					const __this = this as CodeEditBehavior<
						NodeEditBehaviorScriptInstance
					>;
					const libsArr =
						__this.editorMode === 'main'
							? __this.newSettings.value.libraries
							: __this.newSettings.value.backgroundLibraries ||
							  [];
					window.app.$.paperLibrariesSelector.updateLibraries(
						libsArr as any,
						this.newSettings as CRM.ScriptNode,
						this.editorMode as 'main' | 'background'
					);
					if (
						__this.newSettings.value.ts &&
						__this.newSettings.value.ts.enabled
					) {
						window.app.$.editorTypescript.classList.add('active');
					} else {
						window.app.$.editorTypescript.classList.remove(
							'active'
						);
					}
				}

				this.fullscreenEditorManager = await editorCont.createFrom(
					this.editorManager
				);
				...
				if (window.app.storageLocal.hideToolsRibbon !== undefined) {
					if (window.app.storageLocal.hideToolsRibbon) {
						window.doc.showHideToolsRibbonButton.classList.add(
							'hidden'
						);
					} else {
						window.doc.showHideToolsRibbonButton.classList.remove(
							'hidden'
						);
					}
				} else {
				...
				$(editorCont).animate(
					...
					},
					{
						...
						complete: () => {
							...
							window.addCalcFn(
								window.app.$.fullscreenEditorHorizontal,
								'height',
								'',
								true
							);
							window.app.$.fullscreenEditorHorizontal.style.height =
								'100vh';
							this.popInRibbons();
							resolve(null);
						},
		...
		static showOptions(this: CodeEditBehaviorInstance) {
			...
			this.unchangedEditorSettings = $.extend(
				true,
				{},
			...
			const editorWidth = this.getEditorInstance()
				.editor.getDomNode()
				.getBoundingClientRect().width;
			const editorHeight = this.getEditorInstance()
				.editor.getDomNode()
			...
			(this.$$(
				'#editorThemeFontSizeInput'
			) as HTMLPaperInputElement).value = window.app.settings.editor.zoom;
				...
				.animate(
					...
					},
					{
						...
						progress: (animation: any) => {
							this.$.editorOptionsContainer.style.marginLeft =
								settingsInitialMarginLeft -
								animation.tweens[3].now +
								'px';
							this.$.editorOptionsContainer.style.marginTop =
								-animation.tweens[2].now + 'px';
						},
					}
				);
		}
		...
		static _hideFullscreenOptions(this: CodeEditBehaviorInstance) {
			window.setTransform(
				window.app.$.fullscreenSettingsContainer,
				'translateX(500px)'
			);
			window.setTimeout(() => {
			...
			$(this.$.settingsShadow).animate(
				...
				},
				{
					...
					complete: () => {
						...
						if (
							JSON.stringify(this.unchangedEditorSettings) !==
							JSON.stringify(window.app.settings.editor)
						) {
		 ...
		 */
		static async fillEditorOptions(
			this: CodeEditBehaviorInstance,
			container: CrmApp | CodeEditBehaviorInstance
		) {
			if (this.item.type === 'script') {
				...
				window.app.settings.editor.keyBindings = window.app.settings
					.editor.keyBindings || {
				...
				Array.prototype.slice
					.apply(
						scriptContainer.$.keyBindingsTemplate.querySelectorAll(
							'paper-input'
						)
					)
					.forEach((input: HTMLPaperInputElement) => {
						input.setAttribute(
							'data-prev-value',
							input.$$('input').value
						);
			...
			if (window.app.settings.editor.theme === 'white') {
				container.$.editorThemeSettingWhite.classList.add(
					'currentTheme'
				);
			} else {
				container.$.editorThemeSettingWhite.classList.remove(
					'currentTheme'
				);
			}
			if (window.app.settings.editor.theme === 'dark') {
				container.$.editorThemeSettingDark.classList.add(
					'currentTheme'
				);
			} else {
				container.$.editorThemeSettingDark.classList.remove(
					'currentTheme'
				);
			}

			container.$.editorThemeFontSizeInput.$$('input').value =
				window.app.settings.editor.zoom || '100';
		}
		...
		static getAutoUpdateState(this: CodeEditBehaviorInstance) {
			if (
				(this.newSettings.type !== 'script' &&
					this.newSettings.type !== 'stylesheet') ||
				this.newSettings.nodeInfo.source === 'local'
			) {
				return [false, true];
			}
			if (this.newSettings.nodeInfo && this.newSettings.nodeInfo.source) {
		...
		static initUI(this: CodeEditBehaviorInstance) {
			this.$.dropdownMenu.addEventListener('expansionStateChange', (({
				detail,
			}: Polymer.EventType<
		...
		static toggleAutoUpdate(this: CodeEditBehaviorInstance) {
			...
			this.newSettings.nodeInfo.source.autoUpdate = !this.newSettings
				.nodeInfo.source.autoUpdate;
			this.setUpdateIcons(
...
export type CodeEditBehaviorBase = Polymer.El<
	'code-edit-behavior',
	typeof CodeEditBehaviorNamespace.CEB
>;

export type CodeEditBehaviorGlobal = CodeEditBehaviorNamespace.CodeEditBehaviorGlobal;

export type CodeEditBehavior<
	T =
		| CodeEditBehaviorScriptInstanceAdditions
		| CodeEditBehaviorStylesheetInstanceAdditions
...
window.Polymer.CodeEditBehavior =
	window.Polymer.CodeEditBehavior ||
	(CodeEditBehaviorNamespace.CEB as CodeEditBehaviorBase);
