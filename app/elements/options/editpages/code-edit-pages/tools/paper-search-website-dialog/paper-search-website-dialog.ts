...
namespace PaperSearchWebsiteDialog {
	...
	type PaperSearchWebsiteDialogWindow =
		| 'initialWindow'
	...
	export class PSWD {
		...
		static goBackWindow(this: PaperSearchWebsiteDialog) {
			const newWindow = this.windows[
				this.windowPath[this.windowPath.length - 2]
			];
			this.windowPath.pop();
		 ...
		 */
		static async switchToWindow(
			this: PaperSearchWebsiteDialog,
			dialogWindow: PaperSearchWebsiteDialogWindow
		) {
		...
		static switchWindow(
			...
			const el = window.app.util.findElementWithTagname(
				event,
				'paper-button'
			);
			this.switchToWindow(
				el.getAttribute('window') as PaperSearchWebsiteDialogWindow
			);
		}
		...
		static loadWindow(
			this: PaperSearchWebsiteDialog,
			window: PaperSearchWebsiteDialogWindow,
			prom: Promise<string>
		) {
		...
		static async insertCode(this: PaperSearchWebsiteDialog) {
			const code = `var search = crmAPI.getSelection() || prompt('${await this.__async(
				I18NKeys.options.tools.paperSearchWebsiteDialog.enterSearchQuery
			)}');
...
${
	this.$.howToOpenLink.selected === 'currentTab'
		 ...
		 */
		static processSearchEngines(
			this: PaperSearchWebsiteDialog
		): Promise<string> {
			return new Promise((resolve, reject) => {
				const data = (this.$.manualInputListChoiceInput.$$(
					'iron-autogrow-textarea'
				) as Polymer.RootElement).$$('textarea').value as EncodedString<
					string[]
				>;
		...
		static processManualInput(this: PaperSearchWebsiteDialog) {
			if (this.selectedIsUrl) {
				this.chosenUrl = this.$.manualInputURLInput
					.$$('input')
					.value.replace(
						/custom( )?[rR]ight( )?(-)?[cC]lick( )?[mM]enu/g,
						'%s'
					);
		 ...
		 */
		static async confirmManualSearchListInput(
			this: PaperSearchWebsiteDialog
		) {
			const checkedButton = this.$.listInputSearchList.querySelector(
				'paper-radio-button[checked]'
			);
			if (!checkedButton) {
				window.app.util.showToast(
					await this.__async(
						I18NKeys.options.tools.paperSearchWebsiteDialog
							.selectSomething
					)
				);
				return;
		 ...
		 */
		static cancelAllRadiobuttons(
			this: PaperSearchWebsiteDialog,
			e: Polymer.ClickEvent
		) {
			const target = e.target;
			this.async(() => {
				const checkedEl = this.$.listInputSearchList.querySelector(
					'paper-radio-button[checked]'
				) as HTMLPaperRadioButtonElement;
		...
		static ready(this: PaperSearchWebsiteDialog) {
			this.$.paperSearchWebsiteDialog.addEventListener(
				'iron-overlay-closed ',
				this.clear
			);
		}
		...
		static init(this: PaperSearchWebsiteDialog) {
			this.$.manualInputSavedChoice.disabled =
				BrowserAPI.getBrowser() !== 'chrome';
			this.clear();
			this.fit();
		}

		static setOutputType(
			this: PaperSearchWebsiteDialog,
			outputType: 'link' | 'script'
		) {
		...
		static fixFit(this: PaperSearchWebsiteDialog) {
			const paperInputContainer = this.$.manualInputListChoiceInput.$$(
...
export type PaperSearchWebsiteDialog = Polymer.El<
	'paper-search-website-dialog',
	typeof PaperSearchWebsiteDialog.PSWD &
		typeof PaperSearchWebsiteDialog.paperSearchWebsiteDialogProperties
>;
