...
namespace PaperLibrariesSelectorElement {
	...
	export class PLS implements I18NClass {
		...
		static sortByName(
			first: LibrarySelectorLibrary,
			second: LibrarySelectorLibrary
		): number {
			return (
				first.name[0].toLowerCase().charCodeAt(0) -
				second.name[0].toLowerCase().charCodeAt(0)
			);
		}
		...
		private static _getLibraries(
			this: PaperLibrariesSelector,
			selectedObj: {
				[key: string]: boolean;
			}
		) {
		...
		private static _setSelectedLibraries(
			this: PaperLibrariesSelector,
			libraries: LibrarySelectorLibrary[]
		) {
		...
		private static _displayLibraries(
			this: PaperLibrariesSelector,
			anonymous: LibrarySelectorLibrary[],
			libraries: LibrarySelectorLibrary[]
		) {
			...
			anonymous.forEach((item) => {
				const itemCopy: LibrarySelectorLibrary = {
					isLibrary: true,
					name: `${item.url} (${this.___(
						I18NKeys.options.tools.paperLibrariesSelector.anonymous
					)})`,
			...
			libraries.push({
				name: this.___(
					I18NKeys.options.tools.paperLibrariesSelector.addOwn
				),
		...
		static onLangChanged(this: PaperLibrariesSelector) {
			...
			this._displayLibraries(
				this._viewLibs.anonymous,
		...
		static async init(
			...
			const { anonymous, selectedObj } = this.categorizeLibraries();
		...
		static confirmLibraryFile(
			...
			setTimeout(() => {
				...
				const denyAdditionListener = () => {
					window.doc.addLibraryConfirmationContainer.style.display =
						'none';
					window.doc.addLibraryProcessContainer.style.display =
						'block';
					this._resetAfterAddDecision();
					window.doc.addLibraryConfirmationInput.value = '';
				};
				window.doc.addLibraryConfirmAddition.addEventListener(
					'click',
					confirmAdditionListener
				);
				window.doc.addLibraryDenyConfirmation.addEventListener(
					'click',
					denyAdditionListener
				);
		...
		private static _addLibraryToState(
			this: PaperLibrariesSelector,
			name: string,
			isTypescript: boolean,
			code: string,
		...
		private static _hideElements<T extends keyof typeof window.doc>(
			...els: T[]
		) {
		...
		private static _showElements<T extends keyof typeof window.doc>(
			...els: T[]
		) {
			for (let i = 0; i < els.length; i++) {
		...
		private static _showElementsFlex<T extends keyof typeof window.doc>(
			...els: T[]
		) {
			for (let i = 0; i < els.length; i++) {
		...
		static async addLibraryFile(
			...
			this._hideElements(
				...
				'addLibraryProcessContainer',
				'addLibraryDialogSuccesCheckmark'
			);
			...
			$(window.doc.addLibraryDialogSucces).animate(
				...
				},
				{
					...
					complete: () => {
						...
						window.doc.addLibraryDialogSuccesCheckmark.classList.add(
							'animateIn'
						);
						setTimeout(() => {
		...
		private static _getStatusCodeDescr(
			this: PaperLibrariesSelector,
			code: number
		) {
			switch ((code + '')[0]) {
			...
			if (name !== '' && !taken) {
				...
				if (window.doc.addLibraryRadios.selected === 'url') {
					...
					window.app.util
						.xhr(url, false)
						.then((data) => {
							this.confirmLibraryFile(
								name,
								window.doc.addLibraryIsTS.checked,
								data,
								url
							);
						})
						.catch(async (statusCode) => {
							const statusMsg = this._getStatusCodeDescr(
								statusCode
							);
							const msg = statusMsg
								? await this.__async(
										I18NKeys.options.tools
								...
								: await this.__async(
										I18NKeys.options.tools
											.paperLibrariesSelector.xhrFailed,
										statusCode
								  );
					...
					this.addLibraryFile(
						...
						) as Polymer.RootElement).$$('textarea').value
					);
				}
			} else {
				if (taken) {
					...
					input.errorMessage = await this.__async(
						I18NKeys.options.tools.paperLibrariesSelector
							.nameMissing
					);
				}
				input.invalid = true;
		...
		private static _handleCheckmarkClick(
			this: PaperLibrariesSelector,
			...
			const changeType:
				| 'addMetaTags'
				| 'removeMetaTags' = container.classList.contains(
				'iron-selected'
			)
				? 'removeMetaTags'
				: 'addMetaTags';
			...
			const mainModel = window.scriptEdit.editorManager.getModel(
				'default'
			);
			const backgroundModel = window.scriptEdit.editorManager.getModel(
				'background'
			);
			const TSLibsMod =
				window.scriptEdit.editorManager.CustomEditorModes
					.TS_LIBRARIES_META;
			type TSLibsMod = MonacoEditorTSLibrariesMetaMods;
			if (
				typeof mainModel.editorType === 'object' &&
			...
			if (
				backgroundModel &&
				typeof backgroundModel.editorType === 'object' &&
		...
		static _click(this: PaperLibrariesSelector, e: Polymer.ClickEvent) {
			const container = window.app.util.findElementWithTagname(
				e,
				'paper-item'
			);
		...
		private static _getInstalledLibrary(
			this: PaperLibrariesSelector,
			library: LibrarySelectorLibrary
		...
		private static async _openLibraryEditor(
			this: PaperLibrariesSelector,
			library: LibrarySelectorLibrary
		) {
			...
			window.scriptEdit.fullscreenEditorManager.switchToModel(
				'libraryEdit',
				installedLibrary.code,
				isTs
		...
		static async _edit(
			...
			if (e.target.tagName.toLowerCase() === 'path') {
				parentNode = e.target.parentElement.parentElement
					.parentElement as LibraryElement;
		...
		static updateLibraries(
			this: PaperLibrariesSelector,
			libraries: LibrarySelectorLibrary[],
			srcNode: CRM.ScriptNode,
			mode: 'main' | 'background' = 'main'
		) {
...
export type PaperLibrariesSelectorBase = Polymer.El<
	'paper-libraries-selector',
	typeof PaperLibrariesSelectorElement.PLS &
		typeof PaperLibrariesSelectorElement.paperLibrariesSelectorProperties
>;
export type PaperLibrariesSelector = PaperDropdownBehavior<
	PaperLibrariesSelectorBase
>;
