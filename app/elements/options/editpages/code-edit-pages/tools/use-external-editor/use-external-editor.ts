...
namespace UseExternalEditorElement {
	...
	type ConnectedEditorMessage =
		| ChooseFileMessage
		| SetupExistingFileMessage
		| SetupNewFileMessage
		| UpdateFromAppMessage;

	type ExternalEditorMessage =
	...
	export class UEE {
		...
		static async init(this: UseExternalEditor) {
			...
			window.doc.externalEditorDialogTrigger.style.color =
				'rgb(38, 153, 244)';
		 ...
		 */
		private static updateFromExternal(
			this: UseExternalEditor,
			msg: UpdateFromAppMessage
		) {
			if (this.connection.id === msg.connectionId) {
				if (window.scriptEdit && window.scriptEdit.active) {
					...
					window.stylesheetEdit.newSettings.value.stylesheet =
						msg.code;
		...
		private static EditingOverlay = class EditingOverlay {
			private static createToolsCont(): HTMLElement {
				...
				toolsCont.appendChild(
					window.app.util.createElement(
						'div',
						{
			...
			private static createDisconnect() {
				...
				const el = window.app.util.createElement(
					'div',
					{
								...
								window.app.util.createElement(
									...
									[
										window.app.util.createElement('path', {
											props: {
												d:
													'M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17' +
			...
			private static createShowLocation() {
								...
								window.app.util.createElement(
									...
									[
										...
										window.app.util.createElement('path', {
											props: {
												d:
													'M20 6h-8l-2-2H4c-1.1 0-1.99.9-1.99 2L2 ' +
				...
				el.addEventListener('click', () => {
					...
					window.doc.externalEditoOpenLocationInBrowser.setAttribute(
						'href',
						'file:///' + location
					);
					const externalEditorLocationToast =
						window.doc.externalEditorLocationToast;
					externalEditorLocationToast.text =
						'File is located at: ' + location;
										...
										[
											window.app.util.createElement(
												'path',
												{
													props: {
														d:
															'M6 2c-1.1 0-1.99.9-1.99 2L4 20c0 1.1' +
								...
								window.app.util.createElement(
									...
									[
										window.app.util.createElement('path', {
											props: {
												d:
													'M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58' +
			...
			private static createCont(toolsCont: HTMLElement) {
				const cont = toolsCont.appendChild(
			...
			private static appendWrapper(toolsCont: HTMLElement) {
				...
				const editor = editorManager.editor as
				...
				const widget = {
					...
					getPosition() {
						return {
							preference:
								monaco.editor.OverlayWidgetPositionPreference
									.TOP_RIGHT_CORNER,
						};
					},
				};
		...
		static setupExternalEditing(this: UseExternalEditor) {
			...
			if (manager.isTextarea(manager.getEditorAsMonaco())) {
				window.app.util.showToast(
					'Please update your chrome (at least chrome 30) to use this feature'
				);
				return;
			}
			...
			if (this.connection.connected) {
				...
				const tempListener = function (
					msg: SetupExistingFileMessage | SetupNewFileMessage
				) {
					if (
						msg.status === 'connected' &&
						(msg.action === 'setupScript' ||
							msg.action === 'setupStylesheet') &&
						msg.connectionId === _this.connection.id
					) {
				...
				if (item.file) {
					this.appPort.postMessage({
						...
						code:
							window.scriptEdit && window.scriptEdit.active
								? (item as CRM.ScriptNode).value.script
								: (item as CRM.StylesheetNode).value.stylesheet,
						id: item.file.id,
					});
				} else {
					this.appPort.postMessage({
		...
		private static appMessageHandler(
			this: UseExternalEditor,
			msg: ConnectedEditorMessage
		) {
			switch (msg.action) {
				case 'chooseFile':
					const chooseFileDialog =
						window.doc.externalEditorChooseFile;
		...
		private static establishConnection(this: UseExternalEditor) {
			if (!this.appPort) {
				this.appPort = browserAPI.runtime.connect(
					EXTERNAL_EDITOR_APP_ID
				);
		...
		static async initEditor(
			...
			this.editorManager = await window.doc.chooseFileMerger.createDiff(
				[oldScript, newScript],
				mode,
				{
					wordWrap: 'off',
		...
		static onDialogMainDivAnimationHideEnd(
			this: UseExternalEditor,
			dialogRect: ClientRect,
			...
			this.playIfExists(this.dialogExpansionAnimation) ||
				(this.dialogExpansionAnimation = this.doCSSAnimation(
					window.doc.externalEditorChooseFile,
					[
						{
					...
					() => {
						...
						this.playIfExists(
							this.dialogComparisonDivAnimationShow
						) ||
							(this.dialogComparisonDivAnimationShow = this.doCSSAnimation(
								window.doc.chooseFileMerger,
								[
									{
										marginTop: '70px',
										opacity: 0,
		...
		static showMergeDialog(
			...
			this.dialogMainDivAnimationHide.onfinish = () => {
				this.onDialogMainDivAnimationHideEnd(
					dialogRect,
					dialogStyle,
					oldScript,
					newScript
		...
		private static chooseFileDialog(
			this: UseExternalEditor,
			...
			return (local, file, callback) => {
				...
				if (_this.dialogStyleProperties) {
					_this.resetStyles(
						chooseFileDialog.style,
						_this.dialogStyleProperties
					);
				}
			};
		}
		...
		static async ready(this: UseExternalEditor) {
			...
			await window.onExists('app');
			const chooseFileDialog = window.doc
				.externalEditorChooseFile as ChooseFileDialog;
			chooseFileDialog.init = this.chooseFileDialog(chooseFileDialog);
			window.doc.externalEditorTryAgainButton.addEventListener(
				'click',
				() => {
			...
			window.doc.chooseFileChooseCRM.addEventListener('click', () => {
				...
				if (this.editorManager.isDiff(editor)) {
					chooseFileDialog.callback(
						editor.getModel().original.getValue()
					);
				}
			});
...
export type UseExternalEditor = Polymer.El<
	'use-external-editor',
	typeof UseExternalEditorElement.UEE
>;
