	...
	export class TS {
		...
		static colorTypeChoices(this: TypeSwitcher) {
			Array.prototype.slice
				.apply(this.shadowRoot.querySelectorAll('.typeSwitchChoice'))
				.forEach((choice: HTMLElement) => {
					$(choice).attr('type', $(choice).children()[0].innerHTML);
				});
		}

		static closeTypeSwitchContainer(
			this: TypeSwitcher,
			quick: boolean = false,
			callback?: () => void
		) {
				...
				.animate(
					...
					},
					{
						...
						complete: () => {
							...
							const choicesContainer = this.shadowRoot.querySelector(
								'#typeSwitchChoicesContainer'
							);
		...
		static shadowColumns(
			this: TypeSwitcher,
			column: HTMLElement,
			reverse: boolean
		) {
			$(column.querySelector('#itemCont'))
				.animate({
		...
		static changeType(
			this: TypeSwitcher,
			e: Polymer.ClickEvent | CRM.NodeType
		) {
			...
			if (typeof e === 'string') {
				...
				const path = window.app.util.getPath(
					e
			...
			if (
				item.name ===
				`My ${prevType[0].toUpperCase() + prevType.slice(1)}`
			) {
			...
			const typeChoices = Array.prototype.slice.apply(
				this.shadowRoot.querySelectorAll('.typeSwitchChoice')
			);
...
export type TypeSwitcher = Polymer.El<
	'type-switcher',
	typeof TypeSwitcherElement.TS
>;
