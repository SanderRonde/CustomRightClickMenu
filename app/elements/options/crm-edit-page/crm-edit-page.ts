...
namespace CrmEditPageElement {
	...
	export class CEP {
		...
		static getCreatedBy(this: CrmEditPage) {
			return this.___(
				I18NKeys.options.crmEditPage.createdByYou,
				`<b id="nodeInfoByYou" title="${this.___(
					I18NKeys.options.crmEditPage.hasAllPermissions
				)}">You</b>`
			);
		}
		...
		static hideNodeInfo(
			...
			return (
				!this._nodeInfoExists(nodeInfo) ||
				(this.isLocal(nodeInfo.source) &&
					!this.hasInstallDate(nodeInfo))
			);
		}
		...
		private static _animateIn(this: CrmEditPage) {
			...
			return window.animateTransform(
				this.$.overlayCont,
				{
					propName: 'scale',
					postfix: '',
		...
		static animateOut(this: CrmEditPage) {
			...
			const animation = window.animateTransform(
				this.$.overlayCont,
				{
					propName: 'scale',
					postfix: '',
			...
			if (node) {
				if (window.app.storageLocal.upgradeErrors) {
					if (window.app.storageLocal.upgradeErrors[node.id]) {
						return (
							'Some errors have occurred in updating this script. Please resolve them by clicking the link and replace any chrome ' +
		...
		static hideUpdateMergeDialog(this: CrmEditPage) {
			if (this.showUpgradeNotice(this.hideUpdateMessage, this.item)) {
				var height = this.$.scriptUpdateNotice.getBoundingClientRect()
					.height;
		...
		static showScriptUpdateDiff(this: CrmEditPage) {
			...
			chooseDialog.init(
				...
				(chosenScript: string) => {
					if (window.app.storageLocal.upgradeErrors) {
						delete window.app.storageLocal.upgradeErrors[
							this.item.id
						];
					}
					...
					browserAPI.storage.local.set({
						upgradeErrors:
							window.app.storageLocal.upgradeErrors || {},
					} as any);
				},
				true,
				window.app.storageLocal.upgradeErrors &&
					window.app.storageLocal.upgradeErrors[this.item.id]
			);
		...
		static getInstallDate(this: CrmEditPage) {
			if (
				...
				const format =
					new Date(2016, 1, 13).toLocaleDateString() === '13-2-2016'
						? 'eu'
						: 'na';
				let date;
				if (format === 'eu') {
		...
		static ready(this: CrmEditPage) {
			...
			window.onExists('app').then(() => {
				...
				this.$.nodeInfoVersion.addEventListener('input', () => {
					this.item.nodeInfo.version =
						this.$.nodeInfoVersion.innerText.length > 0
							? this.$.nodeInfoVersion.innerText
							: '1.0';
				});
			});
		}
		...
		static init(this: CrmEditPage) {
			...
			if ((valueStorer.isScript = node.type === 'script')) {
				...
				(valueStorer.isStylesheet = node.type === 'stylesheet')
			) {
...
export type CrmEditPage = Polymer.El<
	'crm-edit-page',
	typeof CrmEditPageElement.CEP &
		typeof CrmEditPageElement.crmEditPageProperties
>;
