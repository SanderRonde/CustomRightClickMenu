...
import {
	...
	CodeEditBehavior,
} from '../editpages/code-edit-pages/code-edit-behavior';
...
namespace NodeEditBehaviorNamespace {
	...
	export class NEB {
		...
		static getContentTypeLaunchers(
			this: NodeEditBehaviorInstance,
			resultStorage: Partial<CRM.Node>
		) {
			const containers = this.$.showOnContentIconsContainer.children;
			resultStorage.onContentTypes = Array.prototype.slice
				.apply(containers)
				.map((item: Element) => {
		...
		static getTriggers(
			...
			for (let i = 0; i < containers.length; i++) {
				triggers[i] = {
					url: containers[i].querySelector('paper-input').$$('input')
						.value,
					not: (containers[i].querySelector(
						'.executionTriggerNot'
					) as HTMLPaperCheckboxElement).checked,
				};
			}
		...
		static save(
			this: NodeEditBehaviorInstance,
			_event?: Polymer.ClickEvent,
			resultStorage?: Partial<CRM.Node> | MouseEvent
		) {
			...
			const itemInEditPage = window.app.editCRM.getCRMElementFromPath(
				this.item.path,
				false
			);
			...
			if (newSettings.value && newSettings.type !== 'link') {
				if (
					(newSettings as CRM.ScriptNode | CRM.StylesheetNode).value
					...
					(newSettings as
						| CRM.ScriptNode
						| CRM.StylesheetNode).onContentTypes = [
						true,
						true,
						true,
						true,
			...
			window.app.templates.mergeObjectsWithoutAssignment(
				resultStorage,
				newSettings
			);
		...
		static assignContentTypeSelectedValues(this: NodeEditBehaviorInstance) {
			...
			for (i = 0; i < 6; i++) {
				this[
					arr[i] as keyof NodeEditBehaviorProperties
				] = this.item.onContentTypes[i];
			}
		}

		static checkToggledIconAmount(
			this: NodeEditBehaviorInstance,
			e: {
				path?: HTMLElement[];
				Aa?: HTMLElement[];
			...
			this.async(() => {
				...
				if (
					selectedCheckboxes.onContentTypes.filter((item) => item)
						.length === 0
				) {
		...
		static toggleIcon(
			...
			const checkbox = ($(element).find(
				'paper-checkbox'
			)[0] as unknown) as HTMLPaperCheckboxElement;
		...
		static clearTrigger(
			...
			const index = (Array.prototype.slice.apply(
				triggers
			) as HTMLElement[]).indexOf(element.parentElement);
		...
		static _getPattern(
			...
			Array.prototype.slice
				.apply(this.querySelectorAll('.triggerInput'))
				.forEach(function (triggerInput: HTMLPaperInputElement) {
		...
		static _getLabel(
			...
			if (this.newSettings.value.launchMode === 2) {
				return this.__(
					lang,
					langReady,
					I18NKeys.options.nodeEditBehavior.globPattern
				);
			} else {
				return this.__(
					lang,
					langReady,
					I18NKeys.options.nodeEditBehavior.matchPattern
				);
			}
		}

		static animateTriggers(
			this: CodeEditBehavior,
			show: boolean
		): Promise<void> {
		...
		static animateContentTypeChooser(
			this: CodeEditBehavior<{}>,
			show: boolean
		) {
		 ...
		 */
		static async selectorStateChange(
			this: CodeEditBehavior,
			prevState: number,
			state: number
		) {
			...
			if (
				oldStates.showContentTypeChooser !==
				newStates.showContentTypeChooser
			) {
				this.animateContentTypeChooser(
					newStates.showContentTypeChooser
				);
			}
			...
			if (
				...
				oldStates.showInsteadOfExecute
			) {
				((this.$ as any)[
					'showOrExecutetxt'
				] as HTMLSpanElement).innerText = newStates.showInsteadOfExecute
		...
		static matchesTypeScheme(
			this: NodeEditBehaviorInstance,
			type: CRM.NodeType,
			data: CRM.LinkVal | CRM.ScriptVal | CRM.StylesheetVal | null
		): boolean {
			switch (type) {
				case 'link':
		...
		static _doTypeChange(
			...
			const typeSwitcher = editCrmEl.shadowRoot.querySelector(
				'type-switcher'
			);
			typeSwitcher.onReady();

			const typeChoices = Array.prototype.slice.apply(
				typeSwitcher.shadowRoot.querySelectorAll('.typeSwitchChoice')
			);
			for (let i = 0; i < typeSwitcher.remainingTypes.length; i++) {
				typeChoices[i].setAttribute(
					'type',
					typeSwitcher.remainingTypes[i]
				);
			}

			if (prevType === 'menu') {
				//Turn children into "shadow items"
				const column = (typeSwitcher.parentElement.parentElement
		...
		static async _changeType(
			...
			const editCrmEl = window.app.editCRM.getCRMElementFromPath(
				window.app.nodesById.get(id).path,
				false
			);
		...
		static initDropdown(this: CodeEditBehavior) {
			this.showTriggers =
				this.item.value.launchMode > 1 &&
				this.item.value.launchMode !== 4;
			this.showContentTypeChooser =
				this.item.value.launchMode === 0 ||
				this.item.value.launchMode === 3;
			...
			this.$.dropdownMenu._addListener(
	...
	window.Polymer.NodeEditBehavior =
		window.Polymer.NodeEditBehavior ||
		(NodeEditBehaviorNamespace.NEB as NodeEditBehaviorBase);
}

export type NodeEditBehaviorBase = Polymer.El<
	'node-edit-behavior',
	typeof NodeEditBehaviorNamespace.NEB &
		typeof NodeEditBehaviorNamespace.nodeEditBehaviorProperties
>;
export type NodeEditBehavior<T> = T & NodeEditBehaviorBase;

export type NodeEditBehaviorScriptInstance = NodeEditBehavior<
	CodeEditBehaviorScriptInstance & {
		newSettings: Partial<CRM.ScriptNode>;
	}
>;
export type NodeEditBehaviorStylesheetInstance = NodeEditBehavior<
	CodeEditBehaviorStylesheetInstance & {
		newSettings: Partial<CRM.StylesheetNode>;
	}
>;
export type NodeEditBehaviorLinkInstance = NodeEditBehavior<
	LinkEdit & {
		newSettings: Partial<CRM.LinkNode>;
	}
>;
export type NodeEditBehaviorMenuInstance = NodeEditBehavior<
	MenuEdit & {
		newSettings: Partial<CRM.MenuNode>;
	}
>;
export type NodeEditBehaviorDividerInstance = NodeEditBehavior<
	DividerEdit & {
		newSettings: Partial<CRM.DividerNode>;
	}
>;

export type NodeEditBehaviorInstance =
	| NodeEditBehaviorScriptInstance
	| NodeEditBehaviorStylesheetInstance
	| NodeEditBehaviorLinkInstance
	| NodeEditBehaviorMenuInstance
	| NodeEditBehaviorDividerInstance;
