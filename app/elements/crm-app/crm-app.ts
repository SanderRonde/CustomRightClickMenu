...
(() => {
	 ...
	 */
	function insertInto<T>(toAdd: T, target: Array<T>, position: number): Array<T> {
	...
	export type CRMApp = PolymerElement<typeof CA & typeof properties & {
		editCRM: any;
	}>;
	...
	class CA {
			...
			if (toRequest.length > 0 || force) {
				chrome.permissions.getAll(function(allowed) {
					...
					var interval = window.setInterval(function() {
						try {
							if ((window.doc['requestPermissionsCenterer'] as CenterElement).$['content'].children[0].open) {
		...
		static parseOldCRMNode(this: CRMApp, string: string, openInNewTab: boolean) {
			var node = {};
		...
		static transferCRMFromOld(this: CRMApp, openInNewTab: boolean, storageSource: Storage = localStorage): CRMTree {
			var i;
		...
		static animateLoadingBar(this: CRMApp, settings, progress) {
			var _this = this;
		...
		get getPermissionDescription(this: CRMApp): (permission: string) => string {
		...
		static applyAddedPermissions(this: CRMApp) {
			...
			panels.forEach(function(panel) {
				var node = _this.nodesById[panel.getAttribute('data-id')] as ScriptNode;
		...
		static templates = class CRMAppTemplates {
			...
			static getPermissionDescription(permission: string): string {
				...
				type PermissionDescriptionKeys = typeof descriptions;
		...
		static pageDemo = class CRMAppPageDemo {
			...
			static handlers = class CRMAppPageDemoHandlers {
				 ...
				 */
				stylesheet = class CRMAppPageDemoHandlersStylesheet {
					/**
			...
			node = class CRMAppPageDemoNode {
				 ...
				 * @returns {Object} The root node of a subtree
				 */
				static menu(toAdd, crmType, index) {
					var _this = this;
					var item = {};
					item.name = toAdd.name;
					var childItems = {};
			 ...
			 */
			static isNodeVisible(node, showContentType) {
				var i;
				var length;
			 ...
			 */
			static buildForCrmType(crmType) {
				var _this = this;
				var index = {
					num: 0
				};
			...
			static bindContextMenu(crmType) {
				var items;
				var _this = this;
				if (crmType === 0) {
		 ...
		 * CRM functions.
		 */
		crm = class CRMAppCRMFunctions {
			static _getEvalPath(path) {
				return 'window.app.settings.crm[' + (path.join('].children[')) + ']';
			};

			static lookup(path, returnArray) {
			...
			static _lookupId(id, returnArray, node) {
			...
			static lookupId(id, returnArray) {
                if (!returnArray) {
			 ...
			 */
			static move(toMove, target, sameColumn) {
