...
} from '../../options/editpages/monaco-editor/monaco-editor';
	...
	export class IC {
		...
		static getDescription(
			...
			const descriptions = {
				...
				webNavigation:
					this.___(I18NKeys.permissions.webNavigation) +
					' (https://developer.chrome.com/extensions/webNavigation)',
				webRequest: this.___(I18NKeys.permissions.webRequest),
				webRequestBlocking: this.___(
				...
				GM_addValueChangeListener: this.___(
					I18NKeys.permissions.GMAddValueChangeListener
				),
				GM_removeValueChangeListener: this.___(
		...
		static showPermissionDescription(
			this: InstallConfirm,
		...
		private static _isManifestPermissions(
			this: InstallConfirm,
			permission: CRM.Permission
		): boolean {
			const permissions = [
						...
						try {
							if (!browserAPI.permissions) {
								window.app.util.showToast(
									this.___(
										I18NKeys.install.confirm.notAsking,
										permission
									)
								);
								return;
			...
			this._getCheckboxes().forEach((checkbox) => {
				checkbox.checked &&
					allowedPermissions.push(
						checkbox.getAttribute('permission') as CRM.Permission
					);
			});
		...
		private static _setMetaTag(
			this: InstallConfirm,
			name: keyof ModuleMap['install-confirm'],
		...
		private static _setMetaInformation(
			this: InstallConfirm,
			tags: CRM.MetaTags
		) {
			...
			window.installPage.$.title.innerHTML = `Installing <b>${
				tags['name'] && tags['name'][0]
			}</b>`;
		...
		private static async _loadEditor(this: InstallConfirm) {
			...
			const editorManager = (this._editorManager = await this.$.editorCont.create(
				this.$.editorCont.EditorMode.JS_META,
				{
					value: this.script,
					language: 'javascript',
					theme:
						this._settings.editor.theme === 'dark'
							? 'vs-dark'
							: 'vs',
					wordWrap: 'off',
					readOnly: true,
...
export type InstallConfirm = Polymer.El<
	'install-confirm',
	typeof InstallConfirmElement.IC &
		typeof InstallConfirmElement.installConfirmProperties
>;
