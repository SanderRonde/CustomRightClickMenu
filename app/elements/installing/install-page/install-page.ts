...
namespace InstallPageElement {
	...
	export class IP implements I18NClass {
		...
		private static _getSearchParams(
			url: string
		): {
			[key: string]: string;
		} {
			...
			const searchParamsArr = queryString
				.split('&')
				.map((keyVal: string) => {
		...
		static async displayFetchedUserscript(
			this: InstallPage,
			script: string
		) {
		...
		static onLangChanged(this: InstallPage) {
			this.$.title.innerHTML = this.___(
				I18NKeys.install.page.installing,
				this.userscriptUrl
			);
		}
		...
		static async ready(this: InstallPage) {
			...
			this._initSettings();
			this.$.title.innerHTML = await this.__async(
				I18NKeys.install.page.installing,
				this.userscriptUrl
			);
		}
...
export type InstallPage = Polymer.El<
	'install-page',
	typeof InstallPageElement.IP &
		typeof InstallPageElement.installPageProperties
>;
