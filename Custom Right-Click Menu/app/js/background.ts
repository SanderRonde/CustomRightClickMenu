...
(function (extensionId, globalObject: GlobalObject, sandboxes) {
	...
	*/
	var jsonFn = {
		...
		parse: function (str, date2Obj?: boolean) {
	...
	function updateTabAndIdLists(force?: boolean) {
	...
	function createSecretKey(): Array<number> {
	...
	function makeSafe(node): SafeCRMNode {
		var newNode: {
			children?: Array<SafeCRMNode>
		} = {};
		if (node.children) {
	...
	function parseNode(node, isSafe?: boolean) {
	...
	function uploadChanges(type, changes, useStorageSync?: boolean) {
		switch (type) {
			case 'local':
	...
	function checkBackgroundPagesForChange(changes, toUpdate?: Array<number>) {
		if (toUpdate) {
	...
	function updateCrm(toUpdate?: Array<number>) {
	...
	//#region CRMFunction
	function respondToCrmAPI(message, type, data, stackTrace?: string) {
		var msg: {
			type: string,
			callbackId: number,
			messageType: string,
	...
	function throwChromeError(message, error, stackTrace?: string) {
	...
	function installUserscript(metaTags, code, downloadURL, allowedPermissions, oldNodeId?: any): {
		node: any,
		path: Array<number>,
		oldNodeId: number,
	}|{
	...
	function updateScripts(callback?: (data: any) => void) {
		var checking = [];
		var updatedScripts = [];
	...
	function convertFileToDataURI(url, callback, onError?: () => void) {
		var xhr = new window.XMLHttpRequest();
	...
	function respondToInstanceMessageCallback(message, status, data?: any) {
		var msg = {
			type: status,
			callbackId: message.onFinish,
			messageType: 'callback',
		...
		switch (message.type) {
			...
			case 'installUserScript':
				...
				if (newScript.path) { //Has old node
					var nodePath = newScript.path as Array<number>;
...
window.specialJSON = {
	resolveJson: function(root, args){
		...
		var F = function(){};
		function walk(it, stop, defaultId?: any, needsPrefix?: any, schema?: any, defaultObject?: any){
