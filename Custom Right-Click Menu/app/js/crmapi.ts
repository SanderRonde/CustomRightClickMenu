...
interface Window {
	sendCallbackMessage: (settings: {}) => void;
	handshake: (id: number, secretKey: Array<number>, messageHandler: (message) => void) => void;
	log: (...params) => void;
	$: any;
}
 ...
 */
function CrmAPIInit(node: CRMNode, id: number, tabData: TabData, clickData: ClickData, secretKey: Array<number>, nodeStorage: NodeStorage, greasemonkeyData: GreaseMonkeyData, isBackground: boolean) {
	var _this = this;
	...
	*/
	...
	var callInfo: { [key: number]: {
		callback: (status: string, data: any, stacktrace: Array<string>) => void;
		stackTrace: Array<string>;
		persistent: boolean;
	}} = {};

	function getStackTrace(error): Array<string> {
		return error.stack.split('\n');
	}

	function createDeleterFunction(index: number): () => void {
		return function() {
	 ...
	 */
	function createCallback(callback: (status: string, data: any, stacktrace: Array<string>) => void,
							error, persistent?: boolean): Function {
		...
		stringifiedFn.replace(/CBID/, String(index));
		stringifiedFn.replace(/TABID/, String(tabData.id));
		stringifiedFn.replace(/NODEID/, String(id));
	 ...
	 * @returns {Function} - The value to use as a callback function
	 */
	function createCallbackFunction(callback: MessageHandler, error, persistent?: boolean) {
		function onFinish(status: string, messageOrParams: Array<any>|any, stackTrace: Array<string>) {
			if (status === 'error') {
	...
	function messageHandler(message) {
		if (queue) { //Message queue is not empty
			...
			var instanceArr: Array<number> = message.instances;
	...
	type EmptyFn = () => void;
	var emptyFn: EmptyFn = function(...params) {};
	 ...
	 */
	function checkType(value: any, type: Array<string>|string, nameOrMode: string|boolean): boolean {
	 ...
	 */
	function lookup<T>(path: Array<string|number>, data: Array<T>|{ [key: string]: T }|{ [key: number]: T }, hold?: boolean): T|Array<T>|{ [key: string]: T }|{ [key: number]: T } {
		checkType(path, 'array', 'path');
		checkType(data, 'Object', 'data');
		var length = path.length;
		hold && length--;
		var dataChild: T|Array<T>|{ [key: string]: T }|{ [key: number]: T } = data;
	 ...
	 */
	function isFn(fn: Maybe<Function>): fn is Function {
	 ...
	 */
	this.comm = {};
	var commListeners: Array<Function> = [];
	var backgroundPageListeners: Array<Function> = [];
	...
	function generateBackgroundResponse(message): (data) => void {
	...
	function generateSendInstanceMessageFunction(instanceId: number): (message, callback: Function) => void {
		return function(message, callback) {
			sendInstanceMessage(instanceId, message, callback);
		}
	}

	function sendInstanceMessage(instanceId: number, message, callback: Function) {
	...
	function updateCommHandlerStatus(hasHandler: boolean) {
		sendMessage({
			id: id,
	 ...
	 */
	this.comm.getInstances = function(): Array<Instance> {
		return instances;
	}
	 ...
	 */
	this.comm.sendMessage = function (instance: number|Instance, message, callback: Function) {
		var instanceObj;
	 ...
	 */
	this.comm.addListener = function(listener: MessageHandler): number {
	 ...
	 * 		by adding it.
	 */
	this.comm.removeListener = function(listener: number|MessageHandler) {
	 ...
	 */
	this.comm.messageBackgroundPage = function (message, response: MessageHandler) {
		if (isBackground) {
	 ...
	 */
	this.comm.listenAsBackgroundPage = function (callback: MessageHandler) {
		if (isBackground) {
	...
	//#region Storage
	var storage: NodeStorage = nodeStorage;
	 ...
	 */
	function notifyChanges(keyPath: string, oldValue: any, newValue: any, remote?: boolean) {
	 ...
	 */
	this.storage.get = function (keyPath: (string|Array<string>|Array<number>)): any {
		if (!keyPath) {
			return storage;
		}
		if (checkType(keyPath, 'string', true)) {
			var keyPathString = keyPath;
			if (typeof keyPathString === 'string') { //Shutting up typescript
	 ...
	 */
	this.storage.set = function (keyPath: (string|Array<string>|Array<number>|{ [key: string]: any }|{ [key: number]: any}), value?: any): void {
		if (checkType(keyPath, 'string', true)) {
		...
		if (checkType(keyPath, 'array', true)) {
			...
			if (Array.isArray(keyPathArr)) {
				var data: Array<any>|{ [key: string]: any }|{ [key: number]: any } = lookup(keyPathArr, nodeStorage, true);
	 ...
	 *		one section of the path, or just a plain string without dots as the key
	 */
	this.storage.remove = function (keyPath: (string|Array<string>|Array<number>)) {
		if (checkType(keyPath, 'string', true)) {
			var keyPathStr = keyPath;
			if (typeof keyPathStr === 'string') {
				if (keyPathStr.indexOf('.') === -1) {
	 ...
	 */
	this.storage.onChange.addListener = function (listener: StorageListener, key: string): number {
	 ...
	 */
	this.storage.onChange.removeListener = function (listener: StorageListener|number, key) {
		var indexes;
		if (typeof listener === 'number') {
			indexes = [listener];
		} else {
	 ...
	 */
	this.getSelection = function (): string {
	 ...
	 */
	this.getClickInfo = function (): ClickData {
		return clickData;
	}
	 ...
	 */
	this.getTabInfo = function (): TabData {
		return tabData;
	}
	 ...
	 */
	function sendCrmMessage(action: string, callback: Function, params?: any) {
	...
	*/
	 ...
	 */
	this.crm.getTree = function (callback: (data: any) => void) {
	 ...
	 * @param {function} callback - A function that is called when done with the data as an argument
	 */
	this.crm.getSubTree = function (nodeId: number, callback: (data: any) => void) {
	 ...
	 */
	this.crm.getNode = function (nodeId: number, callback: () => void) {
	 ...
	 */
	this.crm.getNodeIdFromPath = function (path: Array<number>, callback: (id: number) => void) {
	 ...
	 */
	this.crm.queryCrm = function (query: { name?: string, type?: Type, inSubTree?: number}, callback: (results: Array<SafeCRMNode>) => void) {
	 ...
	 */
	this.crm.getParentNode = function (nodeId: number, callback: (node: SafeCRMNode) => void) {
	 ...
	 */
	this.crm.getChildren = function (nodeId: number, callback: (result: Array<SafeCRMNode>) => void) {
	 ...
	 */
	this.crm.getNodeType = function (nodeId: number, callback: (type: Type) => void) {
	 ...
	 */
	this.crm.getNodeValue = function (nodeId: number, callback: (value: LinkVal|ScriptVal|StylesheetVal|{}) => void) {
	 ...
	 */
	this.crm.createNode = function (options: {
										position?: Relation;
										name?: string;
										type?: string;
									...
									}, callback: (node: SafeCRMNode) => void) {
	 ...
	 * @param {CrmAPIInit~crmCallback} callback - A callback given the new node as an argument
	 */
	this.crm.copyNode = function (nodeId: number, options: {
													name?: string;
													position?: Relation
												}, callback: (node: SafeCRMNode) => void) {
		options = options || {};
	 ...
	 */
	this.crm.moveNode = function (nodeId: number, position: Relation, callback: (node: SafeCRMNode) => void) {
		//To prevent the user's stuff from being disturbed if they re-use the object
		var	positionCopy;
		if (position) {
	 ...
	 */
	this.crm.deleteNode = function (nodeId: number, callback: (result: boolean|string) => void) {
	 ...
	 */
	this.crm.editNode = function (nodeId: number, options: { name?: string, type?: string }, callback: (node: SafeCRMNode) => void) {
		options = options || {};
		//To prevent the user's stuff from being disturbed if they re-use the object
		var optionsCopy = JSON.parse(JSON.stringify(options));
	 ...
	 */
	this.crm.getTriggers = function (nodeId: number, callback: (triggers: CRMTrigger) => void) {
	 ...
	 */
	this.crm.setTriggers = function (nodeId: number, triggers: Array<{url: string, not: boolean}>, callback: (node: SafeCRMNode) => void) {
	 ...
	 */
	this.crm.stylesheet.getTriggerUsage = function (nodeId: number, callback: (usage: boolean) => void) {
	 ...
	 * @param {CrmAPIInit~crmCallback} callback - A function to run when done, with the node as an argument
	 */
	this.crm.stylesheet.setTriggerUsage = function (nodeId: number, useTriggers: boolean, callback: (node: SafeCRMNode) => void) {
	 ...
	 */
	this.crm.getContentTypes = function (nodeId: number, callback: (contentTypes: CRMContentTypes) => void) {
	 ...
	 */
	this.crm.setContentType = function (nodeId: number, index: number, value: boolean, callback: (contentTypes: CRMContentTypes) => void) {
	 ...
	 * @param {CrmAPIInit~crmCallback} callback - A function to run when done, with the node as an argument
	 */
	this.crm.setContentTypes = function (nodeId: number, contentTypes: CRMContentTypes, callback: (node: SafeCRMNode) => void) {
	 ...
	 */
	this.crm.link.getLinks = function (nodeId: number, callback: (result: Array<CRMLinkValueInstance>) => void) {
	 ...
	 */
	this.crm.link.push = function (nodeId: number, items: CRMLinkValueInstance|Array<CRMLinkValueInstance>,
									 callback: (arr: Array<CRMLinkValueInstance>) => void) {
	 ...
	 */
	this.crm.link.splice = function (nodeId: number, start: number, amount: number,
									 callback: (spliced: Array<CRMLinkValueInstance>, newArr: Array<CRMLinkValueInstance>) => void) {
	 ...
	 */
	this.crm.script.setLaunchMode = function (nodeId: number, launchMode: number, callback: (node: SafeCRMNode) => void) {
	 ...
	 */
	this.crm.script.getLaunchMode = function (nodeId: number, callback: (launchMode: number) => void) {
	 ...
	 */
	this.crm.script.libraries.push = function (nodeId: number, libraries: Library|Array<Library>,
												callback: (libs: Array<Library>) => void) {
	 ...
	 * @param {function} callback - A function that gets called with the spliced items as the first parameter and the new array as the second parameter
	 */
	this.crm.script.libraries.splice = function (nodeId: number, start: number, amount: number,
													callback: (spliced: Array<Library>, newArr: Array<Library>) => void) {
	 ...
	 * @param {function} callback - A callback with the new array as an argument
	 */
	this.crm.script.backgroundLibraries.push = function (nodeId: number, libraries: Library|Array<Library>,
												callback: (libs: Array<Library>) => void) {
	 ...
	 * @param {function} callback - A function that gets called with the spliced items as the first parameter and the new array as the second parameter
	 */
	this.crm.script.backgroundLibraries.splice = function (nodeId: number, start: number, amount: number,
													callback: (spliced: Array<Library>, newArr: Array<Library>) => void) {
	 ...
	 */
	this.crm.script.setScript = function (nodeId: number, script: string, callback: (node: SafeCRMNode) => void) {
	 ...
	 */
	this.crm.script.getScript = function (nodeId: number, callback: (script: string) => void) {
	 ...
	 */
	this.crm.script.getScript = function (nodeId: number, callback: (backgroundScript: string) => void) {
	 ...
	 */
	this.crm.menu.getChildren = function (nodeId: number, callback: (nodes: Array<SafeCRMNode>) => void) {
	 ...
	 * @param {CrmAPIInit~crmCallback} callback - A callback with the node as an argument
	 */
	this.crm.menu.setChildren = function (nodeId: number, childrenIds: Array<number>, callback: (node: SafeCRMNode) => void) {
	 ...
	 * @param {CrmAPIInit~crmCallback} callback - A callback with the node as an argument
	 */
	this.crm.menu.push = function (nodeId: number, childrenIds: Array<number>, callback: (node: SafeCRMNode) => void) {
	 ...
	 * @param {function} callback - A function that gets called with the spliced items as the first parameter and the new array as the second parameter
	 */
	this.crm.menu.splice = function (nodeId: number, start: number, amount: number,
									 callback: (spliced: Array<SafeCRMNode>, newArr: Array<SafeCRMNode>) => void) {
	 ...
	 */
	this.libraries.register = function (name: string, options: {url: string, code: string},
										callback: (lib: Library) => void) {
	...
	function genChromeRequest(api: string, type?: string) {
		var request = {
			api: api,
			chromeAPIArguments: []
		};
		...
		var fn: ChromeRequestReturn;
		fn = (() => {
			var _fn: any = function() {
	 ...
	 */
	function chromeReturnFunction(fn: (returnValue: any) => void) {
	 ...
	 */
	this.chrome = function (api: string) {
		return genChromeRequest(api);
	};

	function chromeSpecialRequest(api: string, type: string) {
	 ...
	 */
	this.GM.GM_info = function (): GreaseMonkeyDataInfo {
	 ...
	 */
	this.GM.GM_getValue = function <T>(name: string, defaultValue:T): any|T|void {
	 ...
	 */
	this.GM.GM_setValue = function (name: string, value: any) {
	 ...
	 */
	this.GM.GM_deleteValue = function (name: string) {
	 ...
	 */
	this.GM.GM_listValues = function (): Array<string> {
		var keys = [];
	 ...
	 */
	this.GM.GM_getResourceURL = function (name: string): string {
	 ...
	 */
	this.GM.GM_getResourceString = function (name: string): string {
	 ...
	 */
	this.GM.GM_addStyle = function (css: string) {
	 ...
	 */
	this.GM.GM_openInTab = function (url: string) {
		window.open(url);
	}
	 ...
	 */
	this.GM.GM_addValueChangeListener = function (name: string, callback: StorageListener): number {
	 ...
	 */
	this.GM.GM_removeValueChangeListener = function (listenerId: number) {
	 ...
	 */
	this.GM.GM_download = function (detailsOrUrl: DownloadSettings|string, name: string) {
	...
	function instantCb(cb: Function) {
		cb();
	}
	 ...
	 */
	function addNotificationListener(notificationId: number, onclick: Function, ondone: Function) {
		sendMessage({
			id: id,
	 ...
	 */
	this.GM.GM_notification = function (text: string|NotificationOptions, title: string, image: string, onclick: Function) {
		var details;
	 ...
	 */
	this.$crmAPI = function(selector, context): Array<HTMLElement>|void {
