	...
	function CodeMirror(place, options) {
		...
		function colorFunction(change) {
			setTimeout(function () {
				...
				for (var i = change.from.line; i < change.to.line + 1; i++) {
					line = $(lines[i]).children('pre').children('span');

					childNodes = line.find('span').toArray();
					for (var j = 0; j < childNodes.length; j++) {
						node = childNodes[j];
						if (node.className === 'cm-atom') {
							if (node.innerHTML === 'rgb' && childNodes[j + 1].className === 'cm-number' && childNodes[j + 2].className === 'cm-number' && childNodes[j + 3].className === 'cm-number') {
