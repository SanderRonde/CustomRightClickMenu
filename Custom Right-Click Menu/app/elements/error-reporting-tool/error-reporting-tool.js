Polymer({
	...
	scaleScreenshot: function (canvas, context, newImg, callback) {
	...
	translateX: function(el, x) {
		el.xPos = x;
		el.style.transfrom = 'translate(' + x + ',' + (el.yPos || 0) + ')';
	},

	translateY: function(el, y) {
		el.yPos = y;
		el.style.transform = 'translate(' + (el.xPos || 0) + ',' + y + ')';
	},

	setSelection: function (startX, startY, width, height, posX, posY) {
	...
	convertImageToBlob: function(dataURI) {
	...
	getDownloadPermission: function (callback) {
		var _this = this;
