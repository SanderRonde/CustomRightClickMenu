(function() {
	...
	Polymer({
		...
		parseOldCRMNode: function(string) {
			var node = {};

			var oldNodeSplit = string.split('%123');
			node.name = oldNodeSplit[0];
			node.type = oldNodeSplit[1].toLowerCase();
		...
		transferCRMFromOld: function() {
			var amount = localStorage.getItem('numberofrows');

			var nodes = [];
			for (var i = 0; i < amount; i++) {
				nodes.push(parseOldCRMNode(localStorage.getItem(i)));
			}
		},
		...
		handleDataTransfer: function(_this) {
			...
			var defaultSyncStorage = {
				...
				openInCurrentTab: (localStorage.getItem('whatpage') !== 'false'),
				showOptions: (localStorage.getItem('optionson') !== 'false'),
				shrinkTitleRibbon: false,
				crm: [
					_this.transferCRMFromOld()
				]
			};
